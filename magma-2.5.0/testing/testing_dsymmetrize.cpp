/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zsymmetrize.cpp, normal z -> d, Sun Sep 15 17:28:13 2019
       @author Mark Gates
            ...
            for( int j = 0; j < N; ++j ) {
                for( int i = 0; i < j; ++i ) {
                    if ( opts.uplo == MagmaLower ) {
                        h_A[i + j*lda] = MAGMA_D_CONJ( h_A[j + i*lda] );
                    }
                    else {
                        h_A[j + i*lda] = MAGMA_D_CONJ( h_A[i + j*lda] );
                    }
                }
                // real diagonal
                h_A[j + j*lda] = MAGMA_D_MAKE( MAGMA_D_REAL( h_A[j + j*lda] ), 0 );
            }

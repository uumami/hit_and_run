/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zhemv_vbatched.cpp, normal z -> s, Sun Sep 15 17:31:31 2019
       @author Mark Gates
       @author Azzam Haidar
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (magma_int_t i = 0; i < batchCount; i++) {
                ...
                if ( opts.uplo == MagmaUpper ) {
                    lapackf77_slaset( "Lower", &Ni, &Ni, &MAGMA_S_NAN, &MAGMA_S_NAN, &hAi[1], &h_lda[i] );
                }
                else {
                    lapackf77_slaset( "Upper", &Ni, &Ni, &MAGMA_S_NAN, &MAGMA_S_NAN, &hAi[ h_lda[i] ], &h_lda[i] );
                }
                hAi += h_lda[i] * h_N[i];
            }
            ...
            for (int s = 0; s < batchCount; ++s) {
                Anorm[s] = safe_lapackf77_slansy( "F", lapack_uplo_const(opts.uplo), &h_N[s], h_A_tmp, &h_lda[s], work );
                Xnorm[s] = lapackf77_slange( "F", &ione, &h_N[s], h_X_tmp, &h_incx[s], work );
                Ynorm[s] = lapackf77_slange( "F", &ione, &h_N[s], h_Y_tmp, &h_incy[s], work );
            ...
            h_A_tmp = h_A;
            for (int i = 0; i < batchCount; i++) {
                magma_ssetmatrix( h_N[i], h_N[i], h_A_tmp, h_lda[i], h_A_array[i], h_ldda[i], opts.queue );
            ...
            #if 0
            magmablas_ssymv_vbatched_max_nocheck(opts.uplo, d_N,
                             alpha, d_A_array, d_ldda,
                                    d_X_array, d_incx,
                             beta,  d_Y_array, d_incy,
                             batchCount, max_N, opts.queue);
            #else
            magmablas_ssymv_vbatched(opts.uplo, d_N,
                             alpha, d_A_array, d_ldda,
                                    d_X_array, d_incx,
                             beta,  d_Y_array, d_incy,
            ...
            if ( opts.lapack ) {
                ...
                {
                    blasf77_ssymv( lapack_uplo_const(opts.uplo), &h_N[s],
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++){
                    ...
                    if (normalize == 0)
                        normalize = 1;
                    blasf77_saxpy( &h_N[s], &c_neg_one, h_Y_tmp, &h_incy[s], h_Ymagma_tmp, &h_incy[s] );
                    error = lapackf77_slange( "F", &ione, &h_N[s], h_Ymagma_tmp, &h_incy[s], work )
                          / normalize;

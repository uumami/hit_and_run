/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zher2k_mgpu.cpp, normal z -> c, Sun Sep 15 17:27:46 2019
...
int main( int argc, char** argv)
{
    ...
    float           error, work[1];
    magmaFloatComplex *hC, *hR, *hR2, *hA, *hB;
    magmaFloatComplex_ptr dA[MagmaMaxGPUs], dB[MagmaMaxGPUs], dC[MagmaMaxGPUs];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_CHECK( magma_cmalloc_cpu( &hC,  lda*n ));
                TESTING_CHECK( magma_cmalloc_cpu( &hR,  lda*n ));
                TESTING_CHECK( magma_cmalloc_cpu( &hR2, lda*n ));
                TESTING_CHECK( magma_cmalloc_cpu( &hA,  lda*k ));
                TESTING_CHECK( magma_cmalloc_cpu( &hB,  lda*k ));
                for( int dev = 0; dev < ngpu; ++dev ) {
                    magma_int_t nlocal = ((n / nb) / ngpu + 1) * nb;
                    magma_setdevice( dev );
                    TESTING_CHECK( magma_cmalloc( &dC[dev], ldda*nlocal ));
                    TESTING_CHECK( magma_cmalloc( &dA[dev], ldda*k*2    ));
                    //TESTING_CHECK( magma_cmalloc( &dB[dev], ldda*k      ));
                ...
                // for error checks
                float Anorm = lapackf77_clange( "F", &n, &k, hA, &lda, work );
                float Bnorm = lapackf77_clange( "F", &n, &k, hB, &lda, work );
                float Cnorm = safe_lapackf77_clanhe( "F", lapack_uplo_const(opts.uplo), &n, hC, &lda, work );
                
                /* ====================================================================
                   Performs operation using MAGMA
                   =================================================================== */
                magma_csetmatrix_1D_col_bcyclic( ngpu, n, n, nb, hC, lda, dC, ldda, queues0 );
                for( int dev = 0; dev < ngpu; ++dev ) {
                    magma_setdevice( dev );
                    dB[dev] = dA[dev] + ldda*k;
                    magma_csetmatrix( n, k, hA, lda, dA[dev], ldda, opts.queue );
                    magma_csetmatrix( n, k, hB, lda, dB[dev], ldda, opts.queue );
                }
                ...
                if ( opts.version == 1 ) {
                    ...
                    printf( "magmablas_cher2k_mgpu_spec not compiled\n" );
                ...
                magma_cgetmatrix_1D_col_bcyclic( ngpu, n, n, nb, dC, ldda, hR, lda, queues0 );
                ...
                if ( opts.lapack || opts.check ) {
                    ...
                    Anorm  = lapackf77_clange("f", &n_offset, &k, hA, &lda, work );
                    Anorm *= lapackf77_clange("f", &n_offset, &k, hB, &lda, work );
                    Anorm += lapackf77_clange("f", &n_offset, &n_offset, &hC[offset + offset*lda], &lda, work );
                    ...
                    error = safe_lapackf77_clanhe("fro", "Lower", &n_offset, &hR[offset + offset*lda], &lda, work)

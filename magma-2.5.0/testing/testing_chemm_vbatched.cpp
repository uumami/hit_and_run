/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zhemm_vbatched.cpp, normal z -> c, Sun Sep 15 17:31:26 2019
*/
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex *h_A_tmp, *h_B_tmp, *h_C_tmp, *h_Cmagma_tmp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                gflops += FLOPS_CHEMM(opts.side, h_M[i], h_N[i]) / 1e9;
            ...
            for (int s = 0; s < batchCount; ++s) {
                Anorm[s] = safe_lapackf77_clanhe( "F", lapack_uplo_const(opts.uplo), &An[s], h_A_tmp, &h_lda[s], work );
                Bnorm[s] = lapackf77_clange( "F", &h_M[s], &h_N[s], h_B_tmp, &h_ldb[s], work );
                Cnorm[s] = lapackf77_clange( "F", &h_M[s], &h_N[s], h_C_tmp, &h_ldc[s], work );
            ...
            h_C_tmp = h_C;
            for (int i = 0; i < batchCount; i++) {
                magma_csetmatrix( An[i],  An[i],  h_A_tmp, h_lda[i], h_A_array[i], h_ldda[i], opts.queue );
                magma_csetmatrix( h_M[i], h_N[i], h_B_tmp, h_ldb[i], h_B_array[i], h_lddb[i], opts.queue );
                magma_csetmatrix( h_M[i], h_N[i], h_C_tmp, h_ldc[i], h_C_array[i], h_lddc[i], opts.queue );
            ...
            #if 0
            magmablas_chemm_vbatched_max_nocheck(opts.side, opts.uplo,
                             d_M, d_N,
                             ...
                             opts.queue);
            #else
            magmablas_chemm_vbatched(opts.side, opts.uplo,
                             d_M, d_N,
                             alpha, d_A_array, d_ldda,
                                    d_B_array, d_lddb,
                             beta,  d_C_array, d_lddc,
            ...
            h_C_tmp = h_Cmagma;
            for (int i = 0; i < batchCount; i++) {
                magma_cgetmatrix( h_M[i], h_N[i], h_C_array[i], h_lddc[i], h_C_tmp, h_ldc[i], opts.queue );
                h_C_tmp += h_N[i] * h_ldc[i];
            }
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++) {
                    normalize = sqrt(float(An[s]+2))*fabs(alpha)*Anorm[s]*Bnorm[s] + 2*fabs(beta)*Cnorm[s];
                    if (normalize == 0)
                        normalize = 1;
                    magma_int_t Csize = h_ldc[s] * h_N[s];
                    blasf77_caxpy( &Csize, &c_neg_one, h_C_tmp, &ione, h_Cmagma_tmp, &ione );
                    error = lapackf77_clange( "F", &h_M[s], &h_N[s], h_Cmagma_tmp, &h_ldc[s], work )
                          / normalize;

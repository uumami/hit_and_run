/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zswap.cpp, normal z -> c, Sun Sep 15 17:28:17 2019
       @author Mark Gates
*/
// includes, system
#include <stdlib.h>
...
static void init_matrix(
    magma_int_t m, magma_int_t n,
    magmaFloatComplex *A, magma_int_t lda, magma_int_t offset )
{
    assert( lda >= m );
    for( magma_int_t j = 0; j < n; ++j ) {
        for( magma_int_t i=0; i < m; ++i ) {
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_cmalloc_pinned( &h_A1, lda*N ));
            TESTING_CHECK( magma_cmalloc_pinned( &h_A2, lda*N ));
            TESTING_CHECK( magma_cmalloc_pinned( &h_R1, lda*N ));
            TESTING_CHECK( magma_cmalloc_pinned( &h_R2, lda*N ));
            
            TESTING_CHECK( magma_imalloc_cpu( &ipiv,  nb ));
            TESTING_CHECK( magma_imalloc_cpu( &ipiv2, nb ));
            
            TESTING_CHECK( magma_imalloc( &d_ipiv, nb ));
            TESTING_CHECK( magma_cmalloc( &d_A1, ldda*N ));
            TESTING_CHECK( magma_cmalloc( &d_A2, ldda*N ));
            ...
            magma_csetmatrix( N, N, h_A1, lda, d_A1(0,0), ldda, opts.queue );
            magma_csetmatrix( N, N, h_A2, lda, d_A2(0,0), ldda, opts.queue );
            
            time = magma_sync_wtime( opts.queue );
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magma_cswap( N, d_A1(0,j), 1, d_A2(0,ipiv[j]-1), 1, opts.queue );
                }
            }
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    blasf77_cswap( &N, h_A1(0,j), &ione, h_A2(0,ipiv[j]-1), &ione);
                }
            }
            magma_cgetmatrix( N, N, d_A1(0,0), ldda, h_R1, lda, opts.queue );
            magma_cgetmatrix( N, N, d_A2(0,0), ldda, h_R2, lda, opts.queue );
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magma_cswap( N, d_A1(j,0), ldda, d_A2(ipiv[j]-1,0), ldda, opts.queue );
                }
            }
            time = magma_sync_wtime( opts.queue ) - time;
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magmablas_cswap( N, d_A1(0,j), 1, d_A2(0,ipiv[j]-1), 1, opts.queue );
                }
            }
            time = magma_sync_wtime( opts.queue ) - time;
            ...
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    magmablas_cswap( N, d_A1(j,0), ldda, d_A2(ipiv[j]-1,0), ldda, opts.queue );
                }
            }
            time = magma_sync_wtime( opts.queue ) - time;
             ...
             */
                ...
                time = magma_sync_wtime( opts.queue );
                magmablas_cswapblk( MagmaRowMajor, N, d_A1(0,0), ldda, d_A2(0,0), ldda, 1, nb, ipiv, 1, 0, opts.queue );
                time = magma_sync_wtime( opts.queue ) - time;
                ...
                time = magma_sync_wtime( opts.queue );
                magmablas_cswapblk( MagmaColMajor, N, d_A1(0,0), ldda, d_A2(0,0), ldda, 1, nb, ipiv, 1, 0, opts.queue );
                time = magma_sync_wtime( opts.queue ) - time;
                col_perf2 = gbytes / time;
                
                for( j=0; j < nb; j++) {
                    if ( j != (ipiv[j]-1)) {
                        blasf77_cswap( &N, h_A1(j,0), &lda, h_A2(ipiv[j]-1,0), &lda );
                    }
                }
                magma_cgetmatrix( N, N, d_A1(0,0), ldda, h_R1, lda, opts.queue );
                magma_cgetmatrix( N, N, d_A2(0,0), ldda, h_R2, lda, opts.queue );
            ...
            time = magma_sync_wtime( opts.queue );
            magmablas_claswp( N, d_A1(0,0), ldda, 1, nb, ipiv, 1, opts.queue );
            time = magma_sync_wtime( opts.queue ) - time;
            row_perf4 = gbytes / time;
            
            for( j=0; j < nb; j++) {
                if ( j != (ipiv[j]-1)) {
                    blasf77_cswap( &N, h_A1(0,j), &ione, h_A1(0,ipiv[j]-1), &ione );
                }
            }
            magma_cgetmatrix( N, N, d_A1(0,0), ldda, h_R1, lda, opts.queue );
            ...
            magmablas_claswp2( N, d_A1(0,0), ldda, 1, nb, d_ipiv(0), 1, opts.queue );
            time = magma_sync_wtime( opts.queue ) - time;
            ...
            time = magma_sync_wtime( opts.queue );
            magmablas_claswpx( N, d_A1(0,0), ldda, 1, 1, nb, ipiv, 1, opts.queue );
            time = magma_sync_wtime( opts.queue ) - time;
            ...
            time = magma_sync_wtime( opts.queue );
            magmablas_claswpx( N, d_A1(0,0), 1, ldda, 1, nb, ipiv, 1, opts.queue );
            time = magma_sync_wtime( opts.queue ) - time;
            ...
            time = magma_sync_wtime( opts.queue );
            magma_ccopymatrix( N, nb, d_A1(0,0), ldda, d_A2(0,0), ldda, opts.queue );
            time = magma_sync_wtime( opts.queue ) - time;
            // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap
            col_perf6 = 0.5 * gbytes / time;
            
            time = magma_sync_wtime( opts.queue );
            magma_ccopymatrix( nb, N, d_A1(0,0), ldda, d_A2(0,0), ldda, opts.queue );
            time = magma_sync_wtime( opts.queue ) - time;
            // copy reads 1 matrix and writes 1 matrix, so has half gbytes of swap

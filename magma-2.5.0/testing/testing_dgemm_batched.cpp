/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgemm_batched.cpp, normal z -> d, Sun Sep 15 17:30:43 2019
       @author Mark Gates
       @author Azzam Haidar
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if(opts.version == 1){
                cublasDgemmBatched(opts.handle, cublas_trans_const(opts.transA), cublas_trans_const(opts.transB),
                                   int(M), int(N), int(K),
                                   &alpha, (const double**) d_A_array, int(ldda),
                ...
                cublasDgemmStridedBatched(opts.handle, cublas_trans_const(opts.transA), cublas_trans_const(opts.transB),
                                   int(M), int(N), int(K),
                                   &alpha, (const double*) d_A, int(ldda), ldda * An, 

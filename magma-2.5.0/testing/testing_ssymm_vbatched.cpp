/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zhemm_vbatched.cpp, normal z -> s, Sun Sep 15 17:31:28 2019
*/
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                gflops += FLOPS_SSYMM(opts.side, h_M[i], h_N[i]) / 1e9;
            ...
            for (int s = 0; s < batchCount; ++s) {
                Anorm[s] = safe_lapackf77_slansy( "F", lapack_uplo_const(opts.uplo), &An[s], h_A_tmp, &h_lda[s], work );
            ...
            h_C_tmp = h_C;
            for (int i = 0; i < batchCount; i++) {
                magma_ssetmatrix( An[i],  An[i],  h_A_tmp, h_lda[i], h_A_array[i], h_ldda[i], opts.queue );
                magma_ssetmatrix( h_M[i], h_N[i], h_B_tmp, h_ldb[i], h_B_array[i], h_lddb[i], opts.queue );
            ...
            #if 0
            magmablas_ssymm_vbatched_max_nocheck(opts.side, opts.uplo,
                             d_M, d_N,
                             ...
                             opts.queue);
            #else
            magmablas_ssymm_vbatched(opts.side, opts.uplo,
                             d_M, d_N,
                             alpha, d_A_array, d_ldda,
                                    d_B_array, d_lddb,
                             beta,  d_C_array, d_lddc,

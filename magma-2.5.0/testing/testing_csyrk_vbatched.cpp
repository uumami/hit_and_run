/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zsyrk_vbatched.cpp, normal z -> c, Sun Sep 15 17:31:37 2019
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex **h_A_array;
    magmaFloatComplex **h_C_array;
    magmaFloatComplex **d_A_array;
    magmaFloatComplex **d_C_array;
    magmaFloatComplex *h_A_tmp, *h_C_tmp, *h_Cmagma_tmp;
    
    magmaFloatComplex c_neg_one = MAGMA_C_NEG_ONE;
    magmaFloatComplex alpha = MAGMA_C_MAKE(0.29, -0.48);
    magmaFloatComplex beta  = MAGMA_C_MAKE(-0.48, 0.29);
    int status = 0;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                gflops += FLOPS_CHERK( h_K[i], h_N[i] ) / 1e9;
            }
            ...
            #if 0
            magmablas_csyrk_vbatched_max_nocheck(opts.uplo, opts.transA, d_N, d_K,
                             alpha, d_A_array, d_ldda,
                             beta,  d_C_array, d_lddc,
                             batchCount, max_N, max_K, opts.queue);
            #else
            magmablas_csyrk_vbatched(opts.uplo, opts.transA, d_N, d_K,
                             alpha, d_A_array, d_ldda,
                             beta,  d_C_array, d_lddc,
            ...
            if ( opts.lapack ) {
                ...
                for (int i=0; i < batchCount; i++)
                {
                    blasf77_csyrk( lapack_uplo_const(opts.uplo),
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++) {
                    normalize = sqrt(float(h_K[s]+2))*Anorm[s]*Anorm[s] + 2*Cnorm[s];
                    if (normalize == 0)
                        normalize = 1;

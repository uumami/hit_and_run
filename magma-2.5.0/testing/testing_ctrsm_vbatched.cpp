/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_ztrsm_vbatched.cpp, normal z -> c, Sun Sep 15 17:31:42 2019
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex **hinvA_array = NULL;
    magmaFloatComplex **hwork_array = NULL;
    magmaFloatComplex **dinvA_array = NULL;
    magmaFloatComplex **dwork_array = NULL;
    
     magmaFloatComplex *h_A_tmp, *h_B_tmp;
    ...
    TESTING_CHECK( magma_malloc_cpu((void**)&hinvA_array, batchCount*sizeof(magmaFloatComplex*)) );
    TESTING_CHECK( magma_malloc_cpu((void**)&hwork_array, batchCount*sizeof(magmaFloatComplex*)) );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for(int k = 0; k < batchCount; k++)
            {
                ...
                gflops += FLOPS_CTRSM(opts.side, h_M[k], h_N[k]) / 1e9;
                h_invA_size[k] = magma_roundup( Ak[k], CTRTRI_BATCHED_NB )*CTRTRI_BATCHED_NB;
            ...
            TESTING_CHECK( magma_cmalloc_cpu(&h_B,  total_size_B_cpu) );
            TESTING_CHECK( magma_cmalloc_cpu(&h_X,  total_size_B_cpu) );
            TESTING_CHECK( magma_cmalloc_cpu(&h_Blapack,  total_size_B_cpu) );
            TESTING_CHECK( magma_cmalloc_cpu(&h_Bcublas,  total_size_B_cpu) );
            TESTING_CHECK( magma_cmalloc_cpu(&h_Bmagma,  total_size_B_cpu) );
            TESTING_CHECK( magma_imalloc_cpu(&ipiv,  max_Ak) );
            
            TESTING_CHECK( magma_cmalloc(&d_A, total_size_A_dev) );
            TESTING_CHECK( magma_cmalloc(&d_B, total_size_B_dev) );
            ...
            TESTING_CHECK( magma_cmalloc( &dinvA, dinvA_batchSize ));
            TESTING_CHECK( magma_cmalloc( &dwork, dwork_batchSize ));
            ...
            magma_setvector(batchCount, sizeof(magmaFloatComplex*), h_B_array, 1, d_B_array, 1, opts.queue);
            magma_setvector(batchCount, sizeof(magmaFloatComplex*), hinvA_array, 1, dinvA_array, 1, opts.queue);
            magma_setvector(batchCount, sizeof(magmaFloatComplex*), hwork_array, 1, dwork_array, 1, opts.queue);
            
            memset(h_Bmagma, 0, total_size_B_cpu*sizeof(magmaFloatComplex));
            for(int k = 0; k < batchCount; k++)
            {
                magmablas_claset( MagmaFull, h_lddb[k], h_N[k], c_zero, c_zero, hwork_array[k], h_lddb[k], opts.queue);
            }
            ...
            for (int k=0; k < batchCount; k++)
            {
                lapackf77_cgetrf( &Ak[k], &Ak[k], h_A_tmp, &h_lda[k], ipiv, &info );
            ...
            lapackf77_clarnv( &ione, ISEED, &total_size_B_cpu, h_B );
            memcpy( h_Blapack, h_B, total_size_B_cpu*sizeof(magmaFloatComplex) );
            ...
            for(int k = 0; k < batchCount; k++)
            {
                magma_csetmatrix( Ak[k], Ak[k], h_A_tmp, h_lda[k], h_A_array[k], h_ldda[k], opts.queue);
                magma_csetmatrix( h_M[k],  h_N[k], h_B_tmp, h_ldb[k], h_B_array[k], h_lddb[k], opts.queue);
                h_A_tmp += Ak[k] * h_lda[k];
            ...
            if (opts.version == 1) {
                ...
                h_B_tmp = h_Bmagma;
                magmaFloatComplex *work_tmp = dwork;
                for(int k = 0; k < batchCount; k++)
                {
                    magma_cgetmatrix( h_M[k], h_N[k], work_tmp, h_lddb[k], h_B_tmp, h_ldb[k], opts.queue);
                    h_B_tmp  += h_N[k] * h_ldb[k];
                ...
                h_B_tmp = h_Bmagma;
                magmaFloatComplex *d_B_tmp = d_B;
                for(int k = 0; k < batchCount; k++)
                {
                    magma_cgetmatrix( h_M[k], h_N[k], d_B_tmp, h_lddb[k], h_B_tmp, h_ldb[k], opts.queue);
                    h_B_tmp  += h_N[k] * h_ldb[k];
            ...
            if ( opts.lapack )
            {
                ...
                magma_error  = 0;
                memcpy( h_X, h_Bmagma, total_size_B_cpu*sizeof(magmaFloatComplex) );
                magmaFloatComplex *h_X_tmp = h_X;
                magmaFloatComplex *h_Bmagma_tmp = h_Bmagma;
                h_B_tmp = h_B;
                // check magma
                for (magma_int_t s=0; s < batchCount; s++)
                {
                    ...
                    blasf77_ctrmm(
                        ...
                        h_A_array[s], &h_lda[s],
                        h_X_tmp,      &h_ldb[s] );
                    blasf77_caxpy( &NN, &c_neg_one, h_B_tmp, &ione, h_X_tmp, &ione );
    
                    normR = lapackf77_clange( "M", &h_M[s], &h_N[s], h_X_tmp,      &h_ldb[s], work );
                    normX = lapackf77_clange( "M", &h_M[s], &h_N[s], h_Bmagma_tmp, &h_ldb[s], work );
                ...
                magmaFloatComplex *h_Blapack_tmp = h_Blapack;
                h_B_tmp = h_B;
                h_X_tmp = h_X;
                
                lapack_error = 0;
                memcpy( h_X, h_Blapack, total_size_B_cpu*sizeof(magmaFloatComplex) );
                for (magma_int_t s=0; s < batchCount; s++)
                {
                    magma_int_t NN = h_ldb[s]*h_N[s];
                    normA = lapackf77_clantr( "M",
                    ...
                    blasf77_caxpy( &NN, &c_neg_one, h_B_tmp, &ione, h_X_tmp, &ione );
                    normR = lapackf77_clange( "M", &h_M[s], &h_N[s], h_X_tmp,       &h_ldb[s], work );
                    normX = lapackf77_clange( "M", &h_M[s], &h_N[s], h_Blapack_tmp, &h_ldb[s], work );
                    normalize = normX*normA;
                    if (normalize == 0)
                        normalize = 1;
                    error = normR / normalize;

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgemm_vbatched.cpp, normal z -> s, Sun Sep 15 17:31:23 2019
       @author Mark Gates
       @author Azzam Haidar
...
int main( int argc, char** argv)
{
    ...
    float *d_A, *d_B, *d_C;
    float *h_A_tmp, *h_B_tmp, *h_C_tmp, *h_Cmagma_tmp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                gflops += FLOPS_SGEMM( h_M[i], h_N[i], h_K[i] ) / 1e9;
            ...
            for (int s = 0; s < batchCount; ++s) {
                Anorm[s] = lapackf77_slange( "F",  &Am[s],  &An[s], h_A_tmp, &h_lda[s], work );
                Bnorm[s] = lapackf77_slange( "F",  &Bm[s],  &Bn[s], h_B_tmp, &h_ldb[s], work );
                Cnorm[s] = lapackf77_slange( "F", &h_M[s], &h_N[s], h_C_tmp, &h_ldc[s], work );
            ...
            h_C_tmp = h_C;
            for (int i = 0; i < batchCount; i++) {
                magma_ssetmatrix( Am[i],  An[i],  h_A_tmp, h_lda[i], h_A_array[i], h_ldda[i], opts.queue );
                magma_ssetmatrix( Bm[i],  Bn[i],  h_B_tmp, h_ldb[i], h_B_array[i], h_lddb[i], opts.queue );
                magma_ssetmatrix( h_M[i], h_N[i], h_C_tmp, h_ldc[i], h_C_array[i], h_lddc[i], opts.queue );
            ...
            #if 0
            magmablas_sgemm_vbatched_max_nocheck(opts.transA, opts.transB,
                             d_M, d_N, d_K,
                             ...
                             opts.queue);
            #else
            magmablas_sgemm_vbatched(opts.transA, opts.transB,
                             d_M, d_N, d_K,
                             alpha, d_A_array, d_ldda,
                                    d_B_array, d_lddb,
                             beta,  d_C_array, d_lddc,
            ...
            h_C_tmp = h_Cmagma;
            for (int i = 0; i < batchCount; i++) {
                magma_sgetmatrix( h_M[i], h_N[i], h_C_array[i], h_lddc[i], h_C_tmp, h_ldc[i], opts.queue );
                h_C_tmp += h_N[i] * h_ldc[i];
            }
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++) {
                    ...
                    error = lapackf77_slange( "F", &h_M[s], &h_N[s], h_Cmagma_tmp, &h_ldc[s], work )
                          / normalize;

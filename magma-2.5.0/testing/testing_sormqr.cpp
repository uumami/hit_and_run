/*
    -- MAGMA (version 2.5.0) --
       ...
       @author Mark Gates
       @generated from testing/testing_zunmqr.cpp, normal z -> s, Sun Sep 15 17:30:01 2019
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_smalloc_cpu( &W,   lwork_max ));
            ...
            if (info != 0) {
                printf("magma_sormqr (lwork query) returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            ...
            if ( opts.ngpu == 1 ) {
                ...
                if ( side[iside] == MagmaLeft ) {
                    magma_sormqr_m( abs_ngpu, side[iside], trans[itran],
                                    m, n, k,
                                    A, lda, tau, R, ldc, W, lwork, &info );
                }
                else {
                    printf( "%5lld %5lld %5lld   %4c   %5c   skipping because magma_sormqr_m doesn't support MagmaRight\n",
                            (long long) m, (long long) n, (long long) k,
                            lapacke_side_const( side[iside] ),
                            lapacke_trans_const( trans[itran] ) );
                    goto cleanup;
            ...
            if (info != 0) {
                printf("magma_sormqr returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }

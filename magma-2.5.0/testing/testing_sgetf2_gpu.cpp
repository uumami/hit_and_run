/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgetf2_gpu.cpp, normal z -> s, Sun Sep 15 17:29:00 2019
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( N > 512 ) {
                printf( "%5lld %5lld   skipping because sgetf2 does not support N > 512\n", (long long) M, (long long) N );
                continue;
            }
            ...
            magma_sgetf2_gpu( M, N, d_A, ldda, ipiv, opts.queue, &info );
            gpu_time = magma_sync_wtime( opts.queue ) - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0) {
                printf("magma_sgetf2_gpu returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            
            real_Double_t get_time = magma_wtime();
            magma_sgetmatrix( M, N, d_A, ldda, h_A, lda, opts.queue );

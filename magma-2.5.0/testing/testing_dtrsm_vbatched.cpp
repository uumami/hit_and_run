/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_ztrsm_vbatched.cpp, normal z -> d, Sun Sep 15 17:31:43 2019
...
int main( int argc, char** argv)
{
    ...
    double **hinvA_array = NULL;
    double **hwork_array = NULL;
    double **dinvA_array = NULL;
    double **dwork_array = NULL;
    
     double *h_A_tmp, *h_B_tmp;
    ...
    TESTING_CHECK( magma_malloc_cpu((void**)&hinvA_array, batchCount*sizeof(double*)) );
    TESTING_CHECK( magma_malloc_cpu((void**)&hwork_array, batchCount*sizeof(double*)) );
    ...
    TESTING_CHECK( magma_malloc((void**)&dinvA_array, batchCount*sizeof(double*)) );
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for(int k = 0; k < batchCount; k++)
            {
                ...
                gflops += FLOPS_DTRSM(opts.side, h_M[k], h_N[k]) / 1e9;
                h_invA_size[k] = magma_roundup( Ak[k], DTRTRI_BATCHED_NB )*DTRTRI_BATCHED_NB;
            ...
            TESTING_CHECK( magma_dmalloc_cpu(&h_B,  total_size_B_cpu) );
            TESTING_CHECK( magma_dmalloc_cpu(&h_X,  total_size_B_cpu) );
            TESTING_CHECK( magma_dmalloc_cpu(&h_Blapack,  total_size_B_cpu) );
            TESTING_CHECK( magma_dmalloc_cpu(&h_Bcublas,  total_size_B_cpu) );
            TESTING_CHECK( magma_dmalloc_cpu(&h_Bmagma,  total_size_B_cpu) );
            TESTING_CHECK( magma_imalloc_cpu(&ipiv,  max_Ak) );
            
            TESTING_CHECK( magma_dmalloc(&d_A, total_size_A_dev) );
            TESTING_CHECK( magma_dmalloc(&d_B, total_size_B_dev) );
            
            TESTING_CHECK( magma_malloc( (void**) &dW1_displ,   batchCount * sizeof(double*) ));
            TESTING_CHECK( magma_malloc( (void**) &dW2_displ,   batchCount * sizeof(double*) ));
            TESTING_CHECK( magma_malloc( (void**) &dW3_displ,   batchCount * sizeof(double*) ));
            TESTING_CHECK( magma_malloc( (void**) &dW4_displ,   batchCount * sizeof(double*) ));
            
            double* dinvA=NULL;
            double* dwork=NULL; // invA and work are workspace in dtrsm
            
            TESTING_CHECK( magma_dmalloc( &dinvA, dinvA_batchSize ));
            TESTING_CHECK( magma_dmalloc( &dwork, dwork_batchSize ));
            ...
            magma_setvector(batchCount, sizeof(double*), h_B_array, 1, d_B_array, 1, opts.queue);
            magma_setvector(batchCount, sizeof(double*), hinvA_array, 1, dinvA_array, 1, opts.queue);
            magma_setvector(batchCount, sizeof(double*), hwork_array, 1, dwork_array, 1, opts.queue);
            
            memset(h_Bmagma, 0, total_size_B_cpu*sizeof(double));
            for(int k = 0; k < batchCount; k++)
            {
                magmablas_dlaset( MagmaFull, h_lddb[k], h_N[k], c_zero, c_zero, hwork_array[k], h_lddb[k], opts.queue);
            }
            ...
            for (int k=0; k < batchCount; k++)
            {
                lapackf77_dgetrf( &Ak[k], &Ak[k], h_A_tmp, &h_lda[k], ipiv, &info );
            ...
            lapackf77_dlarnv( &ione, ISEED, &total_size_B_cpu, h_B );
            memcpy( h_Blapack, h_B, total_size_B_cpu*sizeof(double) );
            ...
            for(int k = 0; k < batchCount; k++)
            {
                magma_dsetmatrix( Ak[k], Ak[k], h_A_tmp, h_lda[k], h_A_array[k], h_ldda[k], opts.queue);
                magma_dsetmatrix( h_M[k],  h_N[k], h_B_tmp, h_ldb[k], h_B_array[k], h_lddb[k], opts.queue);
                h_A_tmp += Ak[k] * h_lda[k];
            ...
            if (opts.version == 1) {
                ...
                h_B_tmp = h_Bmagma;
                double *work_tmp = dwork;
                for(int k = 0; k < batchCount; k++)
                {
                    magma_dgetmatrix( h_M[k], h_N[k], work_tmp, h_lddb[k], h_B_tmp, h_ldb[k], opts.queue);
                    h_B_tmp  += h_N[k] * h_ldb[k];
                ...
                for(int k = 0; k < batchCount; k++)
                {
                    magma_dgetmatrix( h_M[k], h_N[k], d_B_tmp, h_lddb[k], h_B_tmp, h_ldb[k], opts.queue);
                    h_B_tmp  += h_N[k] * h_ldb[k];
            ...
            if ( opts.lapack )
            {
                ...
                magma_error  = 0;
                memcpy( h_X, h_Bmagma, total_size_B_cpu*sizeof(double) );
                double *h_X_tmp = h_X;
                double *h_Bmagma_tmp = h_Bmagma;
                h_B_tmp = h_B;
                // check magma
                for (magma_int_t s=0; s < batchCount; s++)
                {
                    ...
                    blasf77_dtrmm(
                        ...
                        h_A_array[s], &h_lda[s],
                        h_X_tmp,      &h_ldb[s] );
                    blasf77_daxpy( &NN, &c_neg_one, h_B_tmp, &ione, h_X_tmp, &ione );
    
                    normR = lapackf77_dlange( "M", &h_M[s], &h_N[s], h_X_tmp,      &h_ldb[s], work );
                    normX = lapackf77_dlange( "M", &h_M[s], &h_N[s], h_Bmagma_tmp, &h_ldb[s], work );
                ...
                double *h_Blapack_tmp = h_Blapack;
                h_B_tmp = h_B;
                h_X_tmp = h_X;
                
                lapack_error = 0;
                memcpy( h_X, h_Blapack, total_size_B_cpu*sizeof(double) );
                for (magma_int_t s=0; s < batchCount; s++)
                {
                    magma_int_t NN = h_ldb[s]*h_N[s];
                    normA = lapackf77_dlantr( "M",
                    ...
                    blasf77_daxpy( &NN, &c_neg_one, h_B_tmp, &ione, h_X_tmp, &ione );
                    normR = lapackf77_dlange( "M", &h_M[s], &h_N[s], h_X_tmp,       &h_ldb[s], work );
                    normX = lapackf77_dlange( "M", &h_M[s], &h_N[s], h_Blapack_tmp, &h_ldb[s], work );
                    normalize = normX*normA;
                    if (normalize == 0)
                        normalize = 1;
                    error = normR / normalize;

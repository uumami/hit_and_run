/*
   ...
   @generated from testing/testing_zgetri_batched.cpp, normal z -> s, Sun Sep 15 17:31:15 2019
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {    
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_smalloc_cpu( &work,     lwork*batchCount ));
            ...
            TESTING_CHECK( magma_smalloc( &d_invA,   ldda*N * batchCount ));
            ...
            magma_ssetmatrix( N, columns, h_R, lda, d_A, ldda, opts.queue );

            /* ====================================================================
               Performs operation using MAGMA
               =================================================================== */
            magma_sset_pointer( dA_array, d_A, ldda, 0, 0, ldda * N, batchCount, opts.queue );
            magma_sset_pointer( dinvA_array, d_invA, ldda, 0, 0, ldda * N, batchCount, opts.queue );
            ...
            if (info1 != 0) printf("magma_sgetrf_batched returned argument error %lld: %s.\n", (long long) info1, magma_strerror( info1 ));
            if (info2 != 0) printf("magma_sgetri_batched returned argument error %lld: %s.\n", (long long) info2, magma_strerror( info2 ));
            ...
            if ( opts.check ) {
                ...
                magma_sgetmatrix( N, N*batchCount, d_invA, ldda, h_Ainv, lda, opts.queue );
                error = 0;
                for (magma_int_t i=0; i < batchCount; i++)
                {

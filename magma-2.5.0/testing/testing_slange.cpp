/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zlange.cpp, normal z -> s, Sun Sep 15 17:27:57 2019
       @author Mark Gates
*/
// includes, system
#include <stdlib.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      for( int inorm = 0; inorm < 3; ++inorm ) {  /* < 4 for Frobenius */
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_smalloc_cpu( &h_work, M ));
            ...
            norm_magma = magmablas_slange( norm[inorm], M, N, d_A, ldda, d_work, lwork, opts.queue );
            ...
            *h_A(i,j) = MAGMA_S_NAN;
            ...
            norm_magma  = magmablas_slange( norm[inorm], M, N, d_A, ldda, d_work, lwork, opts.queue );
            norm_lapack = lapackf77_slange( lapack_norm_const( norm[inorm] ),
                                            &M, &N, h_A, &lda, h_work );

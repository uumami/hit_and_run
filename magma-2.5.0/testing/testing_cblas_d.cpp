/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_cblas_z.cpp, normal z -> d, Sun Sep 15 17:27:50 2019
       @author Mark Gates
...
{
    // SKIPPED_FLAG indicates skipped, e.g., ddot with MKL -- it isn't an error
...
int main( int argc, char** argv )
{
    ...
    // tell user about disabled functions
        ...
        printf( "n/a: cblas_ddot, cblas_ddot, blasf77_ddot, and blasf77_ddot are disabled with MKL, due to segfaults.\n\n" );
    #endif
    
    #if defined(__APPLE__)
        printf( "n/a: blasf77_ddot and blasf77_ddot are disabled on MacOS, due to segfaults.\n\n" );
    #endif
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        TESTING_CHECK( magma_dmalloc_cpu( &B, size ));
            ...
            for( int iincy = 0; iincy < ninc; ++iincy ) {
                ...
                if ( incx > 0 && incx == incy ) {  // positive, no incy
                    ...
                    for( j=0; j < k; ++j ) {
                        ...
                        x_f = blasf77_dasum( &m, A(0,j), &incx );
                        error_fblas = max( error_fblas, fabs(x_m - x_f) / fabs(m*x_f) );
                        
                        // inline implementation
                        x_i = 0;
                        for( i=0; i < m; ++i ) {
                            x_i += MAGMA_D_ABS1( *A(i*incx,j) );  // |real(Aij)| + |imag(Aij)|
                        }
                        error_inline = max( error_inline, fabs(x_m - x_i) / fabs(m*x_i) );
                        
                        //printf( "dasum xm %.8e, xc %.8e, xf %.8e, xi %.8e\n", x_m, x_c, x_f, x_i );
                    }
                    output( "dasum", m, n, k, incx, incy, error_cblas, error_fblas, error_inline );
                }
            }
        }
        printf( "\n" );
            ...
            for( int iincy = 0; iincy < ninc; ++iincy ) {
                ...
                if ( incx > 0 && incx == incy ) {  // positive, no incy
                    ...
                    for( j=0; j < k; ++j ) {
                        ...
                        x_f = blasf77_dnrm2( &m, A(0,j), &incx );
                        error_fblas = max( error_fblas, fabs(x_m - x_f) / fabs(m*x_f) );
                        ...
                        //printf( "dnrm2 xm %.8e, xc %.8e, xf %.8e, xi %.8e\n", x_m, x_c, x_f, x_i );
                    }
                    output( "dnrm2", m, n, k, incx, incy, error_cblas, error_fblas, error_inline );
                }
            }
        }
        printf( "\n" );
            ...
            for( int iincy = 0; iincy < ninc; ++iincy ) {
                ...
                for( j=0; j < k; ++j ) {
                    ...
                    // crashes with MKL 11.2.3 and MacOS 10.9
                        ...
                        x2_f = blasf77_ddot( &m, A(0,j), &incx, B(0,j), &incy );
                    ...
                    //printf( "ddot xm %.8e + %.8ei, xc %.8e + %.8ei, xf %.8e + %.8ei, xi %.8e + %.8ei\n",
                    //        real(x2_m), imag(x2_m),
                    //        real(x2_c), imag(x2_c),
                    //        real(x2_f), imag(x2_f),
                    //        real(x2_i), imag(x2_i) );
                }
                output( "ddot", m, n, k, incx, incy, error_cblas, error_fblas, error_inline );
            }
        }
        printf( "\n" );

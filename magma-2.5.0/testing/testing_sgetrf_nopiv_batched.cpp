/*
   ...
   @generated from testing/testing_zgetrf_nopiv_batched.cpp, normal z -> s, Sun Sep 15 17:31:13 2019
 */
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_smalloc( &dA_magma,  ldda*N * batchCount ));
            ...
            {
                if (cpu_info[i] != 0 ) {
                    printf("magma_sgetrf_batched matrix %lld returned internal error %lld\n", (long long) i, (long long) cpu_info[i] );
                }
            }
            if (info != 0) {

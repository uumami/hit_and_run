/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgemm_vbatched.cpp, normal z -> d, Sun Sep 15 17:31:22 2019
       @author Mark Gates
       @author Azzam Haidar
...
int main( int argc, char** argv)
{
    ...
    double *d_A, *d_B, *d_C;
    double *h_A_tmp, *h_B_tmp, *h_C_tmp, *h_Cmagma_tmp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                gflops += FLOPS_DGEMM( h_M[i], h_N[i], h_K[i] ) / 1e9;
            ...
            for (int s = 0; s < batchCount; ++s) {
                Anorm[s] = lapackf77_dlange( "F",  &Am[s],  &An[s], h_A_tmp, &h_lda[s], work );
                Bnorm[s] = lapackf77_dlange( "F",  &Bm[s],  &Bn[s], h_B_tmp, &h_ldb[s], work );
                Cnorm[s] = lapackf77_dlange( "F", &h_M[s], &h_N[s], h_C_tmp, &h_ldc[s], work );
            ...
            h_C_tmp = h_C;
            for (int i = 0; i < batchCount; i++) {
                magma_dsetmatrix( Am[i],  An[i],  h_A_tmp, h_lda[i], h_A_array[i], h_ldda[i], opts.queue );
                magma_dsetmatrix( Bm[i],  Bn[i],  h_B_tmp, h_ldb[i], h_B_array[i], h_lddb[i], opts.queue );
                magma_dsetmatrix( h_M[i], h_N[i], h_C_tmp, h_ldc[i], h_C_array[i], h_lddc[i], opts.queue );
            ...
            #if 0
            magmablas_dgemm_vbatched_max_nocheck(opts.transA, opts.transB,
                             d_M, d_N, d_K,
                             ...
                             opts.queue);
            #else
            magmablas_dgemm_vbatched(opts.transA, opts.transB,
                             d_M, d_N, d_K,
                             alpha, d_A_array, d_ldda,
                                    d_B_array, d_lddb,
                             beta,  d_C_array, d_lddc,
            ...
            h_C_tmp = h_Cmagma;
            for (int i = 0; i < batchCount; i++) {
                magma_dgetmatrix( h_M[i], h_N[i], h_C_array[i], h_lddc[i], h_C_tmp, h_ldc[i], opts.queue );
                h_C_tmp += h_N[i] * h_ldc[i];
            }
            ...
            if ( opts.lapack ) {
                ...
                {
                    blasf77_dgemm( lapack_trans_const(opts.transA),
                                   lapack_trans_const(opts.transB),
                                   &h_M[s], &h_N[s], &h_K[s],
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++) {
                    ...
                    error = lapackf77_dlange( "F", &h_M[s], &h_N[s], h_Cmagma_tmp, &h_ldc[s], work )
                          / normalize;

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zlascl.cpp, normal z -> c, Sun Sep 15 17:28:03 2019
       @author Mark Gates
*/
    ...
    for( int iuplo = 0; iuplo < 3; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //magmablas_clascl( uplo[iuplo], 1, 1, cfrom, cto, M-2, N-2, d_A+1+ldda, ldda, opts.queue, &info );  // inset by 1 row & col
            magmablas_clascl( uplo[iuplo], 1, 1, cfrom, cto, M, N, d_A, ldda, opts.queue, &info );
            gpu_time = magma_sync_wtime( opts.queue ) - gpu_time;
            gpu_perf = gbytes / gpu_time;
            if (info != 0) {
                printf("magmablas_clascl returned error %lld: %s.\n",
            ...
            if (info != 0) {
                printf("lapackf77_clascl returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }

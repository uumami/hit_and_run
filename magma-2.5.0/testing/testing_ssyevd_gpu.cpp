/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zheevd_gpu.cpp, normal z -> s, Sun Sep 15 17:30:01 2019
...
int main( int argc, char** argv)
{
    ...
    if (opts.version == 3 || opts.version == 4) {
        printf("%% magma_ssyevr and magma_ssyevx not available for real precisions (single, float).\n");
        status = -1;
        return status;
    }
    #endif
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.version == 1 || opts.version == 2 ) {
                ...
                #ifdef COMPLEX
                magma_ssyevr_gpu( opts.jobz, range, opts.uplo,
                                  N, NULL, ldda,     // A
                                  vl, vu, il, iu, abstol,
                                  &Nfound, NULL,         // w
                ...
                #ifdef COMPLEX
                magma_ssyevx_gpu( opts.jobz, range, opts.uplo,
                                  N, NULL, ldda,     // A
                                  vl, vu, il, iu, abstol,
                                  &Nfound, NULL,         // w
                                  NULL, ldda,        // Z
            ...
            TESTING_CHECK( magma_smalloc_cpu( &h_A,    N*lda  ));
            ...
            TESTING_CHECK( magma_smalloc_pinned( &h_R,    N*lda  ));
            TESTING_CHECK( magma_smalloc_pinned( &h_work, lwork  ));
            
            TESTING_CHECK( magma_smalloc( &d_R,    N*ldda ));
            
            if (opts.version == 3) {
                TESTING_CHECK( magma_smalloc( &d_Z,    N*ldda     ));
            ...
            if ( opts.version == 1 ) {
                ...
                magma_ssyevdx_gpu( opts.jobz, range, opts.uplo,
                                   N, d_R, ldda,
                                   vl, vu, il, iu,
                                   &Nfound, w1,
                                   h_R, lda,
                ...
                magma_ssyevr_gpu( opts.jobz, range, opts.uplo,
                                  ...
                                  iwork, liwork,
                                  &info );
                magmablas_slacpy( MagmaFull, N, N, d_Z, ldda, d_R, ldda, opts.queue );
                #endif
            }
            ...
            if (info != 0) {
                printf("magma_ssyevd_gpu returned error %lld: %s.\n",

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgesvd.cpp, normal z -> d, Sun Sep 15 17:30:32 2019
       @author Mark Gates
    ...
    std::string& path_str )
{
    ...
    #ifdef COMPLEX
        /* =====================================================================
           lwork formulas for dgesvd (Complex)
           =================================================================== */
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( auto jobv = opts.jobv.begin(); jobv != opts.jobv.end(); ++jobv ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_dgesvd( lapack_vec_const(*jobu), lapack_vec_const(*jobv), &M, &N,
                              unused, &lda, runused,
                              unused, &ldu,
                              unused, &ldv,
                              dummy, &ineg_one,
            ...
            if ( opts.lapack ) {
                ...
                const char *func = "lapackf77_dgesvd";
                if ( *svd_work == MagmaSVD_min_1 ) {
                    if (info == -13) {
                        printf( "ok: with lwork = min-1 = %lld, %s returned expected info = %lld\n",
                                (long long) lwork_magma.value, func, (long long) info );

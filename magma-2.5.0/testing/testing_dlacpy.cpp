/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zlacpy.cpp, normal z -> d, Sun Sep 15 17:27:54 2019
       @author Mark Gates
*/
    ...
    for( int iuplo = 0; iuplo < 3; ++iuplo ) {
      for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_dmalloc_cpu( &h_B, size   ));
            TESTING_CHECK( magma_dmalloc_cpu( &h_R, size   ));
            
            TESTING_CHECK( magma_dmalloc( &d_A, ldda*N ));
            TESTING_CHECK( magma_dmalloc( &d_B, lddb*N ));
            ...
            if ( opts.verbose ) {
                ...
                printf( "dB=" );  magma_dprint_gpu( M, N, d_B, lddb, opts.queue );
            }

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_ztrtri_gpu.cpp, normal z -> c, Sun Sep 15 17:28:34 2019
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_ctrtri_gpu( opts.uplo, opts.diag, N, d_A, ldda, &info );
            gpu_time = magma_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0) {
                printf("magma_ctrtri_gpu returned error %lld: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                if (info != 0) {
                    printf("lapackf77_ctrtri returned error %lld: %s.\n",
                           (long long) info, magma_strerror( info ));
                }
                ...
                if ( opts.verbose ) {
                    ...
                    printf( "R=" );  magma_cprint( N, N, h_R, lda );
                }
                norm  = lapackf77_clantr("f", lapack_uplo_const(opts.uplo), MagmaNonUnitStr, &N, &N, h_A, &lda, work);
                blasf77_caxpy(&n2, &c_neg_one, h_A, &ione, h_R, &ione);
                error = lapackf77_clantr("f", lapack_uplo_const(opts.uplo), MagmaNonUnitStr, &N, &N, h_R, &lda, work) / norm;
                if ( opts.verbose ) {
                    printf( "diff=" );  magma_cprint( N, N, h_R, lda );
                }
                bool okay = (error < tol);
                status += ! okay;

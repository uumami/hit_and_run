/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_ztrsm.cpp, normal z -> d, Sun Sep 15 17:27:37 2019
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (opts.uplo == MagmaLower)
                lapackf77_dlaset( "upper", &Ak_1, &Ak_1, &MAGMA_D_NAN, &MAGMA_D_NAN, &hA[ 1*lda ], &lda );
            else
                lapackf77_dlaset( "lower", &Ak_1, &Ak_1, &MAGMA_D_NAN, &MAGMA_D_NAN, &hA[ 1     ], &lda );
            ...
            lapackf77_dpotrf( lapack_uplo_const(opts.uplo), &Ak, hA, &lda, &info );
            assert( info == 0 );

            lapackf77_dlarnv( &ione, ISEED, &sizeB, hB );
            lapackf77_dlacpy( MagmaFullStr, &M, &N, hB, &ldb, hBlapack, &lda );

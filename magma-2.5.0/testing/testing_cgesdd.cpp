/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgesdd.cpp, normal z -> c, Sun Sep 15 17:30:27 2019
       @author Mark Gates
    ...
    std::string& path_str )
{
    ...
    #ifdef COMPLEX
        /* =====================================================================
           lwork formulas for cgesdd (Complex)
           =================================================================== */
      ...
      for( auto jobz = opts.jobu.begin(); jobz != opts.jobu.end(); ++jobz ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.lapack ) {
                ...
                const char *func = "lapackf77_cgesdd";
                if ( *svd_work == MagmaSVD_min_1 || *svd_work == MagmaSVD_min_old_1 ) {
                    if (info == -12) {
                        printf( "ok: with lwork = min-1 = %lld, %s returned expected info = %lld\n",
                                (long long) lwork_magma.value, func, (long long) info );

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_ztrtri.cpp, normal z -> d, Sun Sep 15 17:28:45 2019
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            //h_R[ 10 + 10*lda ] = MAGMA_D_ZERO;
            
            gpu_time = magma_wtime();
            magma_dtrtri( opts.uplo, opts.diag, N, h_R, lda, &info );
            gpu_time = magma_wtime() - gpu_time;
            gpu_perf = gflops / gpu_time;
            if (info != 0) {
                printf("magma_dtrtri returned error %lld: %s.\n",
            ...
            if ( opts.lapack ) {
                ...
                Anorm = lapackf77_dlantr("f", lapack_uplo_const(opts.uplo), MagmaNonUnitStr, &N, &N, h_A, &lda, work);
                error = lapackf77_dlantr("f", lapack_uplo_const(opts.uplo), MagmaNonUnitStr, &N, &N, h_R, &lda, work) / Anorm;
                bool okay = (error < tol);
                status += ! okay;

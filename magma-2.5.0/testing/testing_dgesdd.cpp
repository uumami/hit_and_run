/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgesdd.cpp, normal z -> d, Sun Sep 15 17:30:28 2019
       @author Mark Gates
    ...
    std::string& path_str )
{
    ...
    #ifdef COMPLEX
        /* =====================================================================
           lwork formulas for dgesdd (Complex)
           =================================================================== */
...
/* ////////////////////////////////////////////////////////////////////////////
   -- Testing dgesdd (SVD with Divide & Conquer)
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      for( auto jobz = opts.jobu.begin(); jobz != opts.jobu.end(); ++jobz ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lapackf77_dgesdd( lapack_vec_const(*jobz), &M, &N,
                              unused, &lda, runused,
                              unused, &ldu,
                              unused, &ldv,
                              dummy, &ineg_one,
                              ...
                              iunused, &info );
            assert( info == 0 );
            query_lapack = (magma_int_t) MAGMA_D_REAL( dummy[0] );
            ...
            if (*jobz == MagmaNoVec) {
                ...
                // This is due to dbdsdc incorrectly claiming it needs 4*n,
                // when actually it needed 6*n in some cases.
                // dbdsdc is fixed in LAPACK >= 3.7.0.
                //assign( lrwork, 5*mn );
                assign( lrwork, 7*mn );
            }
            else if (mx >= mnthr) {
            ...
            TESTING_CHECK( magma_dmalloc_cpu( &S,     min_mn ));
            TESTING_CHECK( magma_dmalloc_cpu( &Sref,  min_mn ));
            TESTING_CHECK( magma_imalloc_cpu( &iwork, 8*min_mn ));
            
            TESTING_CHECK( magma_dmalloc_pinned( &hR,    lda*N ));
            TESTING_CHECK( magma_dmalloc_pinned( &hwork, lwork ));
            ...
            if ( *jobz == MagmaOverwriteVec && M >= N ) {
                ...
                TESTING_CHECK( magma_dmalloc_cpu( &Umalloc, ldu*N_U )); // M x M (jobz=A) or M x min(M,N)
                U = Umalloc;
            }
            if ( *jobz == MagmaOverwriteVec && M < N ) {
                VT  = hR;
                ...
                TESTING_CHECK( magma_dmalloc_cpu( &VTmalloc, ldv*N )); // N x N (jobz=A) or min(M,N) x N
                VT = VTmalloc;
            }
            
            #ifdef COMPLEX
            TESTING_CHECK( magma_dmalloc_cpu( &rwork, lrwork.value ));
            #endif
            ...
            if ( opts.magma ) {
                ...
                /* ====================================================================
                   ...
                   =================================================================== */
                if ( info == 0 ) {
                    check_dgesvd( opts.check, jobu, jobv, M, N, hA, lda, S, U, ldu, VT, ldv, result );
                }
                if (opts.verbose) {
            ...
            if ( opts.lapack ) {
                ...
                const char *func = "lapackf77_dgesdd";
                if ( *svd_work == MagmaSVD_min_1 || *svd_work == MagmaSVD_min_old_1 ) {
                    if (info == -12) {
                        printf( "ok: with lwork = min-1 = %lld, %s returned expected info = %lld\n",
                                (long long) lwork_magma.value, func, (long long) info );
                ...
                if ( info == 0 && opts.check == 2 ) {
                    check_dgesvd( opts.check, jobu, jobv, M, N, hA, lda, Sref, U, ldu, VT, ldv, result_lapack );
                }
                if (opts.verbose) {
                ...
                /* =====================================================================
                   ...
                   =================================================================== */
                if ( opts.magma ) {
                    blasf77_daxpy( &min_mn, &d_neg_one, S, &ione, Sref, &ione );

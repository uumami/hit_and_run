/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zher2k_vbatched.cpp, normal z -> c, Sun Sep 15 17:31:34 2019
       @author Mark Gates
       @author Azzam Haidar
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex *h_A_tmp, *h_B_tmp, *h_C_tmp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int s = 0; s < batchCount; ++s) {
                Anorm[s] = lapackf77_clange( "F", &An[s], &Ak[s], h_A_tmp, &h_lda[s], work );
                Bnorm[s] = lapackf77_clange( "F", &Bn[s], &Bk[s], h_B_tmp, &h_ldb[s], work );
                Cnorm[s] = safe_lapackf77_clanhe( "F", lapack_uplo_const(opts.uplo), &h_N[s], h_C_tmp, &h_ldc[s], work );
            ...
            magma_setvector(batchCount, sizeof(magmaFloatComplex*), h_C_array, 1, d_C_array, 1, opts.queue );
            
            h_A_tmp = h_A;
            h_B_tmp = h_B;
            h_C_tmp = h_C;
            for (int i = 0; i < batchCount; i++) {
                magma_csetmatrix( An[i], Ak[i], h_A_tmp, h_lda[i], h_A_array[i], h_ldda[i], opts.queue );
                magma_csetmatrix( Bn[i], Bk[i], h_B_tmp, h_ldb[i], h_B_array[i], h_lddb[i], opts.queue );
                magma_csetmatrix( h_N[i], h_N[i], h_C_tmp, h_ldc[i], h_C_array[i], h_lddc[i], opts.queue );
            ...
            #if 0
            magmablas_cher2k_vbatched_max_nocheck(opts.uplo, opts.transA, d_N, d_K,
                             alpha, d_A_array, d_ldda,
                                    d_B_array, d_lddb,
                             beta,  d_C_array, d_lddc,
                             batchCount, max_N, max_K, opts.queue );
            #else
            magmablas_cher2k_vbatched(opts.uplo, opts.transA, d_N, d_K,
                             alpha, d_A_array, d_ldda,
                                    d_B_array, d_lddb,
                             beta,  d_C_array, d_lddc,
            ...
            h_C_tmp = h_Cmagma;
            for (int i = 0; i < batchCount; i++) {
                magma_cgetmatrix( h_N[i], h_N[i], h_C_array[i], h_lddc[i], h_C_tmp, h_ldc[i], opts.queue );
                h_C_tmp += h_N[i] * h_ldc[i];
            }
            ...
            if ( opts.lapack ) {
                ...
                {
                    blasf77_cher2k( lapack_uplo_const(opts.uplo),
                                    lapack_trans_const(opts.transA),
                                    &h_N[s], &h_K[s],
            ...
            if ( opts.lapack ) {
                ...
                h_C_tmp = h_C;
                magmaFloatComplex* h_Cmagma_tmp = h_Cmagma;
                for (int s=0; s < batchCount; s++) {
                    normalize = 2*sqrt(float(h_K[s]+2))*Anorm[s]*Bnorm[s] + 2*Cnorm[s];
                    if (normalize == 0)
                        normalize = 1;
                    magma_int_t Csize = h_ldc[s] * h_N[s];
                    blasf77_caxpy( &Csize, &c_neg_one, h_C_tmp, &ione, h_Cmagma_tmp, &ione );
                    error = safe_lapackf77_clanhe( "F", lapack_uplo_const(opts.uplo), &h_N[s], h_Cmagma_tmp, &h_ldc[s], work )
                          / normalize;

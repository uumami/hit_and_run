/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zher2k_mgpu.cpp, normal z -> s, Sun Sep 15 17:27:45 2019
...
int main( int argc, char** argv)
{
    ...
    float           error, work[1];
    float *hC, *hR, *hR2, *hA, *hB;
    magmaFloat_ptr dA[MagmaMaxGPUs], dB[MagmaMaxGPUs], dC[MagmaMaxGPUs];
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int offset = 0; offset < n; offset += min(k,nb) ) {
            for( int iter = 0; iter < opts.niter; ++iter ) {
                ...
                TESTING_CHECK( magma_smalloc_cpu( &hC,  lda*n ));
                TESTING_CHECK( magma_smalloc_cpu( &hR,  lda*n ));
                TESTING_CHECK( magma_smalloc_cpu( &hR2, lda*n ));
                TESTING_CHECK( magma_smalloc_cpu( &hA,  lda*k ));
                TESTING_CHECK( magma_smalloc_cpu( &hB,  lda*k ));
                for( int dev = 0; dev < ngpu; ++dev ) {
                    magma_int_t nlocal = ((n / nb) / ngpu + 1) * nb;
                    magma_setdevice( dev );
                    TESTING_CHECK( magma_smalloc( &dC[dev], ldda*nlocal ));
                    TESTING_CHECK( magma_smalloc( &dA[dev], ldda*k*2    ));
                    //TESTING_CHECK( magma_smalloc( &dB[dev], ldda*k      ));
                ...
                // for error checks
                float Anorm = lapackf77_slange( "F", &n, &k, hA, &lda, work );
                float Bnorm = lapackf77_slange( "F", &n, &k, hB, &lda, work );
                float Cnorm = safe_lapackf77_slansy( "F", lapack_uplo_const(opts.uplo), &n, hC, &lda, work );
                
                /* ====================================================================
                   Performs operation using MAGMA
                   =================================================================== */
                magma_ssetmatrix_1D_col_bcyclic( ngpu, n, n, nb, hC, lda, dC, ldda, queues0 );
                for( int dev = 0; dev < ngpu; ++dev ) {
                    magma_setdevice( dev );
                    dB[dev] = dA[dev] + ldda*k;
                    magma_ssetmatrix( n, k, hA, lda, dA[dev], ldda, opts.queue );
                    magma_ssetmatrix( n, k, hB, lda, dB[dev], ldda, opts.queue );
                }
                ...
                magma_sgetmatrix_1D_col_bcyclic( ngpu, n, n, nb, dC, ldda, hR, lda, queues0 );
                ...
                if ( opts.lapack || opts.check ) {
                    ...
                    Anorm  = lapackf77_slange("f", &n_offset, &k, hA, &lda, work );
                    Anorm *= lapackf77_slange("f", &n_offset, &k, hB, &lda, work );
                    Anorm += lapackf77_slange("f", &n_offset, &n_offset, &hC[offset + offset*lda], &lda, work );

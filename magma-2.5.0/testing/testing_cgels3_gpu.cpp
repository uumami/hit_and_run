/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgels3_gpu.cpp, normal z -> c, Sun Sep 15 17:29:19 2019
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            lhwork = -1;
            lapackf77_cgeqrf( &M, &N, unused, &M, unused, tmp, &lhwork, &info );
            lhwork2 = (magma_int_t) MAGMA_C_REAL( tmp[0] );
            
            lhwork = -1;
            lapackf77_cunmqr( MagmaLeftStr, Magma_ConjTransStr,
                              &M, &nrhs, &min_mn,
                              unused, &lda, unused,
                              unused, &ldb,
                              tmp, &lhwork, &info);
            ...
            if (info != 0) {
                printf("magma_cgels3_gpu returned error %lld: %s.\n",

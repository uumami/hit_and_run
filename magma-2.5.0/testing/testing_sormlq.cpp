/*
    -- MAGMA (version 2.5.0) --
       ...
       @author Mark Gates
       @generated from testing/testing_zunmlq.cpp, normal z -> s, Sun Sep 15 17:29:57 2019
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            magma_sgelqf( k, nn, A, lda, tau, W, lwork_max, &info );
            if (info != 0) {
            ...
            if (info != 0) {
                printf("lapackf77_sormlq returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            ...
            if (info != 0) {
                printf("magma_sormlq (lwork query) returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            ...
            if (info != 0) {
                printf("magma_sormlq returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }

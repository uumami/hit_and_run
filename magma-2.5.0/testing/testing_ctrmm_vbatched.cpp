/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_ztrmm_vbatched.cpp, normal z -> c, Sun Sep 15 17:31:39 2019
...
int main( int argc, char** argv)
{
    ...
    magmaFloatComplex **hA_array, **hB_array;
    ...
    magmaFloatComplex *d_A_tmp, *d_B_tmp, *h_Bmagma_tmp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                gflops += FLOPS_CTRMM(opts.side, h_M[i], h_N[i]);
            }
            gflops /= 1e9;
            
            TESTING_CHECK( magma_cmalloc_cpu( &h_A,      total_sizeA_cpu ) );
            TESTING_CHECK( magma_cmalloc_cpu( &h_B,      total_sizeB_cpu ) );
            TESTING_CHECK( magma_cmalloc_cpu( &h_Bmagma, total_sizeB_cpu ) );
            
            TESTING_CHECK( magma_cmalloc( &d_A, total_sizeA_dev ) );
            TESTING_CHECK( magma_cmalloc( &d_B, total_sizeB_dev ) );
            ...
            magma_setvector(batchCount, sizeof(magmaFloatComplex*), hA_array, 1, dA_array, 1, opts.queue);
            magma_setvector(batchCount, sizeof(magmaFloatComplex*), hB_array, 1, dB_array, 1, opts.queue);
            ...
            lapackf77_clarnv( &ione, ISEED, &total_sizeA_cpu, h_A );
            lapackf77_clarnv( &ione, ISEED, &total_sizeB_cpu, h_B );
            ...
            d_A_tmp = d_A;
            for (int i = 0; i < batchCount; i++) {
                magma_csetmatrix( h_Ak[i], h_Ak[i], h_A_tmp, h_lda[i], d_A_tmp, h_ldda[i], opts.queue );
            ...
            d_B_tmp = d_B;
            for (int i = 0; i < batchCount; i++) {
                magma_csetmatrix( h_M[i],  h_N[i],  h_B_tmp, h_ldb[i], d_B_tmp, h_lddb[i], opts.queue );
            ...
            h_B_tmp = h_Bmagma;
            for (int i = 0; i < batchCount; i++) {
                magma_cgetmatrix( h_M[i], h_N[i], hB_array[i], h_lddb[i], h_B_tmp, h_ldb[i], opts.queue );
            ...
            if ( opts.lapack ) {
                ...
                for (int s  = 0; s < batchCount; s++) {
                    ...
                    blasf77_caxpy( &Bsize, &c_neg_one, h_B_tmp, &ione, h_Bmagma_tmp, &ione );
                    error = lapackf77_clange( "F", &h_M[s], &h_N[s], h_Bmagma_tmp, &h_ldb[s], work )
                          / normalize;

/*
   ...
   @generated from testing/testing_zpotrf_vbatched.cpp, normal z -> d, Sun Sep 15 17:31:46 2019
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
...
int main( int argc, char** argv)
{
    ...
    int status = 0;
    double **h_A_array=NULL, **d_A_array = NULL;
    ...
    double *h_A_tmp, *h_R_tmp, *d_A_tmp; 
    ...
    for( int i = 0; i < opts.ntest; ++i ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for(int k = 0; k < batchCount; k++){
                ...
                gflops += FLOPS_DPOTRF( h_N[k] ) / 1e9;
            }

            TESTING_CHECK( magma_dmalloc_cpu(&h_A, total_size_cpu) );
            TESTING_CHECK( magma_dmalloc_pinned(&h_R, total_size_cpu) );
            TESTING_CHECK( magma_dmalloc(&d_A, total_size_dev) );

            /* Initialize the matrix */
            lapackf77_dlarnv( &ione, ISEED, &total_size_cpu, h_A );
            h_A_tmp = h_A; 
            for (int s=0; s < batchCount; s++){
                magma_dmake_hpd( h_N[s], h_A_tmp, h_lda[s]); // need modification
            ...
            d_A_tmp = d_A;
            for(int s = 0; s < batchCount; s++){
                lapackf77_dlacpy( MagmaFullStr, &h_N[s], &h_N[s], h_A_tmp, &h_lda[s], h_R_tmp, &h_lda[s] );
                magma_dsetmatrix( h_N[s], h_N[s], h_A_tmp, h_lda[s], d_A_tmp, h_ldda[s], opts.queue );
                h_A_tmp += h_N[s] * h_lda[s]; 
            ...
            info = magma_dpotrf_vbatched_max_nocheck( 
                       opts.uplo, d_N, 
                       d_A_array, d_ldda, 
            ...
            for (int s=0; s < batchCount; s++){
                if (hinfo_magma[s] != 0 ) {
                    printf("magma_dpotrf_vbatched matrix %d returned internal error %d\n", s, (int)hinfo_magma[s] );
                    status = -1;
                    goto cleanup;
                }
            }
            ...
            if ( opts.lapack ){
                ...
                for (magma_int_t s=0; s < batchCount; s++){
                    lapackf77_dpotrf( lapack_uplo_const(opts.uplo), &h_N[s], h_A_array[s], &h_lda[s], &info );
                    if (info != 0)
                        printf("lapackf77_dpotrf matrix %d returned err %d: %s.\n", (int) s, (int) info, magma_strerror( info ));
                }
                #if !defined (BATCHED_DISABLE_PARCPU) && defined(_OPENMP)
                ...
                d_A_tmp = d_A;
                for(int s = 0; s < batchCount; s++){
                    magma_dgetmatrix(h_N[s], h_N[s], d_A_tmp, h_ldda[s], h_R_tmp, h_lda[s], opts.queue);
                    h_R_tmp += h_N[s] * h_lda[s]; 
                    d_A_tmp += h_N[s] * h_ldda[s];
                }
                magma_error = 0.0;
                ...
                for (int s=0; s < batchCount; s++)
                {
                    ...
                    Anorm = lapackf77_dlansy("f", lapack_uplo_const(opts.uplo), &h_N[s], h_A_tmp, &h_lda[s], work);
                    blasf77_daxpy(&Asize, &c_neg_one, h_A_tmp, &ione, h_R_tmp, &ione);
                    error = lapackf77_dlansy("f", lapack_uplo_const(opts.uplo), &h_N[s], h_R_tmp, &h_lda[s], work) / Anorm;

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zpotrf_mgpu.cpp, normal z -> c, Sun Sep 15 17:28:36 2019
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for( int dev=0; dev < ngpu; dev++ ) {
                magma_setdevice( dev );
                TESTING_CHECK( magma_cmalloc( &d_lA[dev], max_size ));
            }
            ...
            if ( opts.uplo == MagmaUpper ) {
                ...
                magma_csetmatrix_1D_col_bcyclic( ngpu, N, N, nb, h_R, lda, d_lA, ldda, queues );
            }
            else {
                ldda = (1+N/(nb*ngpu))*nb;
                magma_csetmatrix_1D_row_bcyclic( ngpu, N, N, nb, h_R, lda, d_lA, ldda, queues );
            }
            ...
            if (info != 0) {
                printf("magma_cpotrf_mgpu returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            
            if ( opts.uplo == MagmaUpper ) {
                magma_cgetmatrix_1D_col_bcyclic( ngpu, N, N, nb, d_lA, ldda, h_R, lda, queues );
            }
            else {
                magma_cgetmatrix_1D_row_bcyclic( ngpu, N, N, nb, d_lA, ldda, h_R, lda, queues );
            }

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_ztrsv_batched.cpp, normal z -> c, Sun Sep 15 17:31:02 2019
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gflops = FLOPS_CTRSM(opts.side, N, 1) / 1e9 * batchCount;
            ...
            TESTING_CHECK( magma_cmalloc( &d_b,       N*batchCount  ));
            ...
            magmaFloatComplex_ptr dwork=NULL; // invA and work are workspace in ctrsm
            ...
            magmablas_claset( MagmaFull, N, batchCount, c_zero, c_zero, dwork, N, opts.queue );
            ...
            magma_csetmatrix( N,  batchCount, h_b, N, d_b, N, opts.queue );
            ...
            magmablas_ctrsv_work_batched(opts.uplo, opts.transA, opts.diag, 
                                    N, d_A_array, ldda,
                                    d_b_array, 1, dwork_array, batchCount, opts.queue); 

            magma_time = magma_sync_wtime( opts.queue ) - magma_time;
            magma_perf = gflops / magma_time;
            magma_cgetmatrix( N, batchCount, dwork, N, h_bmagma, N, opts.queue );
            ...
            magma_cgetmatrix( N, batchCount, d_b, N, h_bcublas, N, opts.queue );
            ...
            for (s=0; s < batchCount; s++) {
                // error for CUBLAS
                normA = lapackf77_clange( "F", &N, &N, h_A + s * lda * Ak, &lda, work );
                float err;

                #if CUDA_VERSION >= 6050
                normx = lapackf77_clange( "F", &N, &ione, h_bcublas + s * N, &ione, work );
                blasf77_ctrmv( lapack_uplo_const(opts.uplo), lapack_trans_const(opts.transA), lapack_diag_const(opts.diag),
                               &N,
                               h_A + s * lda * Ak, &lda,
                               h_bcublas + s * N, &ione );

                blasf77_caxpy( &N, &c_neg_one, h_b + s * N, &ione, h_bcublas + s * N, &ione );
                normr = lapackf77_clange( "F", &N, &ione, h_bcublas + s * N, &N, work );
                ...
                // error for MAGMA
                normx = lapackf77_clange( "F", &N, &ione, h_bmagma + s * N, &ione, work );
                blasf77_ctrmv( lapack_uplo_const(opts.uplo), lapack_trans_const(opts.transA), lapack_diag_const(opts.diag),
                               &N,
                               h_A + s * lda * Ak, &lda,
                               h_bmagma  + s * N, &ione );

                blasf77_caxpy( &N, &c_neg_one, h_b + s * N, &ione, h_bmagma + s * N, &ione );
                normr = lapackf77_clange( "F", &N, &ione, h_bmagma + s * N, &N, work );
                err = normr / (normA*normx);

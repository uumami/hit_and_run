/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zhemv_batched.cpp, normal z -> s, Sun Sep 15 17:30:50 2019
       @author Mark Gates
       @author Azzam Haidar
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            gflops = FLOPS_SSYMV( N ) / 1e9 * batchCount;
            ...
            for(magma_int_t i = 0; i < batchCount; i++){
                float* Ai = h_A + i * N * lda;
                if ( opts.uplo == MagmaUpper ) {
                    lapackf77_slaset( "Lower", &N1, &N1, &MAGMA_S_NAN, &MAGMA_S_NAN, &Ai[1], &lda );
                }
                else {
                    lapackf77_slaset( "Upper", &N1, &N1, &MAGMA_S_NAN, &MAGMA_S_NAN, &Ai[lda], &lda );
                }
            }

            // Compute norms for error computation
            for (int s = 0; s < batchCount; ++s) {
                Anorm[s] = safe_lapackf77_slansy( "F", lapack_uplo_const(opts.uplo), &N, &h_A[s*lda*N], &lda, work );
                Xnorm[s] = lapackf77_slange( "F", &ione, &N, &h_X[s*N*incx], &incx, work );
                Ynorm[s] = lapackf77_slange( "F", &ione, &N, &h_Y[s*N*incy], &incy, work );
            }
            ...
            magma_ssetvector( N*batchCount, h_X, incx, d_X, incx, opts.queue );
            magma_ssetvector( N*batchCount, h_Y, incy, d_Y, incy, opts.queue );
            
            magma_sset_pointer( d_A_array, d_A, ldda, 0, 0, ldda*N, batchCount, opts.queue );
            magma_sset_pointer( d_X_array, d_X, 1, 0, 0, incx*N, batchCount, opts.queue );
            magma_sset_pointer( d_Y_array, d_Y, 1, 0, 0, incy*N, batchCount, opts.queue );

            magma_time = magma_sync_wtime( opts.queue );
            magmablas_ssymv_batched(opts.uplo, N,
                             alpha, d_A_array, ldda,
                                    d_X_array, incx,
                             beta,  d_Y_array, incy, batchCount, opts.queue);
            magma_time = magma_sync_wtime( opts.queue ) - magma_time;
            magma_perf = gflops / magma_time;
            magma_sgetvector( N*batchCount, d_Y, incy, h_Ymagma, incy, opts.queue );
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++){
                    ...
                    if (normalize == 0)
                        normalize = 1;
                    blasf77_saxpy( &N, &c_neg_one, &h_Y[s*N*incy], &incy, &h_Ymagma[s*N*incy], &incy );
                    error = lapackf77_slange( "F", &ione, &N, &h_Ymagma[s*N*incy], &incy, work )
                          / normalize;

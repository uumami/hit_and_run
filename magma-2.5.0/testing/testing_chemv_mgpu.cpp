/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zhemv_mgpu.cpp, normal z -> c, Sun Sep 15 17:27:43 2019
...
int main(int argc, char **argv)
{
    ...
    magmaFloatComplex *A, *X, *Y, *Ylapack, *Ydev, *Ymagma, *Ymagma1, *hwork;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int ioffset=0; ioffset < noffsets; ioffset += 1 ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_cmalloc_cpu( &A,       matsize ));
            TESTING_CHECK( magma_cmalloc_cpu( &Y,       vecsize ));
            TESTING_CHECK( magma_cmalloc_cpu( &Ydev,    vecsize ));
            TESTING_CHECK( magma_cmalloc_cpu( &Ymagma,  vecsize ));
            TESTING_CHECK( magma_cmalloc_cpu( &Ymagma1, vecsize ));
            TESTING_CHECK( magma_cmalloc_cpu( &Ylapack, vecsize ));

            TESTING_CHECK( magma_cmalloc_pinned( &X,       vecsize ));
            TESTING_CHECK( magma_cmalloc_pinned( &hwork,   lhwork  ));
            
            magma_setdevice( opts.device );
            TESTING_CHECK( magma_cmalloc( &dA, matsize ));
            TESTING_CHECK( magma_cmalloc( &dX, vecsize ));
            TESTING_CHECK( magma_cmalloc( &dY, vecsize ));
            ...
            for( dev=0; dev < opts.ngpu; dev++ ) {
                ...
                magma_setdevice( dev );
                TESTING_CHECK( magma_cmalloc( &d_lA[dev],  ldda*n_local[dev] ));
                TESTING_CHECK( magma_cmalloc( &dwork[dev], ldwork ));
            }
            ...
            /* =====================================================================
               ...
               =================================================================== */
            magma_setdevice( opts.device );
            magma_csetmatrix( Noffset, Noffset, A, lda, dA, ldda, opts.queue );
            magma_csetvector( Noffset, X, incx, dX, incx, opts.queue );
            magma_csetvector( Noffset, Y, incx, dY, incx, opts.queue );
            ...
            magma_cgetvector( Noffset, dY, incx, Ydev, incx, opts.queue );
            ...
            magma_cgetvector( Noffset, dY, incx, Ymagma1, incx, opts.queue );
            
            /* =====================================================================
               Performs operation using MAGMABLAS (multi-GPU)
               =================================================================== */
            magma_csetmatrix_1D_col_bcyclic( opts.ngpu, Noffset, Noffset, nb, A, lda, d_lA, ldda, queues );
            blasf77_ccopy( &Noffset, Y, &incx, Ymagma, &incx );
            
            // workspaces do NOT need to be zero -- set to NAN to prove
            for( dev=0; dev < opts.ngpu; ++dev ) {
                magma_setdevice( dev );
                magmablas_claset( MagmaFull, ldwork, 1, MAGMA_C_NAN, MAGMA_C_NAN, dwork[dev], ldwork, opts.queue );
            }
            ...
            info = magmablas_chemv_mgpu(
                ...
                opts.ngpu, nb, queues );
            if (info != 0) {
                printf("magmablas_chemv_mgpu returned error %lld: %s.\n",
            ...
            info = magmablas_chemv_mgpu_sync(
                ...
                opts.ngpu, nb, queues );
            if (info != 0) {
                printf("magmablas_chemv_sync returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            ...
            /* =====================================================================
               ...
               =================================================================== */
            Ynorm = lapackf77_clange( "F", &Noffset, &ione, Ydev, &Noffset, work );
            blasf77_caxpy( &Noffset, &c_neg_one, Ymagma, &incx, Ydev, &incx );
            error = lapackf77_clange( "F", &Noffset, &ione, Ydev, &Noffset, work ) / Ynorm;

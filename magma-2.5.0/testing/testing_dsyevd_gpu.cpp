/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zheevd_gpu.cpp, normal z -> d, Sun Sep 15 17:30:02 2019
...
int main( int argc, char** argv)
{
    ...
    if (opts.version == 3 || opts.version == 4) {
        printf("%% magma_dsyevr and magma_dsyevx not available for real precisions (single, double).\n");
        status = -1;
        return status;
    }
    #endif
    ...
    const char *versions[] = {
        ...
        "dsyevd_gpu",
        "dsyevdx_gpu",
        "dsyevr_gpu (Complex only)",
        "dsyevx_gpu (Complex only)"
    };
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if ( opts.version == 1 || opts.version == 2 ) {
                ...
                #ifdef COMPLEX
                magma_dsyevr_gpu( opts.jobz, range, opts.uplo,
                                  N, NULL, ldda,     // A
                                  vl, vu, il, iu, abstol,
                                  &Nfound, NULL,         // w
                ...
                #ifdef COMPLEX
                magma_dsyevx_gpu( opts.jobz, range, opts.uplo,
                                  N, NULL, ldda,     // A
                                  vl, vu, il, iu, abstol,
                                  &Nfound, NULL,         // w
                                  NULL, ldda,        // Z
            ...
            TESTING_CHECK( magma_dmalloc( &d_R,    N*ldda ));
            
            if (opts.version == 3) {
                TESTING_CHECK( magma_dmalloc( &d_Z,    N*ldda     ));
            ...
            if ( opts.version == 1 ) {
                ...
                magma_dsyevdx_gpu( opts.jobz, range, opts.uplo,
                                   N, d_R, ldda,
                                   vl, vu, il, iu,
                                   &Nfound, w1,
                                   h_R, lda,
            ...
            if (info != 0) {
                printf("magma_dsyevd_gpu returned error %lld: %s.\n",

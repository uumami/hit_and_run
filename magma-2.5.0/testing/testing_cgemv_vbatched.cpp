/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zgemv_vbatched.cpp, normal z -> c, Sun Sep 15 17:31:24 2019
       @author Mark Gates
       @author Azzam Haidar
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                gflops += FLOPS_CGEMV( h_M[i], h_N[i]) / 1e9;
            }
            ...
            for (int s = 0; s < batchCount; ++s) {
                Anorm[s] = lapackf77_clange( "F", &h_M[s], &h_N[s], h_A_tmp, &h_lda[s],  work );
                Xnorm[s] = lapackf77_clange( "F", &ione,   &Xn[s],  h_X_tmp, &h_incx[s], work );
                Ynorm[s] = lapackf77_clange( "F", &ione,   &Yn[s],  h_Y_tmp, &h_incy[s], work );
            ...
            h_A_tmp = h_A;
            for (int i = 0; i < batchCount; i++) {
                magma_csetmatrix( h_M[i], h_N[i], h_A_tmp, h_lda[i], h_A_array[i], h_ldda[i], opts.queue );
            ...
            #if 0
            magmablas_cgemv_vbatched_max_nocheck(opts.transA, d_M, d_N,
            ...
            #else
            magmablas_cgemv_vbatched(opts.transA, d_M, d_N,
                             alpha, d_A_array, d_ldda,
                                    d_X_array, d_incx,
                             beta,  d_Y_array, d_incy,
            ...
            if ( opts.lapack ) {
                ...
                for (int s=0; s < batchCount; s++){
                    ...
                    if (normalize == 0)
                        normalize = 1;
                    blasf77_caxpy( &Yn[s], &c_neg_one, h_Y_tmp, &h_incy[s], h_Ymagma_tmp, &h_incy[s] );
                    error = lapackf77_clange( "F", &ione, &Yn[s], h_Ymagma_tmp, &h_incy[s], work )
                          / normalize;

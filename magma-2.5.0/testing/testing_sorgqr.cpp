/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zungqr.cpp, normal z -> s, Sun Sep 15 17:29:53 2019
...
int main( int argc, char** argv )
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_smalloc_cpu( &hT,     min_mn*nb ));
            ...
            lapackf77_slacpy( MagmaFullStr, &m, &n, hA, &lda, hR, &lda );
            magma_sgetmatrix( nb, min_mn, dT, nb, hT, nb, opts.queue );  // for multi GPU
            ...
            if (info != 0) {
                printf("magma_sorgqr returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }

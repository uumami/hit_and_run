/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_ztrsm.cpp, normal z -> s, Sun Sep 15 17:27:38 2019
...
int main( int argc, char** argv)
{
    ...
    float *hA, *hB, *hBdev, *hBmagma, *hBlapack, *hX;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_smalloc_cpu( &hX,       ldb*N   ));
            TESTING_CHECK( magma_smalloc_cpu( &hBlapack, ldb*N   ));
            ...
            if (opts.uplo == MagmaLower)
                lapackf77_slaset( "upper", &Ak_1, &Ak_1, &MAGMA_S_NAN, &MAGMA_S_NAN, &hA[ 1*lda ], &lda );
            else
                lapackf77_slaset( "lower", &Ak_1, &Ak_1, &MAGMA_S_NAN, &MAGMA_S_NAN, &hA[ 1     ], &lda );
            ...
            lapackf77_spotrf( lapack_uplo_const(opts.uplo), &Ak, hA, &lda, &info );
            assert( info == 0 );

            lapackf77_slarnv( &ione, ISEED, &sizeB, hB );
            lapackf77_slacpy( MagmaFullStr, &M, &N, hB, &ldb, hBlapack, &lda );
            ...
            /* =====================================================================
               ...
               =================================================================== */
                ...
                if (opts.ngpu == 1) {
                    ...
                    magma_strsm_m( abs_ngpu, opts.side, opts.uplo, opts.transA, opts.diag,
                                   M, N,
                                   alpha, dA(0,0), ldda,
                                          dB(0,0), lddb );
                }
            ...
            magma_strsm( opts.side, opts.uplo, opts.transA, opts.diag,
                         M, N,
                         alpha, dA(0,0), ldda,
                                dB(0,0), lddb, opts.queue );
            ...
            normA = lapackf77_slantr( "M",
            ...
            #if defined(HAVE_CUBLAS)
                // check magma
                memcpy( hX, hBmagma, sizeB*sizeof(float) );
                ...
                blasf77_saxpy( &sizeB, &c_neg_one, hB, &ione, hX, &ione );
                normR = lapackf77_slange( "M", &M, &N, hX,      &ldb, work );
                normX = lapackf77_slange( "M", &M, &N, hBmagma, &ldb, work );
                magma_error = normR/(normX*normA);
            #endif

            // check cuBLAS / clBLAS
            memcpy( hX, hBdev, sizeB*sizeof(float) );
            blasf77_strmm( lapack_side_const(opts.side), lapack_uplo_const(opts.uplo),
                           lapack_trans_const(opts.transA), lapack_diag_const(opts.diag),
                           &M, &N,
                           &inv_alpha, hA, &lda,
            ...
            blasf77_saxpy( &sizeB, &c_neg_one, hB, &ione, hX, &ione );
            normR = lapackf77_slange( "M", &M, &N, hX,    &ldb, work );
            normX = lapackf77_slange( "M", &M, &N, hBdev, &ldb, work );
            ...
            if ( opts.lapack ) {
                ...
                memcpy( hX, hBlapack, sizeB*sizeof(float) );
                blasf77_strmm( lapack_side_const(opts.side), lapack_uplo_const(opts.uplo),
                               lapack_trans_const(opts.transA), lapack_diag_const(opts.diag),
                               &M, &N,
                               &inv_alpha, hA, &lda,
                ...
                blasf77_saxpy( &sizeB, &c_neg_one, hB, &ione, hX, &ione );
                normR = lapackf77_slange( "M", &M, &N, hX,       &ldb, work );
                normX = lapackf77_slange( "M", &M, &N, hBlapack, &ldb, work );

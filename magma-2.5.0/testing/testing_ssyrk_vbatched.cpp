/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zherk_vbatched.cpp, normal z -> s, Sun Sep 15 17:31:33 2019
...
int main( int argc, char** argv)
{
    ...
    float **d_A_array;
    float **d_C_array;
    float *h_A_tmp, *h_C_tmp, *h_Cmagma_tmp;
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            for (int i = 0; i < batchCount; i++) {
                ...
                gflops += FLOPS_SSYRK( h_K[i], h_N[i] ) / 1e9;
            }
            ...
            #if 0
            magmablas_ssyrk_vbatched_max_nocheck(opts.uplo, opts.transA, d_N, d_K,
                             alpha, d_A_array, d_ldda,
                             beta,  d_C_array, d_lddc,
                             batchCount, max_N, max_K, opts.queue);
            #else
            magmablas_ssyrk_vbatched(opts.uplo, opts.transA, d_N, d_K,
                             alpha, d_A_array, d_ldda,
                             beta,  d_C_array, d_lddc,
            ...
            if ( opts.lapack ) {
                ...
                for (int i=0; i < batchCount; i++)
                {
                    blasf77_ssyrk( lapack_uplo_const(opts.uplo),

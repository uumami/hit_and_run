/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from testing/testing_zhegvd.cpp, normal z -> s, Sun Sep 15 17:30:19 2019
...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            if (info != 0) {
                printf("magma_ssygvd returned error %lld: %s.\n",
            ...
            if ( opts.check && opts.jobz != MagmaNoVec ) {
                ...
                if ( opts.itype == 1 || opts.itype == 2 ) {
                    lapackf77_slaset( "A", &N, &N, &c_zero, &c_one, h_S, &lda);
                    blasf77_sgemm("N", "C", &N, &N, &N, &c_one, h_R, &lda, h_R, &lda, &c_zero, h_work, &N);

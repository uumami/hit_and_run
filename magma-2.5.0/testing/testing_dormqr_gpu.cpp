/*
    -- MAGMA (version 2.5.0) --
       ...
       @author Mark Gates
       @generated from testing/testing_zunmqr_gpu.cpp, normal z -> d, Sun Sep 15 17:29:35 2019
*/
// includes, system
#include <stdlib.h>
#include <stdio.h>
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
      ...
      for( int itran = 0; itran < 2; ++itran ) {
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_dmalloc_cpu( &hwork, lwork_max ));
            TESTING_CHECK( magma_dmalloc_cpu( &tau,   k ));
            
            TESTING_CHECK( magma_dmalloc( &dC, ldc*n ));
            TESTING_CHECK( magma_dmalloc( &dA, lda*k ));
            TESTING_CHECK( magma_dmalloc( &dT, dt_size ));
            ...
            if (info != 0) {
                printf("magma_dormqr_gpu (lwork query) returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }
            ...
            if (info != 0) {
                printf("magma_dormqr_gpu returned error %lld: %s.\n",
                       (long long) info, magma_strerror( info ));
            }

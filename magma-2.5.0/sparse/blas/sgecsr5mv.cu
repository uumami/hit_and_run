    ...
    const    int                 local_id)
{
    ...
    if (local_id < 2)   { ai = 8 * baseai - 1;  bi = 8 * basebi - 1;   
                          s_scan[bi] += s_scan[ai]; }
    if (local_id == 0)  { s_scan[31] = s_scan[15]; s_scan[15] = MAGMA_S_ZERO; }
    if (local_id < 2)   { ai = 8 * baseai - 1;  bi = 8 * basebi - 1;   
    ...
    const    magma_index_t         lane_id)
{
    ...
    s_sum[lane_id] = lane_id == MAGMA_CSR5_OMEGA - 1 
                     ? MAGMA_S_ZERO : s_sum[lane_id];
    float sum = tmp_sum = s_sum[lane_id];
    ...
    const float       alpha)
{
    ...
    for (int i = 1; i < c_sigma; i++)
    {
        ...
        direct |= local_bit;
        sum = local_bit ? MAGMA_S_ZERO : sum;
        stop += local_bit;
    ...
    sum = start ? first_sum : MAGMA_S_ZERO;
...
//#endif
    ...
    last_sum += (start <= stop) ? sum : MAGMA_S_ZERO;
    ...
    const magma_index_t        p)
{
    ...
    __shared__ float  s_calibrator[MAGMA_CSR5_THREAD_GROUP];
    //volatile __shared__ 
    //         float  s_sum[MAGMA_CSR5_THREAD_GROUP 
    ...
    s_calibrator[local_id] = sum = global_id < p-1 ? 
                                   d_calibrator[global_id] : MAGMA_S_ZERO;
    __syncthreads();
    ...
    if (s_tile_ptr[0] == s_tile_ptr[MAGMA_CSR5_THREAD_GROUP - 1])
    {
        ...
        if (!local_id)
        {
            atomicAddfloat(&d_y[s_tile_ptr[0]], s_calibrator[0]);
        }
        return;
    }
    ...
    if (global_id < p - 1)
    {
        ...
        if (row_start_previous != row_start_current)
        {
            ...
            {
                atomicAddfloat(&d_y[row_start_target], sum);
            }
            else
...
//    sum = sum_32_shfl<vT>(sum);
//#else
    __shared__ float s_sum[MAGMA_CSR5_OMEGA];
    s_sum[local_id] = sum;
...
__global__ void 
sgecsr5mv_kernel_update_y(int    num_rows,
                          float beta,
                          float * dy)
{
    ...
    magma_queue_t           queue )
{
    ...
    if ( arch >= 600 ) {
        ...
        //sgecsrmv_kernel<<< grid, threads, 0, queue->cuda_stream() >>>

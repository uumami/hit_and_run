/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zgeisai_maxblock.cu, normal z -> c, Sun Sep 15 17:35:34 2019
    ...
    magma_queue_t queue )
{
    ...
    magma_c_matrix L2={Magma_CSR}, L3={Magma_CSR}, 
    ...
    // pattern L
    CHECK( magma_c_mtransfer( L, &T, Magma_DEV, Magma_DEV, queue ) );

    // pattern L^2
    CHECK( magma_c_spmm( MAGMA_C_ONE, L, T, &L2, queue ) );
    // pattern L^3
    CHECK( magma_c_spmm( MAGMA_C_ONE, T, L2, &L3, queue ) );
    // pattern L^4                           
    CHECK( magma_c_spmm( MAGMA_C_ONE, T, L3, &L4, queue ) );
    // pattern L^5                           
     CHECK( magma_c_spmm( MAGMA_C_ONE, T, L4, &L5, queue ) );

    // check for pattern L
    magma_cselect_pattern_kernel<<< grid, block, 0, queue->cuda_stream() >>>    
    ...
    magma_cselect_rowptr_kernel<<< grid0, block0, 0, queue->cuda_stream() >>>    
    ...
    CHECK( magma_cmalloc( &MT->dval, nonzeros ) );
    ...
    magma_cselect_insert_kernel<<< grid2, block2, 0, queue->cuda_stream() >>>    
            ( L.num_rows, 1, 
...
cleanup:
    ...
    magma_cmfree( &L2, queue );
    magma_cmfree( &L3, queue );
    magma_cmfree( &L4, queue );
    magma_cmfree( &L5, queue );

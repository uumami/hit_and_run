/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/magma_zpreselect.cu, normal z -> d, Sun Sep 15 17:34:49 2019
    ...
    magma_queue_t queue )
{
    ...
    if( order == 1 ){ // don't copy the first
        magma_dpreselect_gpu1<<<grid, block, 0, queue->cuda_stream()>>>
        ...
        magma_dpreselect_gpu0<<<grid, block, 0, queue->cuda_stream()>>>
        ( A->num_rows, A->drow,  A->dval, oneA->dval );
        // #pragma omp parallel for
        // for( magma_int_t row=0; row<A->num_rows; row++){

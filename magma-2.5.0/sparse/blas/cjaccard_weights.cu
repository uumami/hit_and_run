...
magma_cjaccardweights_kernel(   
    ...
    magmaFloatComplex sum_i, sum_j, intersect;
    int il, iu, jl, ju;
    ...
    if (k < nnzJ)
    {
        ...
        if( i != j ){
            ...
            sum_i = MAGMA_C_MAKE((float)rowptrA[i+1] - rowptrA[i], 0.0);
            sum_j = MAGMA_C_MAKE((float)rowptrA[j+1] - rowptrA[j], 0.0);
            ...
            valJ[k] = MAGMA_C_MAKE(MAGMA_C_REAL(intersect) / MAGMA_C_REAL( sum_i + sum_j - intersect), 0.0 );
        } else {
            valJ[k] = MAGMA_C_ONE;
        }
    ...
    magma_queue_t queue )
{
    ...
    magma_cjaccardweights_kernel<<< grid, block, 0, queue->cuda_stream() >>>( 
        m, n, nnz, 
        J->drowidx,
        J->dcol,
        J->dval,

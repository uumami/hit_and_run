/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zmgesellcmmv.cu, normal z -> d, Sun Sep 15 17:34:12 2019
    ...
    double * dy)
{
    ...
    if (row < num_rows ) {
        double dot1 = MAGMA_D_MAKE(0.0, 0.0);
        double dot2 = MAGMA_D_MAKE(0.0, 0.0);
        int offset = drowptr[ bdx ];
        int max_ = (drowptr[ bdx+1 ]-offset)/blocksize;  
            // number of elements each thread handles
    ...
    magma_queue_t queue )
{
    ...
    if ( (texture==1) && (precision==1) && (kepler==1) ) {
        ...
        int Ms = num_vecs * blocksize*alignment * sizeof( double );


        if ( alignment == 1) {
            dim3 block( blocksize, num_vecs/2, 1 );
            if ( beta == MAGMA_D_MAKE( 0.0, 0.0 ) )

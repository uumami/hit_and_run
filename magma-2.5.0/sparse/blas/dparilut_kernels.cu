/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zparilut_kernels.cu, normal z -> d, Sun Sep 15 17:34:36 2019
    ...
    magma_queue_t queue )
{
    ...
    magma_dparilut_L_kernel<<< grid1, block1, 0, queue->cuda_stream() >>>
        (A->num_rows, A->drow, A->dcol, A->dval, 
         L->nnz, L->drow, L->drowidx, L->dcol, L->dval,
         U->nnz, U->drow, U->drowidx, U->dcol, U->dval);
        
    magma_dparilut_U_kernel<<< grid2, block2, 0, queue->cuda_stream() >>>
        (A->num_rows, A->drow, A->dcol, A->dval, 
         L->nnz, L->drow, L->drowidx, L->dcol, L->dval,
         U->nnz, U->drow, U->drowidx, U->dcol, U->dval);
    ...
    magma_queue_t queue )
{
    ...
    magma_dparilut_residuals_kernel<<<grid1, block1, 0, queue->cuda_stream()>>>
        (A.num_rows, A.drow, A.dcol, A.dval, 

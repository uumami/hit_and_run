/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zparilut_kernels.cu, normal z -> s, Sun Sep 15 17:34:37 2019
    ...
    float *U_val)
{
    ...
    if (k < L_nnz) {
        ...
        if (row == col) { // end check whether part of L
            L_val[k] = MAGMA_S_ONE; // upper triangular has diagonal equal 1
        } else {
            s = zero;
    ...
    magma_queue_t queue )
{
    ...
    magma_sparilut_L_kernel<<< grid1, block1, 0, queue->cuda_stream() >>>
        (A->num_rows, A->drow, A->dcol, A->dval, 
         L->nnz, L->drow, L->drowidx, L->dcol, L->dval,
         U->nnz, U->drow, U->drowidx, U->dcol, U->dval);
        
    magma_sparilut_U_kernel<<< grid2, block2, 0, queue->cuda_stream() >>>
        (A->num_rows, A->drow, A->dcol, A->dval, 
         L->nnz, L->drow, L->drowidx, L->dcol, L->dval,
         U->nnz, U->drow, U->drowidx, U->dcol, U->dval);
...
__global__ void 
magma_sparilut_residuals_kernel(   
    const magma_int_t num_rows, 
    const magma_index_t *A_row,  
    const magma_index_t *A_col,  
    const float * __restrict__ A_val, 
    ...
    magma_queue_t queue )
{
    ...
    magma_sparilut_residuals_kernel<<<grid1, block1, 0, queue->cuda_stream()>>>
        (A.num_rows, A.drow, A.dcol, A.dval, 

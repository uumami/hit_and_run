/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zgeisai_maxblock.cu, normal z -> d, Sun Sep 15 17:35:35 2019
    ...
    magma_queue_t queue )
{
    ...
    magma_d_matrix L2={Magma_CSR}, L3={Magma_CSR}, 
    ...
    // pattern L
    CHECK( magma_d_mtransfer( L, &T, Magma_DEV, Magma_DEV, queue ) );

    // pattern L^2
    CHECK( magma_d_spmm( MAGMA_D_ONE, L, T, &L2, queue ) );
    // pattern L^3
    CHECK( magma_d_spmm( MAGMA_D_ONE, T, L2, &L3, queue ) );
    // pattern L^4                           
    CHECK( magma_d_spmm( MAGMA_D_ONE, T, L3, &L4, queue ) );
    // pattern L^5                           
     CHECK( magma_d_spmm( MAGMA_D_ONE, T, L4, &L5, queue ) );

    // check for pattern L
    magma_dselect_pattern_kernel<<< grid, block, 0, queue->cuda_stream() >>>    
    ...
    magma_dselect_rowptr_kernel<<< grid0, block0, 0, queue->cuda_stream() >>>    
    ...
    CHECK( magma_dmalloc( &MT->dval, nonzeros ) );
    ...
    magma_dselect_insert_kernel<<< grid2, block2, 0, queue->cuda_stream() >>>    
            ( L.num_rows, 1, 
...
cleanup:
    ...
    magma_dmfree( &L2, queue );
    magma_dmfree( &L3, queue );
    magma_dmfree( &L4, queue );
    magma_dmfree( &L5, queue );

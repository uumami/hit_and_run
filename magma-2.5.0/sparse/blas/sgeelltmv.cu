/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zgeelltmv.cu, normal z -> s, Sun Sep 15 17:32:34 2019
    ...
    magma_queue_t queue )
{
    ...
    if (beta == MAGMA_S_ZERO) {
        sgeelltmv_kernel<true><<< grid, threads, 0, queue->cuda_stream() >>>
                  ( m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    } else {
        sgeelltmv_kernel<false><<< grid, threads, 0, queue->cuda_stream() >>>
                  ( m, n, nnz_per_row, alpha, dval, dcolind, dx, beta, dy );
    }
    ...
    magma_queue_t queue )
{
    ...
    tmp_shift = lambda;
    sgeelltmv_kernel_shift<<< grid, threads, 0, queue->cuda_stream() >>>

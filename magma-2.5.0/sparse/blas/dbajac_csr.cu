/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zbajac_csr.cu, normal z -> d, Sun Sep 15 17:31:50 2019
    ...
    magma_queue_t queue )
{
    ...
    if ( R.nnz > 0 ) { 
        if ( localiters == 1 )
        magma_dbajac_csr_kernel<<< grid, block, 0, queue->cuda_stream() >>>
            ( D.num_rows, D.dval, D.drow, D.dcol, 
                            R.dval, R.drow, R.dcol, b.dval, x->dval );
        else
            magma_dbajac_csr_ls_kernel<<< grid, block, 0, queue->cuda_stream() >>>

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zparilut_candidates.cu, normal z -> d, Sun Sep 15 17:34:40 2019
    ...
    if (row < num_rows) {
        ...
        do{
            ...
            if(ilu0col == ilutcol ){
                ...
                L_new_val[ offsetL + laddL ] = MAGMA_D_ONE + MAGMA_D_ONE + MAGMA_D_ONE;
                laddL++;
                ilu0++;
            }
        } while(ilut<endilut && ilu0<endilu0 );
        ...
        do{
            ...
            if(ilu0col == ilutcol ){
                ...
                U_new_val[ offsetU + laddU ] = MAGMA_D_ONE + MAGMA_D_ONE + MAGMA_D_ONE;
                laddU++;
                ilu0++;
            }
        }while(ilut<endilut && ilu0<endilu0 );
        ...
        for(int el1=L_row[row]; el1<L_row[row+1]-1; el1++ ){
            ...
            for(int el2 = U_row[ col1 ]+1; el2 < U_row[ col1+1 ]; el2++ ){
                ...
                if(cand_col < row ){
                    ...
                    L_new_val[ offsetL + laddL ] = (exist == 0) ? MAGMA_D_ONE : MAGMA_D_ZERO;
                    laddL++;
                } else {
                    // check whether this element already exists in U
                    int exist = 0;
                    ...
                    U_new_val[ offsetU + laddU ] = (exist == 0) ? MAGMA_D_ONE : MAGMA_D_ZERO;
                    laddU++;
                }
            }
        }
    ...
    magma_queue_t queue )
{
    ...
    CHECK(magma_dindexinit_gpu(num_rows+1, L_new->drow, queue));
    CHECK(magma_dindexinit_gpu(num_rows+1, U_new->drow, queue));
    CHECK(magma_dindexinit_gpu(num_rows, insertedL, queue));
    CHECK(magma_dindexinit_gpu(num_rows, insertedU, queue));
    ...
    dparilut_candidates_count_1<<<grid1, block1, 0, queue->cuda_stream()>>>(
        L0.num_rows, L0.drow, L0.dcol, U0.drow, U0.dcol,
        L.drow, L.dcol, U.drow, U.dcol,
        insertedL, insertedU);
    dparilut_candidates_count_2<<<grid1, block1, 0, queue->cuda_stream()>>>(
        L0.num_rows, L0.drow, L0.dcol, U0.drow, U0.dcol,
        L.drow, L.dcol, U.drow, U.dcol,
        insertedL, insertedU);
    CHECK(magma_dget_row_ptr(num_rows, &L_new->nnz, insertedL, 
        L_new->drow, queue));
    CHECK(magma_dget_row_ptr(num_rows, &U_new->nnz, insertedU, 
        U_new->drow, queue));
    
    CHECK(magma_dindexinit_gpu(num_rows, insertedL, queue));
    CHECK(magma_dindexinit_gpu(num_rows, insertedU, queue));
    
    CHECK(magma_dmalloc(&L_new->dval, L_new->nnz));
    CHECK(magma_index_malloc(&L_new->drowidx, L_new->nnz));
    CHECK(magma_index_malloc(&L_new->dcol, L_new->nnz));
    CHECK(magma_dmalloc(&U_new->dval, U_new->nnz));
    CHECK(magma_index_malloc(&U_new->drowidx, U_new->nnz));
    CHECK(magma_index_malloc(&U_new->dcol, U_new->nnz));
    CHECK(magma_dvalinit_gpu(L_new->nnz, L_new->dval, queue));
    CHECK(magma_dvalinit_gpu(U_new->nnz, U_new->dval, queue));
    //CHECK(magma_dindexinit_gpu(L_new->nnz, L_new->dcol, queue));
    //CHECK(magma_dindexinit_gpu(U_new->nnz, U_new->dcol, queue));
    //CHECK(magma_dindexinit_gpu(L_new->nnz, L_new->drowidx, queue));
    //CHECK(magma_dindexinit_gpu(U_new->nnz, U_new->drowidx, queue));
    // we don't need to init rowidx and col
    // the uninitilazed values will be removed anyways
    dparilut_candidates_insert_1<<<grid1, block1, 0, queue->cuda_stream()>>>(
        L0.num_rows, L0.drow, L0.dcol, U0.drow, U0.dcol,
        L.drow, L.dcol, U.drow, U.dcol,
        L_new->drow, L_new->drowidx, L_new->dcol, L_new->dval, insertedL,
        U_new->drow, U_new->drowidx, U_new->dcol, U_new->dval, insertedU);
    dparilut_candidates_insert_2<<<grid1, block1, 0, queue->cuda_stream()>>>(
        L0.num_rows, L0.drow, L0.dcol, U0.drow, U0.dcol,
        L.drow, L.dcol, U.drow, U.dcol,
        L_new->drow, L_new->drowidx, L_new->dcol, L_new->dval, insertedL,
        U_new->drow, U_new->drowidx, U_new->dcol, U_new->dval, insertedU);
        
    CHECK(magma_dthrsholdrm_gpu(1, L_new, &thrs, queue));
    CHECK(magma_dthrsholdrm_gpu(1, U_new, &thrs, queue));

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/blas/zmdotc.cu, normal z -> d, Sun Sep 15 17:33:09 2019
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    dim3 Gs_next;
    int Ms = (local_block_size) * sizeof( double ); // 1 skp 
    magmaDouble_ptr aux1 = d1, aux2 = d2;
    int b = 1;        


    magma_dmdotc1_kernel_1<<< Gs, Bs, Ms, queue->cuda_stream() >>>
            ( Gs.x, n, v0, w0, d1 );
   
    while( Gs.x > 1 ) {
        Gs_next.x = magma_ceildiv( Gs.x, Bs.x );
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_dmdotc1_kernel_2<<< Gs_next.x/2, Bs.x/2, Ms/2, queue->cuda_stream() >>> 
                    ( Gs.x, n, aux1, aux2 );
    ...
    magma_dgetvector( 1 , aux1, 1, skp, 1, queue );
    ...
    magma_queue_t queue )
{
    ...
    dim3 Gs( magma_ceildiv( n, local_block_size ) );
    dim3 Gs_next;
    int Ms = 2 * (local_block_size) * sizeof( double ); // 4 skp 
    magmaDouble_ptr aux1 = d1, aux2 = d2;
    int b = 1;        


    magma_dmdotc2_kernel_1<<< Gs, Bs, Ms, queue->cuda_stream() >>>
            ( Gs.x, n, v0, w0, v1, w1, d1 );
   
    while( Gs.x > 1 ) {
        Gs_next.x = magma_ceildiv( Gs.x, Bs.x );
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_dmdotc2_kernel_2<<< Gs_next.x/2, Bs.x/2, Ms/2, queue->cuda_stream() >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
        ...
        // copy vectors to host
    magma_dgetvector( 2 , aux1, n, skp, 1, queue );
    ...
    magma_queue_t queue )
{
    ...
    dim3 Gs( magma_ceildiv( n, local_block_size ) );
    dim3 Gs_next;
    int Ms = 3 * (local_block_size) * sizeof( double ); // 4 skp 
    magmaDouble_ptr aux1 = d1, aux2 = d2;
    int b = 1;        
    // magma_dmdotc3_gpumemzero<<< Gs, Bs, 0, queue->cuda_stream() >>>( d1, n );

    magma_dmdotc3_kernel_1<<< Gs, Bs, Ms, queue->cuda_stream() >>>
            ( Gs.x, n, v0, w0, v1, w1, v2, w2, d1 );
   
    while( Gs.x > 1 ) {
        Gs_next.x = magma_ceildiv( Gs.x, Bs.x );
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_dmdotc3_kernel_2<<< Gs_next.x/2, Bs.x/2, Ms/2, queue->cuda_stream() >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
        ...
        // copy vectors to host
    magma_dgetvector( 3 , aux1, n, skp, 1, queue );
    ...
    magma_queue_t queue )
{
    ...
    dim3 Gs( magma_ceildiv( n, local_block_size ) );
    dim3 Gs_next;
    int Ms = 4 * (local_block_size) * sizeof( double ); // 4 skp 
    magmaDouble_ptr aux1 = d1, aux2 = d2;
    int b = 1;        


    magma_dmdotc4_kernel_1<<< Gs, Bs, Ms, queue->cuda_stream() >>>
            ( Gs.x, n, v0, w0, v1, w1, v2, w2, v3, w3, d1 );
   
    while( Gs.x > 1 ) {
        Gs_next.x = magma_ceildiv( Gs.x, Bs.x );
        if ( Gs_next.x == 1 ) Gs_next.x = 2;
        magma_dmdotc4_kernel_2<<< Gs_next.x/2, Bs.x/2, Ms/2, queue->cuda_stream() >>> 
                    ( Gs.x, n, aux1, aux2 );
        Gs_next.x = Gs_next.x /2;
        Gs.x = Gs_next.x;
        b = 1 - b;
        ...
        // copy vectors to host
    magma_dgetvector( 4 , aux1, n, skp, 1, queue );

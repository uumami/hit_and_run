/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/control/magma_zparilut_tools.cpp, normal z -> s, Sun Sep 15 17:36:44 2019
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    // new row pointer
    B.row[ 0 ] = 0;
    CHECK( magma_smatrix_createrowptr( B.num_rows, B.row, queue ) );
    ...
    CHECK( magma_smatrix_swap( &B, A, queue) );
    ...
    magma_queue_t queue )
{
    ...
    // set col for values smaller threshold to -1
        ...
        for( magma_int_t row=0; row<A->num_rows; row++){
            ...
            for( magma_int_t i=A->row[row]; i<A->row[row+1]; i++ ){
                float Lscal = L.val[L.row[A->col[i]+1]-1];
                if( MAGMA_S_ABS(A->val[i]*Lscal) <= *thrs ){
                    if( A->col[i]!=row ){
                        //printf("remove (%d %d) >> %.4e\n", row, A->col[i], A->val[i]);
                        // magma_int_t col = A->col[i];
                        A->col[i] = -1; // cheaper than val  
    ...
    magma_queue_t queue )
{
    ...
    // allocate new arrays
    CHECK( magma_smalloc_cpu( &B.val, U->nnz ) );
    ...
    // finally, swap the matrices
    CHECK( magma_smatrix_swap( &B, U, queue) );
        */
    // set the US pointer
    US->val = U->val;
    US->col = U->col;
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &B.val, A->num_rows ) );
    if( order == 1 ){
        #pragma omp parallel for
        for( magma_int_t row=0; row<A->num_rows; row++){
            float max = 0.0;
            magma_int_t el = -1;
            for( magma_int_t i=A->row[row]; i<A->row[row+1]; i++ ){
                if( MAGMA_S_ABS(A->val[i]) > max ){
                    el = i;
                    max = MAGMA_S_ABS(A->val[i]);
                }
            }
            if( el > -1 ){
                B.col[row] = A->col[el];
        ...
        CHECK( magma_sparilut_thrsrm( 1, &B, &thrs, queue ) );
    } else {
        #pragma omp parallel for
        for( magma_int_t row=0; row<A->num_rows; row++){
            float min = 1e18;
            magma_int_t el = -1;
            for( magma_int_t i=A->row[row]; i<A->row[row+1]; i++ ){
                if( MAGMA_S_ABS(A->val[i]) < min && A->col[i]!=row ){
                        el = i;
                        min = MAGMA_S_ABS(A->val[i]);
                }
            }
            if( el > -1 ){
                B.col[row] = A->col[el];
    ...
    // finally, swap the matrices
    // keep the copy!
   CHECK( magma_smatrix_swap( &B, oneA, queue) );
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &B.val, A->num_rows*2 ) );
    
    #pragma omp parallel for
    for( magma_int_t i=0; i<B.num_rows; i++){
            B.row[i] = 2*i;   
            for(magma_int_t j=0; j<2; j++){
                B.val[i*2+j] = MAGMA_S_ZERO;
                B.col[i*2+j] = -1;
                B.rowidx[i*2+j] = i;
            }
    ...
    magma_queue_t queue )
{
    ...
    #pragma omp parallel for
    for( magma_int_t row=0; row<A->num_rows; row++){
        float diag_el = MAGMA_S_ABS(U.val[U.row[row]]); 
        // last element in this row
        float max = 0.0;
        magma_int_t el = -1;
        for( magma_int_t i=A->row[row]; i<A->row[row+1]; i++ ){
            if( (MAGMA_S_ABS(A->val[i]) > max) 
                && (MAGMA_S_ABS(A->val[i]) > rtol / diag_el) ){
                el = i;
                max = MAGMA_S_ABS(A->val[i]);
            }
        }
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &oneA->val, oneA->nnz ) );
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &oneL->val, L->nnz-L->num_rows ) );
    CHECK( magma_smalloc_cpu( &oneU->val, U->nnz-U->num_rows ) );
    ...
    magma_queue_t queue )
{
    ...
    magma_int_t id = 0;
        ...
        for(magma_int_t i=0; i<A.nnz; i++ ){
            ...
            if( (row < (id+1)*el_per_block) && (row >=(id)*el_per_block)  ){
                float tmp_val;
                tmp_val = A.val[i];
                if( MAGMA_S_ABS(tmp_val) > MAGMA_S_ABS(B->val[row]) ){
                    B->val[row] = tmp_val;
    ...
    // magma_sparilut_thrsrm( 1, B, &thrs, queue );
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t e=0; e<L->nnz; e++){
        ...
        if( col < row ){
            ...
            L->val[ e ] = MAGMA_S_ONE; // lower triangular has diagonal equal 1
        }
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &val, LU->true_nnz ));
    ...
    for(magma_int_t rowc=0; rowc<LU->num_rows; rowc++){
        ...
        magma_index_t loc_nnz = 0;
        do{
            float valtt = LU->val[ el ];
            magma_int_t loc = offset+loc_nnz;
#ifdef NANCHECK
            if(magma_s_isnan_inf( valtt ) ){
                info = MAGMA_ERR_NAN;
                el = 0;
            } else
#endif
    ...
    magma_queue_t queue )
{
    ...
    magma_smalloc_cpu( &U_new->val, U_new->nnz );
            ...
            do{
                ...
                if( ilu0col == ilutcol ){
                    ...
                    if( existing==1 ){
                        ...
                        U_new->val[ offsetU + laddU ] = MAGMA_S_ONE;
                        laddU++;
                    }
                }
                else if( ilutcol<ilu0col ){
                    ...
                    U_new->col[ offsetU + laddU ] = ilu0col;
                    U_new->rowidx[ offsetU + laddU ] = row;
                    U_new->val[ offsetU + laddU ] = MAGMA_S_ONE + MAGMA_S_ONE + MAGMA_S_ONE;
                    laddU++;
                    ilu0++;
                }
            }while( ilut<endilut && ilu0<endilu0 );
    ...
    if( ilufill==1 ){
                ...
                for( magma_index_t el2 = U.row[ col1 ]+1; el2 < U.row[ col1+1 ]; el2++ ){
                    ...
                    if( cand_col < row ){
                        ...
                        if( exist == 0 ){
                            ...
                            //add in the next location for this row
                            // U_new->val[ numaddU[row] + laddU ] =  MAGMA_S_MAKE(1e-14,0.0);
        ...
        CHECK( magma_sparilut_thrsrm( 1, U_new, &thrs, queue ) );
...
extern "C" magma_int_t
magma_sparilut_candidates_semilinked( // new
    magma_s_matrix L0, // CSR
    magma_s_matrix U0, // CSC
    magma_s_matrix L, // CSR 
    magma_s_matrix U, // CSC
    magma_s_matrix UT, // CSR
    magma_s_matrix *L_new, // CSR
    magma_s_matrix *U_new,  //CSC
    magma_queue_t queue )
{
    magma_int_t info = 0;
    magma_index_t *insertedL;
    ...
    magma_queue_t queue )
{
    ...
    info = magma_sparilut_set_thrs_randomselect( (int)(*num_rm)*thrs1, L_new, 1, &element1, queue );
    count = 0;
    ...
    magma_int_t id = 0;
        ...
        for( magma_int_t i=lbound; i<ubound; i++ ){
            float val = MAGMA_S_ABS(L_new->val[i]);
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &val, loc_nnz ));
    blasf77_scopy(&loc_nnz, LU->val, &incx, val, &incy );
    ...
    if( order == 0 ){
        magma_sselectrandom( lval, lsize, lnum_rm, queue );
        element = (lval[lnum_rm]);
    } else {
         magma_sselectrandom( lval, lsize, lsize-lnum_rm, queue );
        element = (lval[lsize-lnum_rm]);  
    }
               

    *thrs = MAGMA_S_ABS(element);
    ...
    magma_queue_t queue )
{
    ...
    float *val=NULL;
    CHECK( magma_smalloc_cpu( &val, size ));
    assert( size > num_rm );
    blasf77_scopy(&size, LU->val, &incx, val, &incx );
    if( order == 0 ){
        magma_sselectrandom( val, size, num_rm, queue );
        *thrs = MAGMA_S_ABS(val[num_rm]);
    } else {
         magma_sselectrandom( val, size, size-num_rm, queue );
        *thrs = MAGMA_S_ABS(val[size-num_rm]);  
    }
...
extern "C" magma_int_t
magma_sparilut_set_thrs_L_scaled(
    magma_int_t num_rm,
    magma_s_matrix *L,
    magma_s_matrix *U,
    magma_int_t order,
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &val, size ));
    assert( size > num_rm );
    blasf77_scopy(&size, L->val, &incx, val, &incx );
    if( order == 0 ){
        magma_sselectrandom( val, size, num_rm, queue );
        *thrs = MAGMA_S_ABS(val[num_rm]);
    } else {
...
extern "C" magma_int_t
magma_sparilut_set_thrs_randomselect_approx2(
    magma_int_t num_rm,
    magma_s_matrix *LU,
    magma_int_t order,
    float *thrs,
    magma_queue_t queue )
{
    ...
    if(LU->nnz <=subset){
        ...
        CHECK( magma_smalloc_cpu( &val, size ));
        blasf77_scopy(&size, LU->val, &incy, val, &incx );
        if( order == 0 ){
            magma_sselectrandom( val, size, num_rm_loc, queue );
            *thrs = MAGMA_S_ABS(val[num_rm_loc]);
        } else {
             magma_sselectrandom( val, size, size-num_rm_loc, queue );
            *thrs = MAGMA_S_ABS(val[size-num_rm_loc]);  
        }
    }
    ...
    magma_queue_t queue )
{
    ...
    if( LU->nnz <= 680){
       CHECK( magma_sparilut_set_thrs_randomselect(
           num_rm,
           LU,
           order,
           thrs,
    ...
    num_threads = 1;
#endif
        num_threads = 272;
        CHECK( magma_smalloc_cpu( &dthrs, num_threads ));
        ...
        for( magma_int_t i=0; i<num_threads; i++ ){
            ...
            if( loc_nz > loc_rm ){
            if( order == 0 ){
                magma_sselectrandom( val+start, loc_nz, loc_rm, queue );
                dthrs[i] = val[start+loc_rm];
            } else {
                 magma_sselectrandom( val+start, loc_nz, loc_nz-loc_rm, queue );
        ...
        // compute the median
        magma_sselectrandom( dthrs, num_threads, (num_threads+1)/2, queue);
        
        *thrs = MAGMA_S_ABS(dthrs[(num_threads+1)/2]);
    }
cleanup:
    magma_free_cpu( val );
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &val, size ));
    assert( size > num_rm );
    blasf77_scopy(&L->nnz, L->val, &incx, val, &incx );
    blasf77_scopy(&U->nnz, U->val, &incx, val+L->nnz, &incx );
    if( order == 0 ){
        magma_sselectrandom( val, size, num_rm, queue );
        *thrs = MAGMA_S_ABS(val[num_rm]);
    } else {
    ...
    magma_queue_t queue )
{
    ...
    loc_num_rm = num_rm;
    CHECK( magma_smalloc_cpu( &val, loc_nnz ));
    CHECK( magma_smalloc_cpu( &elements, num_threads ));
    ...
    magma_int_t id = 0;
        ...
        for( magma_int_t i=1; i<num_threads; i++){
            blasf77_scopy(&loc_num_rm, val+i*loc_nnz/num_threads, &incy, val+i*loc_num_rm, &incy );
        }
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smalloc_cpu( &elements, avg_count ));
    ...
    element = MAGMA_S_ZERO;
    for( int z=0; z < avg_count; z++){
        element = element+MAGMA_S_MAKE(MAGMA_S_ABS(elements[z]), 0.0);
    }
    element = element/MAGMA_S_MAKE((float)avg_count, 0.0);

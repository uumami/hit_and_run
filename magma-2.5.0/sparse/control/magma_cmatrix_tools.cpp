/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/control/magma_zmatrix_tools.cpp, normal z -> c, Sun Sep 15 17:36:39 2019
       @author Hartwig Anzt
    ...
    magma_queue_t queue)
{
    ...
    U->row[ 0 ] = 0;
    CHECK(magma_cmatrix_createrowptr(U->num_rows, U->row, queue));
    ...
    magma_queue_t queue)
{
    ...
    // new row pointer
    L->row[ 0 ] = 0;
    CHECK(magma_cmatrix_createrowptr(L->num_rows, L->row, queue));
    ...
    magma_queue_t queue)
{
    ...
    for (magma_int_t i=0; i < A.nnz; i++) {
        locsum = locsum + (MAGMA_C_ABS(A.val[i]) * MAGMA_C_ABS(A.val[i]));
    }
    ...
    magma_queue_t queue)
{
    ...
    if (A->memory_location == Magma_CPU && A->storage_type == Magma_CSR){
        ...
        for (int row=0; row<A->num_rows; row++) {
            magma_cindexsort(&A->col[A->row[row]], 0, 

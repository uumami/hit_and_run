/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/control/magma_zmilustruct.cpp, normal z -> c, Sun Sep 15 17:36:28 2019
       @author Hartwig Anzt
*/
...
void magma_cmexFunction(magma_int_t nlhs, magma_int_t n, magmaFloatComplex omega,
                 magma_int_t levfill, magma_int_t storage,
                magma_index_t * ial, magma_index_t *jal, magmaFloatComplex *al,
                magma_index_t * iau, magma_index_t *jau, magmaFloatComplex *au,
                magma_int_t nrhs,
                magma_index_t * ia, magma_index_t *ja, magmaFloatComplex *a )
{
    ...
    magma_csymbolic_ilu(levfill, n, &nzl, &nzu, ia, ja, ial, jal, iau, jau);
}
    ...
    magma_queue_t queue )
{
    ...
    magma_c_matrix A_copy={Magma_CSR}, B={Magma_CSR};
    magma_c_matrix hA={Magma_CSR}, CSRCOOA={Magma_CSR};
    ...
    if( A->memory_location == Magma_CPU && A->storage_type == Magma_CSR ){
        CHECK( magma_cmtransfer( *A, &A_copy, Magma_CPU, Magma_CPU, queue ));
        CHECK( magma_cmtransfer( *A, &B, Magma_CPU, Magma_CPU, queue ));

        // possibility to scale to unit diagonal
        //magma_cmscale( &B, Magma_UNITDIAG );

        CHECK( magma_cmconvert( B, L, Magma_CSR, Magma_CSR , queue));
        CHECK( magma_cmconvert( B, U, Magma_CSR, Magma_CSR, queue ));
        ...
        magma_csymbolic_ilu( levels, A->num_rows, &num_lnnz, &num_unnz, B.row, B.col,
                                            L->row, L->col, U->row, U->col );
        L->nnz = num_lnnz;
        U->nnz = num_unnz;
        magma_free_cpu( L->val );
        magma_free_cpu( U->val );
        CHECK( magma_cmalloc_cpu( &L->val, L->nnz ));
        CHECK( magma_cmalloc_cpu( &U->val, U->nnz ));
        for( magma_int_t i=0; i<L->nnz; i++ )
            L->val[i] = MAGMA_C_MAKE( 0.0, 0.0 );

        for( magma_int_t i=0; i<U->nnz; i++ )
            U->val[i] = MAGMA_C_MAKE( 0.0, 0.0 );
        ...
        CHECK( magma_cmalloc_cpu( &A->val, L->nnz+U->nnz ));
        A->nnz = L->nnz+U->nnz;
        ...
        CHECK( magma_cmconvert( hA, &CSRCOOA, hA.storage_type, Magma_CSR, queue ));

        CHECK( magma_csymbilu( &CSRCOOA, levels, L, U, queue ));

        magma_cmfree( &hA, queue );
        magma_cmfree( A, queue );
        CHECK( magma_cmconvert( CSRCOOA, &hA, Magma_CSR, A_storage, queue ));
...
cleanup:
    ...
    magma_cmfree( &A_copy, queue );
    magma_cmfree( &B, queue );
    magma_cmfree( &hA, queue );
    magma_cmfree( &CSRCOOA, queue );
    return info;
}

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/control/magma_zvinit.cpp, normal z -> d, Sun Sep 15 17:36:31 2019
       @author Hartwig Anzt
*/
    ...
    magma_queue_t queue )
{
    ...
    if ( mem_loc == Magma_CPU ) {
        ...
        CHECK( magma_dmalloc_cpu( &x->val, x->nnz ));
        for( magma_int_t i=0; i<x->nnz; i++) {
             x->val[i] = MAGMA_D_MAKE( ((double)(2*rand()))/ RAND_MAX - 1., ((double)(2*rand()))/ RAND_MAX - 1. );
        }
    }
    else if ( mem_loc == Magma_DEV ) {
        CHECK( magma_dvinit_rand( &x_h, Magma_CPU, num_rows, num_cols, queue ));
        CHECK( magma_dmtransfer( x_h, x, Magma_CPU, Magma_DEV, queue ) );
    }

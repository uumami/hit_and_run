/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/control/magma_zmconvert.cpp, normal z -> c, Sun Sep 15 17:36:04 2019
       @author Hartwig Anzt
*/
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_cmalloc_cpu( valn, nnz_new ));
    ...
    magma_queue_t queue )
{
    ...
    magma_c_matrix dA={Magma_CSR}, dB={Magma_CSR};
    magma_index_t *row_tmp=NULL, *col_tmp=NULL;
    magmaFloatComplex *val_tmp = NULL;
    magma_index_t *row_tmp2=NULL, *col_tmp2=NULL;
    magmaFloatComplex *val_tmp2 = NULL;
    magmaFloatComplex *transpose=NULL;
    ...
    if ( A.memory_location == Magma_CPU )
    {
        ...
        if ( old_format == Magma_CSR )
        {
            ...
            // CSR to CSRL
                ...
                B->nnz = numzeros;
                CHECK( magma_cmalloc_cpu( &B->val, numzeros ));
                CHECK( magma_index_malloc_cpu( &B->row, A.num_rows+1 ));
                ...
                for( magma_int_t i=0; i < A.num_rows; i++) {
                    ...
                    for( magma_int_t j=A.row[i]; j < A.row[i+1]; j++) {
                        if ( A.col[j] < i) {
                            ...
                            B->val[numzeros] = MAGMA_C_MAKE(1.0, 0.0);
                            B->col[numzeros] = A.col[j];
                            numzeros++;
                        }
                        else if ( A.col[j] == i ) {
            ...
            // CSR to ELLPACKT (using row-major storage)
                ...
                //printf( "Conversion to ELLPACK with %d elements per row: ",
                                                                // maxrowlength );
                //fflush(stdout);
                CHECK( magma_cmalloc_cpu( &B->val, maxrowlength*A.num_rows ));
            ...
            // so the rowlength is padded (SELLP) to a multiple of the alignment
                ...
                //fflush(stdout);
                CHECK( magma_cmalloc_cpu( &B->val, B->row[slices] ));
            ...
            // CSR to BCSR
                ...
                CHECK( magma_cmconvert(dA, &dB, Magma_CSR, Magma_BCSR, queue ) );
                CHECK( magma_cmtransfer(dB, B, Magma_DEV, Magma_CPU, queue ) );
            }
            ...
            // CSR to CSR5
                ...
                B->csr5_num_packets = ceil((float)bit_all
                                           /(float)(sizeof(magma_uindex_t)*8));

                // calculate the number of tiles
                B->csr5_p = ceil((float)B->nnz
                                 / (float)(MAGMA_CSR5_OMEGA * B->csr5_sigma));
                ...
                CHECK( magma_cmalloc_cpu( &B->calibrator, B->csr5_p ));
                for( magma_int_t i=0; i<B->csr5_p; i++) {
                    B->calibrator[i] = MAGMA_C_MAKE(0., 0.);
                }
        ...
        // anything to CSR
            ...
            // ELL/ELLPACK to CSR
                ...
                CHECK( magma_c_csr_compressor(&A.val, &row_tmp, &A.col,
            ...
            // ELL (column-major) to CSR
                ...
                // conversion
                CHECK( magma_cmalloc_cpu( &val_tmp, A.num_rows*A.max_nnz_row ));
                CHECK( magma_index_malloc_cpu( &row_tmp, A.num_rows+1 ));
                ...
                //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros.
                //The CSR compressor removes these
                CHECK( magma_c_csr_compressor(&val_tmp, &row_tmp, &col_tmp,
                           &B->val, &B->row, &B->col, &B->num_rows, queue ));
          ...
          /*      CHECK( magma_cmconvert( A, B, Magma_ELL, Magma_CSR, queue ));
                for( magma_int_t i=0; i < A.num_rows; i++) {
                    magma_cindexsortval(
                    B->col,
                    B->val,
            ...
            */
                ...
                CHECK( magma_cmalloc_cpu( &val_tmp2, A.num_rows*A.max_nnz_row ));
                ...
                //now use AA_ELL, IA_ELL, row_tmp as CSR with some zeros.
                //The CSR compressor removes these
                CHECK( magma_c_csr_compressor(&val_tmp2, &row_tmp, &col_tmp2,
                           &B->val, &B->row, &B->col, &B->num_rows, queue ));
                B->nnz = B->row[B->num_rows];
            }
            ...
            // SELLP to CSR
                ...
                // conversion
                CHECK( magma_cmalloc_cpu( &val_tmp,
            ...
            // BCSR to CSR
                ...
                CHECK( magma_cmtransfer(A, &dA, Magma_CPU, Magma_DEV, queue ) );
                CHECK( magma_cmconvert(dA, &dB, Magma_BCSR, Magma_CSR, queue ) );
            ...
            // COO to CSR
                ...
                CHECK( magma_cmtransfer(A, &dA, Magma_CPU, Magma_DEV, queue ) );
                CHECK( magma_cmconvert(dA, &dB, Magma_COO, Magma_CSR, queue ) );
                magma_cmfree( &dA, queue );
                CHECK( magma_cmtransfer(dB, B, Magma_DEV, Magma_CPU, queue ) );
                magma_cmfree( &dB, queue );
            }
        ...
        if ( old_format == Magma_CSR && new_format == Magma_CSR ) {
            CHECK( magma_cmtransfer( A, B, Magma_DEV, Magma_DEV, queue ));
        }
        // CSR to DENSE
        if ( old_format == Magma_CSR && new_format == Magma_DENSE ) {
            ...
            CHECK( magma_cmconvert( hA, &hB, old_format, new_format, queue ));
        ...
        // CSR to BCSR
            ...
            CHECK( magma_cmalloc( &B->dval, nnzb*size_b*size_b ));
        ...
        // CSR to COO
            ...
            magma_ccopyvector( A.nnz, A.dval, 1, B->dval, 1, queue );
        ...
        // COO to CSR
            ...
            cusparseCgthr(cusparseHandle, nnz, A.dval, B->dval, P, CUSPARSE_INDEX_BASE_ZERO);

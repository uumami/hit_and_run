/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/control/magma_zmslice.cpp, normal z -> d, Sun Sep 15 17:36:18 2019
       @author Hartwig Anzt
*/
    ...
    magma_queue_t queue )
{
    ...
    if ( A.memory_location == Magma_CPU
        ...
        CHECK( magma_dmconvert( A, ALOC, Magma_CSR, Magma_CSR, queue ) );
        magma_free_cpu( ALOC->col );
        magma_free_cpu( ALOC->row );
        magma_free_cpu( ALOC->val );
        CHECK( magma_dmconvert( A, ANLOC, Magma_CSR, Magma_CSR, queue ) );
        ...
        CHECK( magma_dmalloc_cpu( &B->val, nnz ) );         
        
        // for the communication plan
        for( i=0; i<A.num_rows; i++ ) {
            comm_i[i] = 0;
            comm_v[i] = MAGMA_D_ZERO;
        }
            ...
            for( j=A.row[i]; j<A.row[i+1]; j++ ){
                ...
                if( col<lstart || col>=lend ){
                    ...
                    comm_v[ col ] = comm_v[ col ] 
                            + MAGMA_D_MAKE( MAGMA_D_ABS( A.val[j] ), 0.0 );
                    nnz_nloc++;
                } else {
                    nnz_loc++;   
                }
        ...
        CHECK( magma_dmalloc_cpu( &ALOC->val, nnz_loc ) ); 
        ALOC->num_rows = size;
        ALOC->num_cols = size;
        ALOC->nnz = nnz_loc;
        
        CHECK( magma_index_malloc_cpu( &ANLOC->col, nnz_nloc ) );
        CHECK( magma_dmalloc_cpu( &ANLOC->val, nnz_nloc ) ); 
        ANLOC->num_rows = size;

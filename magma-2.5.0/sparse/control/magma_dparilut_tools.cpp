/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/control/magma_zparilut_tools.cpp, normal z -> d, Sun Sep 15 17:36:46 2019
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    // new row pointer
    B.row[ 0 ] = 0;
    CHECK( magma_dmatrix_createrowptr( B.num_rows, B.row, queue ) );
    ...
    CHECK( magma_dmatrix_swap( &B, A, queue) );
    ...
    magma_queue_t queue )
{
    ...
    // set col for values smaller threshold to -1
        ...
        for( magma_int_t row=0; row<A->num_rows; row++){
            ...
            for( magma_int_t i=A->row[row]; i<A->row[row+1]; i++ ){
                double Lscal = L.val[L.row[A->col[i]+1]-1];
                if( MAGMA_D_ABS(A->val[i]*Lscal) <= *thrs ){
                    if( A->col[i]!=row ){
                        //printf("remove (%d %d) >> %.4e\n", row, A->col[i], A->val[i]);
                        // magma_int_t col = A->col[i];
                        A->col[i] = -1; // cheaper than val  
    ...
    magma_queue_t queue )
{
    ...
    // allocate new arrays
    CHECK( magma_dmalloc_cpu( &B.val, U->nnz ) );
    ...
    // finally, swap the matrices
    CHECK( magma_dmatrix_swap( &B, U, queue) );
        */
    // set the US pointer
    US->val = U->val;
    US->col = U->col;
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &B.val, A->num_rows ) );
    if( order == 1 ){
        #pragma omp parallel for
        for( magma_int_t row=0; row<A->num_rows; row++){
            double max = 0.0;
            magma_int_t el = -1;
            for( magma_int_t i=A->row[row]; i<A->row[row+1]; i++ ){
                if( MAGMA_D_ABS(A->val[i]) > max ){
                    el = i;
                    max = MAGMA_D_ABS(A->val[i]);
                }
            }
            if( el > -1 ){
                B.col[row] = A->col[el];
        ...
        CHECK( magma_dparilut_thrsrm( 1, &B, &thrs, queue ) );
    } else {
        #pragma omp parallel for
        for( magma_int_t row=0; row<A->num_rows; row++){
            double min = 1e18;
            magma_int_t el = -1;
            for( magma_int_t i=A->row[row]; i<A->row[row+1]; i++ ){
                if( MAGMA_D_ABS(A->val[i]) < min && A->col[i]!=row ){
                        el = i;
                        min = MAGMA_D_ABS(A->val[i]);
                }
            }
            if( el > -1 ){
                B.col[row] = A->col[el];
    ...
    // finally, swap the matrices
    // keep the copy!
   CHECK( magma_dmatrix_swap( &B, oneA, queue) );
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &B.val, A->num_rows*2 ) );
    
    #pragma omp parallel for
    for( magma_int_t i=0; i<B.num_rows; i++){
            B.row[i] = 2*i;   
            for(magma_int_t j=0; j<2; j++){
                B.val[i*2+j] = MAGMA_D_ZERO;
                B.col[i*2+j] = -1;
                B.rowidx[i*2+j] = i;
            }
    ...
    magma_queue_t queue )
{
    ...
    #pragma omp parallel for
    for( magma_int_t row=0; row<A->num_rows; row++){
        double diag_el = MAGMA_D_ABS(U.val[U.row[row]]); 
        // last element in this row
        double max = 0.0;
        magma_int_t el = -1;
        for( magma_int_t i=A->row[row]; i<A->row[row+1]; i++ ){
            if( (MAGMA_D_ABS(A->val[i]) > max) 
                && (MAGMA_D_ABS(A->val[i]) > rtol / diag_el) ){
                el = i;
                max = MAGMA_D_ABS(A->val[i]);
            }
        }
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &oneA->val, oneA->nnz ) );
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &oneL->val, L->nnz-L->num_rows ) );
    CHECK( magma_dmalloc_cpu( &oneU->val, U->nnz-U->num_rows ) );
    ...
    magma_queue_t queue )
{
    ...
    magma_int_t id = 0;
        ...
        for(magma_int_t i=0; i<A.nnz; i++ ){
            ...
            if( (row < (id+1)*el_per_block) && (row >=(id)*el_per_block)  ){
                double tmp_val;
                tmp_val = A.val[i];
                if( MAGMA_D_ABS(tmp_val) > MAGMA_D_ABS(B->val[row]) ){
                    B->val[row] = tmp_val;
    ...
    // magma_dparilut_thrsrm( 1, B, &thrs, queue );
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &val, LU->true_nnz ));
    ...
    for(magma_int_t rowc=0; rowc<LU->num_rows; rowc++){
        ...
        magma_index_t loc_nnz = 0;
        do{
            double valtt = LU->val[ el ];
            magma_int_t loc = offset+loc_nnz;
#ifdef NANCHECK
            if(magma_d_isnan_inf( valtt ) ){
                info = MAGMA_ERR_NAN;
                el = 0;
            } else
#endif
    ...
    magma_queue_t queue )
{
            ...
            do{
                ...
                if( ilu0col == ilutcol ){
                    ...
                    if( existing==1 ){
                        ...
                        L_new->val[ offsetL + laddL ] = MAGMA_D_ONE;
                        laddL++;
                    }
                }
                else if( ilutcol<ilu0col ){
                    ...
                    L_new->col[ offsetL + laddL ] = ilu0col;
                    L_new->rowidx[ offsetL + laddL ] = row;
                    L_new->val[ offsetL + laddL ] = MAGMA_D_ONE + MAGMA_D_ONE + MAGMA_D_ONE;
                    laddL++;
                    ilu0++;
                }
            }while( ilut<endilut && ilu0<endilu0 );
            ...
            do{
                ...
                if( ilu0col == ilutcol ){
                    ...
                    if( existing==1 ){
                        ...
                        U_new->val[ offsetU + laddU ] = MAGMA_D_ONE;
                        laddU++;
                    }
                }
                else if( ilutcol<ilu0col ){
                    ...
                    U_new->col[ offsetU + laddU ] = ilu0col;
                    U_new->rowidx[ offsetU + laddU ] = row;
                    U_new->val[ offsetU + laddU ] = MAGMA_D_ONE + MAGMA_D_ONE + MAGMA_D_ONE;
                    laddU++;
                    ilu0++;
                }
            }while( ilut<endilut && ilu0<endilu0 );
    ...
    if( ilufill==1 ){
                ...
                for( magma_index_t el2 = U.row[ col1 ]+1; el2 < U.row[ col1+1 ]; el2++ ){
                    ...
                    if( cand_col < row ){
                        ...
                        if( exist == 0 ){
                            ...
                            // L_new->val[ numaddL[row] + laddL ] =  MAGMA_D_MAKE(1e-14,0.0);
                        ...
                        if( exist == 0 ){
                            ...
                            //add in the next location for this row
                            // U_new->val[ numaddU[row] + laddU ] =  MAGMA_D_MAKE(1e-14,0.0);
        ...
        CHECK( magma_dparilut_thrsrm( 1, L_new, &thrs, queue ) );
        CHECK( magma_dparilut_thrsrm( 1, U_new, &thrs, queue ) );
...
extern "C" magma_int_t
magma_dparilut_candidates_semilinked( // new
    magma_d_matrix L0, // CSR
    magma_d_matrix U0, // CSC
    magma_d_matrix L, // CSR 
    magma_d_matrix U, // CSC
    magma_d_matrix UT, // CSR
    magma_d_matrix *L_new, // CSR
    magma_d_matrix *U_new,  //CSC
    magma_queue_t queue )
{
    magma_int_t info = 0;
    magma_index_t *insertedL;
    ...
    magma_queue_t queue )
{
    ...
    info = magma_dparilut_set_thrs_randomselect( (int)(*num_rm)*thrs1, L_new, 1, &element1, queue );
    count = 0;
    ...
    magma_int_t id = 0;
        ...
        for( magma_int_t i=lbound; i<ubound; i++ ){
            double val = MAGMA_D_ABS(L_new->val[i]);
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &val, loc_nnz ));
    blasf77_dcopy(&loc_nnz, LU->val, &incx, val, &incy );
    ...
    if( order == 0 ){
        magma_dselectrandom( lval, lsize, lnum_rm, queue );
        element = (lval[lnum_rm]);
    } else {
         magma_dselectrandom( lval, lsize, lsize-lnum_rm, queue );
        element = (lval[lsize-lnum_rm]);  
    }
               

    *thrs = MAGMA_D_ABS(element);
    ...
    magma_queue_t queue )
{
    ...
    double *val=NULL;
    CHECK( magma_dmalloc_cpu( &val, size ));
    assert( size > num_rm );
    blasf77_dcopy(&size, LU->val, &incx, val, &incx );
    if( order == 0 ){
        magma_dselectrandom( val, size, num_rm, queue );
        *thrs = MAGMA_D_ABS(val[num_rm]);
    } else {
         magma_dselectrandom( val, size, size-num_rm, queue );
        *thrs = MAGMA_D_ABS(val[size-num_rm]);  
    }
...
extern "C" magma_int_t
magma_dparilut_set_thrs_L_scaled(
    magma_int_t num_rm,
    magma_d_matrix *L,
    magma_d_matrix *U,
    magma_int_t order,
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &val, size ));
    assert( size > num_rm );
    blasf77_dcopy(&size, L->val, &incx, val, &incx );
    if( order == 0 ){
        magma_dselectrandom( val, size, num_rm, queue );
        *thrs = MAGMA_D_ABS(val[num_rm]);
    } else {
...
extern "C" magma_int_t
magma_dparilut_set_thrs_randomselect_approx2(
    magma_int_t num_rm,
    magma_d_matrix *LU,
    magma_int_t order,
    double *thrs,
    magma_queue_t queue )
{
    ...
    if(LU->nnz <=subset){
        ...
        CHECK( magma_dmalloc_cpu( &val, size ));
        blasf77_dcopy(&size, LU->val, &incy, val, &incx );
        if( order == 0 ){
            magma_dselectrandom( val, size, num_rm_loc, queue );
            *thrs = MAGMA_D_ABS(val[num_rm_loc]);
        } else {
             magma_dselectrandom( val, size, size-num_rm_loc, queue );
            *thrs = MAGMA_D_ABS(val[size-num_rm_loc]);  
        }
    }
    ...
    magma_queue_t queue )
{
    ...
    if( LU->nnz <= 680){
       CHECK( magma_dparilut_set_thrs_randomselect(
           num_rm,
           LU,
           order,
           thrs,
    ...
    num_threads = 1;
#endif
        num_threads = 272;
        CHECK( magma_dmalloc_cpu( &dthrs, num_threads ));
        ...
        for( magma_int_t i=0; i<num_threads; i++ ){
            ...
            if( loc_nz > loc_rm ){
            if( order == 0 ){
                magma_dselectrandom( val+start, loc_nz, loc_rm, queue );
                dthrs[i] = val[start+loc_rm];
            } else {
                 magma_dselectrandom( val+start, loc_nz, loc_nz-loc_rm, queue );
        ...
        // compute the median
        magma_dselectrandom( dthrs, num_threads, (num_threads+1)/2, queue);
        
        *thrs = MAGMA_D_ABS(dthrs[(num_threads+1)/2]);
    }
cleanup:
    magma_free_cpu( val );
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &val, size ));
    assert( size > num_rm );
    blasf77_dcopy(&L->nnz, L->val, &incx, val, &incx );
    blasf77_dcopy(&U->nnz, U->val, &incx, val+L->nnz, &incx );
    if( order == 0 ){
        magma_dselectrandom( val, size, num_rm, queue );
        *thrs = MAGMA_D_ABS(val[num_rm]);
    } else {
    ...
    magma_queue_t queue )
{
    ...
    loc_num_rm = num_rm;
    CHECK( magma_dmalloc_cpu( &val, loc_nnz ));
    CHECK( magma_dmalloc_cpu( &elements, num_threads ));
    ...
    magma_int_t id = 0;
        ...
        for( magma_int_t i=1; i<num_threads; i++){
            blasf77_dcopy(&loc_num_rm, val+i*loc_nnz/num_threads, &incy, val+i*loc_num_rm, &incy );
        }
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_dmalloc_cpu( &elements, avg_count ));
    ...
    element = MAGMA_D_ZERO;
    for( int z=0; z < avg_count; z++){
        element = element+MAGMA_D_MAKE(MAGMA_D_ABS(elements[z]), 0.0);
    }
    element = element/MAGMA_D_MAKE((double)avg_count, 0.0);
    ...
    magma_queue_t queue )
{
    ...
    #pragma omp parallel for
    for( magma_int_t row=0; row<L.num_rows; row++){
        double Lscal = L.val[L.row[row+1]-1]; // last element in row

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/control/magma_zgeisai_tools.cpp, normal z -> c, Sun Sep 15 17:36:36 2019
       @author Hartwig Anzt
    ...
    magma_queue_t queue )
{
    ...
    if( uplotype == MagmaLower ){
        ...
        for( magma_int_t i=0; i<L.num_rows; i++ ){
            ...
            sizes[ i ] = size;
            rhs[ i*(warpsize)+sizes[i]-1 ] = MAGMA_C_ONE;
        }
    }
    ...
    magma_queue_t queue )
{
    ...
    #pragma omp parallel for
    for(magma_int_t i=0; i<L.num_rows; i++){
        blasf77_ctrsv( lapack_uplo_const(uplotype),

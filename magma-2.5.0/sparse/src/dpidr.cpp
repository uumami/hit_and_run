/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zpidr.cpp, normal z -> d, Sun Sep 15 17:37:32 2019
*/
    ...
    magma_queue_t queue )
{
    ...
    magma_d_matrix dvtmp = {Magma_CSR};
...
//---------------------------------------
    ...
    CHECK( magma_dvinit( &dvtmp, Magma_DEV, dr.num_rows, 1, c_zero, queue ));

    // lu = 0
    CHECK( magma_dvinit( &dlu, Magma_DEV, A.num_rows, 1, c_zero, queue ));
    ...
    do
    {
        ...
        for ( k = 0; k < s; ++k ) {
            ...
            magma_dcopyvector( dU.num_rows, dv.dval, 1, dvtmp.dval, 1, queue );

            // G(:,k) = A U(:,k)
            CHECK( magma_d_spmv( c_one, A, dvtmp, c_zero, dv, queue ));

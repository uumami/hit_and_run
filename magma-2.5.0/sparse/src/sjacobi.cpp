/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zjacobi.cpp, normal z -> s, Sun Sep 15 17:37:19 2019
*/
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_smfree( x, queue ) );
    CHECK( magma_smtransfer(d, x, Magma_DEV, Magma_DEV, queue ) );
    CHECK(  magma_sresidualvec( ACSR, b, *x, &r, &residual, queue));
    if ( solver_par->verbose > 0 ) {
        solver_par->res_vec[0] = (real_Double_t) residual;
    }
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<solver_par->maxiter; i++ ) {
        CHECK( magma_s_spmv( c_neg_one, M, *x, c_zero, t, queue ));        // t = - M * x
        magma_saxpy( dofs, c_one , c.dval, 1 , t.dval, 1, queue );        // t = t + c
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<solver_par->maxiter; i++ ) {
        CHECK( magma_s_spmv( c_neg_one, M, *x, c_zero, precond->work2, queue )); // t = - M * x

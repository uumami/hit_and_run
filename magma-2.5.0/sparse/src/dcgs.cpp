/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zcgs.cpp, normal z -> d, Sun Sep 15 17:37:24 2019
*/
    ...
    magma_queue_t queue )
{
    ...
    do
    {
        ...
        if ( solver_par->numiter > 1 ) {                        // direction vectors
            ...
            magma_daxpy( dofs,  beta, q.dval, 1, u.dval, 1, queue );     // u = u + beta q
        ...
        solver_par->spmv_count++;
        magma_daxpy( dofs,  c_neg_one*alpha, rt.dval, 1, r.dval, 1, queue );       // r = r -alpha*A u_hat
        magma_daxpy( dofs,  alpha, t.dval, 1, x->dval, 1, queue );      // x = x + alpha u_hat
        rho_l = rho;

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zlobpcg.cpp, normal z -> d, Sun Sep 15 17:37:41 2019
*/
...
/**
    ...
    @param[in,out]
    precond_par magma_d_precond_par*
    ...
    magma_queue_t queue )
{
    ...
    double *dwork=NULL, *hwork=NULL;
    double *blockP=NULL, *blockAP=NULL, *blockR=NULL, *blockAR=NULL, *blockAX=NULL, *blockW=NULL;
    double *gramA=NULL, *gramB=NULL, *gramM=NULL;
    double *gevectors=NULL, *h_gramB=NULL;
    ...
    CHECK( magma_dmalloc_pinned( &hwork, lwork ));
    CHECK( magma_dmalloc( &blockAX,  m*n ));
    CHECK( magma_dmalloc( &blockAR,  m*n ));
    CHECK( magma_dmalloc( &blockAP,  m*n ));
    CHECK( magma_dmalloc( &blockR,   m*n ));
    CHECK( magma_dmalloc( &blockP,   m*n ));
    CHECK( magma_dmalloc( &blockW,   m*n ));
    CHECK( magma_dmalloc( &dwork,    m*n ));
    ...
    CHECK( magma_dmalloc_pinned(&hW, n*n));
    CHECK( magma_dmalloc_pinned(&gevectors, 9*n*n));
    CHECK( magma_dmalloc_pinned(&h_gramB,   9*n*n));

    // === Allocate GPU workspace ===
    CHECK( magma_dmalloc(&gramM, n * n));
    CHECK( magma_dmalloc(&gramA, 9 * n * n));
    CHECK( magma_dmalloc(&gramB, 9 * n * n));
    ...
    //magma_dorthomgs( m, n, blockX, queue );
    
    magma_d_bspmv_tuned(m, n, c_one, A, blockX, c_zero, blockAX, queue );
    ...
    for(iterationNumber = 1; iterationNumber < maxIterations; iterationNumber++)
        {
            ...
            magmablas_dlacpy( MagmaFull, m, n, blockAX, m, blockR, m, queue );

            /*
            for(magma_int_t i=0; i<n; i++) {
                magma_daxpy( m, MAGMA_D_MAKE(-evalues[i],0), blockX+i*m, 1, blockR+i*m, 1, queue );
            }
            */
            ...
            magmablas_dnrm2_cols( m, n, blockR, m, residualNorms(0, iterationNumber), queue );

            // === remove the residuals corresponding to already converged evectors
            CHECK( magma_dcompact(m, n, blockR, m,
            ...
            //magmablas_dlacpy( MagmaFull, m, cBlockSize, blockR, m, blockW, m, queue );
            //SWAP(blockW, blockR);
            ...
            magma_dgegqr_gpu(ikind, m, cBlockSize, blockR, m, dwork, hwork, &info );
            #endif
            //magma_dorthomgs( m, cBlockSize, blockR, queue );

            // === compute AR
            magma_d_bspmv_tuned(m, cBlockSize, c_one, A, blockR, c_zero, blockAR, queue );
            solver_par->spmv_count++;
            if (!restart) {
                // === compact P & AP as well
                CHECK( magma_dcompactActive(m, n, blockP,  m, activeMask, queue ));
                CHECK( magma_dcompactActive(m, n, blockAP, m, activeMask, queue ));
                ...
                */
                ...
                magma_dgegqr_gpu(ikind, m, cBlockSize, blockP, m, dwork, hwork, &info );
                #endif
                //magma_dorthomgs( m, cBlockSize, blockP, queue );

                //magma_d_bspmv_tuned(m, cBlockSize, c_one, A, blockP, c_zero, blockAP, queue );
                magma_dsetmatrix( cBlockSize, cBlockSize, hwork, cBlockSize, dwork, cBlockSize, queue );
            ...
            magmablas_dlaset( MagmaFull, ldgram, ldgram, c_zero, c_one, gramB, ldgram, queue );  // identity
            ...
            magma_dgetmatrix( gramDim, gramDim, gramB, ldgram, h_gramB, ldgram, queue );
            ...
            magmablas_dlaset( MagmaFull, ldgram, ldgram, c_zero, c_one, gramA, ldgram, queue );  // identity
            ...
            */
            ...
            if (restart==0) {
                magma_dgetmatrix( gramDim, gramDim, gramA, ldgram, gevectors, ldgram, queue );
            }
            else {
                gramDim = n+cBlockSize;
                magma_dgetmatrix( gramDim, gramDim, gramA, ldgram, gevectors, ldgram, queue );
            ...
            magma_dsetmatrix( gramDim, gramDim, gevectors, ldgram, gramA, ldgram, queue );
            ...
            SWAP(dwork, blockX);
            //magma_daxpy( m*n, c_one, blockP, 1, blockX, 1, queue );
            CHECK( magma_dlobpcg_maxpy( m, n, blockP, blockX, queue ));
            ...
            SWAP(dwork, blockAX);
            //magma_daxpy( m*n, c_one, blockAP, 1, blockAX, 1, queue );
            CHECK( magma_dlobpcg_maxpy( m, n, blockAP, blockAX, queue ));
    ...
    if (blockX != origX)
        magmablas_dlacpy( MagmaFull, m, n, blockX, m, origX, m, queue );

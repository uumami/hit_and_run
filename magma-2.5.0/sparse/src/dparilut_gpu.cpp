/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zparilut_gpu.cpp, normal z -> d, Sun Sep 15 17:37:49 2019
*/
    ...
    magma_queue_t queue)
{
    ...
    for (magma_int_t iters =0; iters<precond->sweeps; iters++) {
        ...
        if (num_rmL>0) {
            CHECK(magma_dsampleselect(dL.nnz, num_rmL, dL.dval, &thrsL, &selecttmp_ptr, &selecttmp_size, queue));
        } else {
            thrsL = 0.0;
        }
        if (num_rmU>0) {
            CHECK(magma_dsampleselect(dU.nnz, num_rmU, dU.dval, &thrsU, &selecttmp_ptr, &selecttmp_size, queue));
        } else {
            thrsU = 0.0;
        }

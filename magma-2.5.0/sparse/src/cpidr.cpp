/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zpidr.cpp, normal z -> c, Sun Sep 15 17:37:33 2019
*/
    ...
    magma_queue_t queue )
{
    ...
    magma_c_matrix dvtmp = {Magma_CSR};
...
//---------------------------------------
    ...
    CHECK( magma_cvinit( &dvtmp, Magma_DEV, dr.num_rows, 1, c_zero, queue ));
    ...
    do
    {
        ...
        for ( k = 0; k < s; ++k ) {
            ...
            magma_ccopyvector( dU.num_rows, dv.dval, 1, dvtmp.dval, 1, queue );

            // G(:,k) = A U(:,k)
            CHECK( magma_c_spmv( c_one, A, dvtmp, c_zero, dv, queue ));

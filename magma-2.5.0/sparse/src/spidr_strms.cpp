...
//---------------------------------------
    ...
    do
    {
        ...
        for ( k = 0; k < s; ++k ) {
            ...
            // v = U \ v;
            // Q1
            CHECK( magma_s_applyprecond_left( MagmaNoTrans, A, dv, &dlu, precond_par, queues[1] )); 
            CHECK( magma_s_applyprecond_right( MagmaNoTrans, A, dlu, &dv, precond_par, queues[1] )); 
            ...
            if ( k > 0 ) {
                ...
                // Q0
                magmablas_sgemv( MagmaNoTrans, dU.num_rows, k, c_n_one, dU.dval, dU.ld, dalpha.dval, 1, c_one, &dU.dval[k*dU.ld], 1, queues[0] );
            }
...
//---------------------------------------
        ...
        // v = U \ v;
        // Q2
        CHECK( magma_s_applyprecond_left( MagmaNoTrans, A, dv, &dlu, precond_par, queues[2] )); 
        CHECK( magma_s_applyprecond_right( MagmaNoTrans, A, dlu, &dv, precond_par, queues[2] )); 

        // t = A v
        // Q2
        CHECK( magma_s_spmv( c_one, A, dv, c_zero, dt, queues[2] ));
        solver_par->spmv_count++;

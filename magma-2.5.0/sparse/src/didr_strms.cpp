/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zidr_strms.cpp, normal z -> d, Sun Sep 15 17:37:16 2019
*/
    ...
    do
    {
        ...
        for ( k = 0; k < s; ++k ) {
            ...
            if ( k > 0 ) {
                ...
                // Q0
                magmablas_dgemv( MagmaNoTrans, dU.num_rows, k, c_n_one, dU.dval, dU.ld, dalpha.dval, 1, c_one, dv.dval, 1, queues[0] );
            }
...
//---------------------------------------
                ...
                // rs = rs - gamma * t 
                // Q1
                magma_daxpy( drs.num_rows, -gamma, dtt.dval, 1, drs.dval, 1, queues[1] );
                ...
                // |rs|
                // Q1
                nrmr = magma_dnrm2( drs.num_rows, drs.dval, 1, queues[1] );       
                // implicit sync Q0 --> |r|
//---------------------------------------
            }
            ...
            if ( (k + 1) == s ) {
               // t = A r
               // Q2
               CHECK( magma_d_spmv( c_one, A, dr, c_zero, dt, queues[2] ));
               solver_par->spmv_count++;
...
//---------------------------------------
        ...
        if ( smoothing <= 0 ) {
            ...
            // v = r
            // Q0
            magma_dcopyvector_async( dr.num_rows, dr.dval, 1, dv.dval, 1, queues[0] );

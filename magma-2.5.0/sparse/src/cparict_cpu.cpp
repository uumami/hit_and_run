/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zparict_cpu.cpp, normal z -> c, Sun Sep 15 17:37:52 2019
*/
    ...
    magma_queue_t queue)
{
    ...
    if (precond->levels > 0) {
        CHECK(magma_csymbilu(&hA, precond->levels, &hL, &LT , queue));
    ...
    for (magma_int_t iters =0; iters<precond->sweeps; iters++) {
        ...
        // step 4: sweep
        start = magma_sync_wtime(queue);
        CHECK(magma_cparict_sweep_sync(&hA, &L_new, queue));
        ...
        if (num_rmL>0) {
            CHECK(magma_cparilut_set_thrs_randomselect(num_rmL, 
                &oneL, 0, &thrsL, queue));
        } else {
            thrsL = 0.0;
        }
        ...
        // step 7: sweep
        start = magma_sync_wtime(queue);
        CHECK(magma_cparict_sweep_sync(&hA, &L, queue));
    ...
    CHECK(magma_cmtransfer(precond->L, &precond->M, Magma_DEV, Magma_DEV, 
        queue));

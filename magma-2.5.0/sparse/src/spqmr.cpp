/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zpqmr.cpp, normal z -> s, Sun Sep 15 17:37:08 2019
*/
    ...
    magma_queue_t queue )
{
    ...
    rho = magma_ssqrt( magma_sdot( dofs, y.dval, 1, y.dval, 1, queue ));
        ...
        // w = wt / psi
        // z = z / psi
    magma_scopy( dofs, vt.dval, 1, v.dval, 1, queue );  
    magma_scopy( dofs, wt.dval, 1, w.dval, 1, queue );  
    ...
    do
    {
        ...
        if( solver_par->numiter == 1 ){
            ...
            magma_saxpy( dofs, c_one, yt.dval, 1, p.dval, 1, queue );
                // q = zt - rde * q;
            magma_sscal( dofs, -rde, q.dval, 1, queue );    
            magma_saxpy( dofs, c_one, zt.dval, 1, q.dval, 1, queue );
        }
        if( magma_s_isnan_inf( rho ) || magma_s_isnan_inf( psi ) ){
            info = MAGMA_DIVERGENCE;
            break;
        ...
        if( magma_s_isnan_inf( epsilon ) || magma_s_isnan_inf( beta ) ){
            ...
            // vt = pt - beta * v;
        magma_scopy( dofs, v.dval, 1, vt.dval, 1, queue );
        magma_sscal( dofs, -beta, vt.dval, 1, queue ); 
        magma_saxpy( dofs, c_one, pt.dval, 1, vt.dval, 1, queue ); 
            // no precond: y = v

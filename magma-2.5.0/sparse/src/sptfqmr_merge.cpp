/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zptfqmr_merge.cpp, normal z -> s, Sun Sep 15 17:37:12 2019
*/
    ...
    magma_queue_t queue )
{
    ...
    // preconditioner
    CHECK( magma_s_applyprecond_left( MagmaNoTrans, A, u_m, &t, precond_par, queue ));
    CHECK( magma_s_applyprecond_right( MagmaNoTrans, A, t, &pu_m, precond_par, queue ));
    ...
    do
    {
        ...
        eta = c * c * alpha;
        sigma = theta * theta / alpha * eta;
        if ( magma_s_isnan_inf( theta ) ||
             magma_s_isnan_inf( c )     ||
             magma_s_isnan_inf( tau )   ||
             magma_s_isnan_inf( eta )   ||
             magma_s_isnan_inf( sigma ) )
        {
        ...
        // preconditioner
        CHECK( magma_s_applyprecond_left( MagmaNoTrans, A, u_mp1, &t, precond_par, queue ));
        CHECK( magma_s_applyprecond_right( MagmaNoTrans, A, t, &pu_m, precond_par, queue ));

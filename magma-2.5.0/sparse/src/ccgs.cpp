/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zcgs.cpp, normal z -> c, Sun Sep 15 17:37:25 2019
*/
    ...
    magma_queue_t queue )
{
    ...
    do
    {
        ...
        rho = magma_cdotc( dofs, r_tld.dval, 1, r.dval, 1, queue );
                                                            // rho = < r,r_tld>    
        ...
        if ( solver_par->numiter > 1 ) {                        // direction vectors
            ...
            magma_caxpy( dofs,  beta, q.dval, 1, u.dval, 1, queue );     // u = u + beta q
        ...
        CHECK( magma_c_spmv( c_one, A, p, c_zero, v_hat, queue ));   // v = A p
        ...
        CHECK( magma_c_spmv( c_one, A, t, c_zero, rt, queue ));   // t = A u_hat
        solver_par->spmv_count++;
        magma_caxpy( dofs,  c_neg_one*alpha, rt.dval, 1, r.dval, 1, queue );       // r = r -alpha*A u_hat
        magma_caxpy( dofs,  alpha, t.dval, 1, x->dval, 1, queue );      // x = x + alpha u_hat
        rho_l = rho;

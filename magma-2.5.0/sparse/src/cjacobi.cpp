/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from sparse/src/zjacobi.cpp, normal z -> c, Sun Sep 15 17:37:20 2019
*/
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_cmfree( x, queue ) );
    CHECK( magma_cmtransfer(d, x, Magma_DEV, Magma_DEV, queue ) );
    CHECK(  magma_cresidualvec( ACSR, b, *x, &r, &residual, queue));
    if ( solver_par->verbose > 0 ) {
        solver_par->res_vec[0] = (real_Double_t) residual;
    }
    ...
    do
    {
        ...
        //CHECK( magma_cjacobiiter_sys( A, b, d, r, x, &jacobiiter_par, queue ) );
        ...
        //CHECK( magma_cjacobispmvupdate_bw(jacobiiter_par.maxiter, A, r, b, d, x, queue ));
        if ( solver_par->verbose > 0 ) {
            CHECK(  magma_cresidualvec( ACSR, b, *x, &r, &residual, queue));
    ...
    magma_queue_t queue )
{
    ...
    CHECK( magma_cmtransfer( diag, d, Magma_CPU, Magma_DEV, queue ));
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<solver_par->maxiter; i++ ) {
        CHECK( magma_c_spmv( c_neg_one, M, *x, c_zero, t, queue ));        // t = - M * x
        magma_caxpy( dofs, c_one , c.dval, 1 , t.dval, 1, queue );        // t = t + c
    ...
    magma_queue_t queue )
{
    ...
    for( magma_int_t i=0; i<solver_par->maxiter; i++ ) {
        CHECK( magma_c_spmv( c_neg_one, M, *x, c_zero, precond->work2, queue )); // t = - M * x

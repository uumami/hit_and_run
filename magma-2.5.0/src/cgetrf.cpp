/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgetrf.cpp, normal z -> c, Sun Sep 15 17:13:27 2019
*/
    ...
    magma_int_t *info)
{
    ...
    magmaFloatComplex_ptr dA, dAT, dwork;
    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        mem_size /= sizeof(magmaFloatComplex);
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            ...
            if (MAGMA_SUCCESS != magma_cmalloc( &dwork, nb*maxm + maxdim*maxdim )) {
        ...
        for( j = 0; j < s; j++ ) {
            ...
            if (j > 0) {
                magmablas_ctranspose( nb, cols, dAT(j,j), lddat, dwork(0), cols, queues[0] );
                magma_queue_sync( queues[0] );
                
                magma_cgetmatrix_async( m-j*nb, nb, dwork(0), cols, work, lda, queues[1] );
            ...
            magma_csetmatrix_async( m-j*nb, nb, work, lda, dwork(0), cols, queues[1] );
            
            for( i=j*nb; i < j*nb + nb; ++i ) {
                ipiv[i] += j*nb;
            }
            magmablas_claswp( n, dAT(0,0), lddat, j*nb + 1, j*nb + nb, ipiv, 1, queues[0] );

            magma_queue_sync( queues[1] );
            
            magmablas_ctranspose( cols, nb, dwork(0), cols, dAT(j,j), lddat, queues[0] );
        ...
        if ( nb0 > 0 ) {
            ...
            magmablas_ctranspose( nb0, rows, dAT(s,s), lddat, dwork(0), cols, queues[0] );
            magma_cgetmatrix_async( rows, nb0, dwork(0), cols, work, lda, queues[0] );
            magma_queue_sync( queues[0] );
            ...
            magmablas_claswp( n, dAT(0,0), lddat, s*nb + 1, s*nb + nb0, ipiv, 1, queues[0] );
            
            // put j-th panel onto device
            magma_csetmatrix_async( rows, nb0, work, lda, dwork(0), cols, queues[0] );
            magmablas_ctranspose( rows, nb0, dwork(0), cols, dAT(s,s), lddat, queues[0] );
        ...
        if (maxdim*maxdim < 2*maxm*maxn) {
            magmablas_ctranspose_inplace( maxdim, dAT(0,0), lddat, queues[0] );
            magma_cgetmatrix( m, n, dAT(0,0), lddat, A, lda, queues[0] );
        }
        else {

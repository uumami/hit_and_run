/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zungqr_2stage_gpu.cpp, normal z -> d, Sun Sep 15 17:14:36 2019
...
/***************************************************************************//**
    ...
    @param[in,out]
    dA      DOUBLE PRECISION array A on the GPU device,
    ...
    @param[in]
    dT      DOUBLE PRECISION work space array on the GPU device,
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if ( (nb > 1) && (nb < k) ) {
        ...
        magmablas_dlaset( MagmaFull, kk,   n-kk, c_zero, c_zero, dA(0, kk), ldda, queues[0] );
        magmablas_dlaset( MagmaFull, m-kk, n-kk, c_zero, c_one,  dA(kk,kk), ldda, queues[0] );
    }
    else {
        ki = 0;
        kk = 0;
    ...
    if (kk < n) {
        ...
        i__2 = n - kk;
        i__3 = k - kk;
        //magma_dgetmatrix( i__1, i__2, dA(kk, kk), ldda, panel, i__1, queues[0] );
        //lapackf77_dorgqr(&i__1, &i__2, &i__3, panel, &i__1, &tau[kk],
        //                 work, &lwork, &iinfo);
        //
        //magma_dsetmatrix( i__1, i__2, panel, i__1, dA(kk, kk), ldda, queues[0] );
        ...
        //magmablas_dlaset( MagmaFull, kk-nb,     nb, c_zero, c_zero, dA(0,kk-nb),     ldda, queues[0] );
        //magmablas_dlaset( MagmaFull, m-(kk-nb), nb, c_zero, c_one,  dA(kk-nb,kk-nb), ldda, queues[0] );
    }
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= nb; i -= nb) {
            ...
            i__2 = m - i;
            //magma_dgetmatrix_async( i__2, ib, dA(i,i), ldda, panel, i__2, queues[0] );  // verify
            if (i + ib < n) {
                /* Apply H to A(i:m,i+ib:n) from the left */
                i__3 = n - i;

                magmablas_dlaset( MagmaFull, i,   ib, c_zero, c_zero, dA(0,i), ldda, queues[0] );
                magmablas_dlaset( MagmaFull, m-i, ib, c_zero, c_one,  dA(i,i), ldda, queues[0] );
            ...
            //                 work, &lwork, &iinfo);
            //magma_dsetmatrix_async( i__2, ib, panel, i__2, dA(i,i), ldda, queues[1] );  // verify

            /* Set rows 1:i-1 of current block to zero */
            i__2 = i + ib;
            //magmablas_dlaset( MagmaFull, i-ib,     ib, c_zero, c_zero, dA(0,i-ib),    ldda, queues[0] );
            //magmablas_dlaset( MagmaFull, m-(i-ib), ib, c_zero, c_one,  dA(i-ib,i-ib), ldda, queues[0] );
        }
    }

    magmablas_dlaset( MagmaFull, m, nb, c_zero, c_one, dA(0,0), ldda, queues[0] );

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zunglq.cpp, normal z -> s, Sun Sep 15 17:13:55 2019
...
/***************************************************************************//**
    ...
    @param[out]
    work    REAL array, dimension (MAX(1,LWORK))
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if (lwork < nb*nb) {
        if (MAGMA_SUCCESS != magma_smalloc_cpu( &work_local, lwkopt )) {
    ...
    if (MAGMA_SUCCESS != magma_smalloc( &dA, ldda*n + n*nb + lddwork*nb + nb*nb )) {
        *info = MAGMA_ERR_DEVICE_ALLOC;
        goto cleanup;
    }
    
    magmaFloat_ptr dV, dW, dT;
    dV = dA + ldda*n;
    dW = dA + ldda*n + n*nb;
    ...
    magmablas_slaset( MagmaFull, m, n, MAGMA_S_NAN, MAGMA_S_NAN, dA, ldda, queue );
    ...
    for( i=ki; i >= 0; i -= nb ) {
        ...
        lapackf77_slaset( "Lower", &ib, &ib, &c_zero, &c_one, A(i,i), &lda );
        // TODO: having this _async was causing numerical errors. Why?
        magma_ssetmatrix( ib, n-i,
                                A(i,i), lda,
                                dV,     nb, queue );
        ...
        n_i = n - i;
        lapackf77_slarft( MagmaForwardStr, MagmaRowwiseStr, &n_i, &ib,
        ...
        magmablas_slaset( MagmaFull, mib, i,   c_zero, c_zero, dA(i,0), ldda, queue );
        magmablas_slaset( MagmaFull, mib, n-i, c_zero, c_one,  dA(i,i), ldda, queue );
        
        if (i < m) {
            // Apply H**H to A(i:m,i:n) from the right
            magma_slarfb_gpu( MagmaRight, MagmaConjTrans, MagmaForward, MagmaRowwise,
                              m-i, n-i, ib,
                              dV, nb,        dT, nb,
...
cleanup:
    ...
    work[0] = magma_smake_lwork( lwkopt );  // before free( work_local )

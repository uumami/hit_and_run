/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zbulge_back.cpp, normal z -> s, Sun Sep 15 17:14:34 2019
...
/******************************************************************************/
typedef struct magma_sapplyQ_data_s {
...
/******************************************************************************/
void magma_sapplyQ_data_init(
    magma_sapplyQ_data *zapplyQ_data, magma_int_t threads_num,
...
/******************************************************************************/
void magma_sapplyQ_data_destroy(
    magma_sapplyQ_data *zapplyQ_data)
{
...
/******************************************************************************/
typedef struct magma_sapplyQ_id_data {
    magma_int_t id;
    magma_sapplyQ_data* data;
} magma_sapplyQ_id_data;


/******************************************************************************/
void magma_sapplyQ_id_data_init(
    magma_sapplyQ_id_data *zapplyQ_id_data,
    magma_int_t id, magma_sapplyQ_data* data)
{
    ...
    magma_int_t* info)
{
    ...
    if (n_gpu < ne) {
        ...
        #endif
        magma_sapplyQ_data data_applyQ;
        magma_sapplyQ_data_init(&data_applyQ, threads, n, ne, n_gpu, nb, Vblksiz, Z, ldz, V, ldv, TAU, T, ldt, dZ, lddz);

        magma_sapplyQ_id_data* arg;
        magma_malloc_cpu((void**) &arg, threads*sizeof(magma_sapplyQ_id_data));
        ...
        for (magma_int_t thread = 1; thread < threads; thread++) {
            magma_sapplyQ_id_data_init(&(arg[thread]), thread, &data_applyQ);
            pthread_create(&thread_id[thread], &thread_attr, magma_sapplyQ_parallel_section, &arg[thread]);
        }
        magma_sapplyQ_id_data_init(&(arg[0]), 0, &data_applyQ);
        ...
        magma_free_cpu(arg);
        magma_sapplyQ_data_destroy(&data_applyQ);


        magma_ssetmatrix( n, ne-n_gpu, Z + n_gpu*ldz, ldz, dZ + n_gpu*ldz, lddz, queue );

        /*============================
         *  use only GPU
         *==========================*/
    } else {
        magma_ssetmatrix( n, ne, Z, ldz, dZ, lddz, queue );
        magma_sbulge_applyQ_v2(MagmaLeft, ne, n, nb, Vblksiz, dZ, lddz, V, ldv, T, ldt, info);
    }
...
static void *magma_sapplyQ_parallel_section(void *arg)
{
    ...
    if (my_core_id == 0) {
        ...
        magma_ssetmatrix( n, n_gpu, E, lde, dE, ldde, queue );
        magma_sbulge_applyQ_v2(MagmaLeft, n_gpu, n, nb, Vblksiz, dE, ldde, V, ldv, T, ldt, &info);
...
/******************************************************************************/
// TODO: this is identical to function in sbulge_back_m.cpp
static void magma_stile_bulge_applyQ(
    magma_int_t core_id, magma_side_t side, magma_int_t n_loc,
    magma_int_t n, magma_int_t nb, magma_int_t Vblksiz,
    float *E, magma_int_t lde,
    ...
    //, magma_int_t* info)
{
    ...
    if ((core_id == 0) || (core_id == 1))
        printf("  APPLY Q2_cpu sbulge_back   N %lld  N_loc %lld  nbchunk %lld  NB %lld  Vblksiz %lld  SIDE %c\n", n, n_loc, nbchunk, nb, Vblksiz, side);
    #endif

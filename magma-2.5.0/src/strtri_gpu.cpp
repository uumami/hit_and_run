/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/ztrtri_gpu.cpp, normal z -> s, Sun Sep 15 17:13:04 2019
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nounit) {
        for (j=0; j < n; ++j) {
            if ( MAGMA_S_EQUAL( *dA(j,j), c_zero )) {
    ...
    */
    ...
    if (nb <= 1 || nb >= n) {
        ...
        lapackf77_strtri( uplo_, diag_, &n, work, &n, info );
        ...
        for (j=0; j < n; j += nb) {
            ...
            lapackf77_strtri( MagmaUpperStr, diag_, &jb, work, &jb, info );
        ...
        for (j=nn; j >= 0; j -= nb) {
            ...
            /* Compute inverse of current diagonal block */
            lapackf77_strtri( MagmaLowerStr, diag_, &jb, work, &jb, info );

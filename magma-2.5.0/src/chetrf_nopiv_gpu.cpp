/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zhetrf_nopiv_gpu.cpp, normal z -> c, Sun Sep 15 17:14:13 2019
*/
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (j=0; j < n; j += nb) {
            ...
            magma_cgetmatrix_async( jb, jb, dA(j, j), ldda, A(j,j), nb, queue[1] );
            trace_gpu_end( 0, 0 );

            // factorize the diagonal block
            magma_queue_sync( queue[1] );
            trace_cpu_start( 0, "potrf", "potrf" );
            magma_chetrf_nopiv_cpu( MagmaUpper, jb, ib, A(j, j), nb, info );
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
            ...
            magma_csetmatrix_async( jb, jb, A(j, j), nb, dA(j, j), ldda, queue[0] );
            trace_gpu_end( 0, 0 );
            ...
            if ( (j+jb) < n) {
                ...
                magma_ctrsm( MagmaLeft, MagmaUpper, MagmaConjTrans, MagmaUnit,
                             ...
                             dA(j, j+jb), ldda,
                             queue[0] );
                magma_ccopymatrix( jb, n-j-jb, dA( j, j+jb ), ldda, dWt( 0, j+jb ), nb, queue[0] );
        ...
        for (j=0; j < n; j += nb) {
            ...
            magma_queue_sync( queue[1] );
            trace_cpu_start( 0, "potrf", "potrf" );
            magma_chetrf_nopiv_cpu( MagmaLower, jb, ib, A(j, j), nb, info );
            trace_cpu_end( 0 );
            if (*info != 0) {
                *info = *info + j;
                break;
            ...
            if ( (j+jb) < n) {
                ...
                magma_ctrsm( MagmaRight, MagmaLower, MagmaConjTrans, MagmaUnit,
                             ...
                             dA(j+jb, j), ldda,
                             queue[0]);
                magma_ccopymatrix( n-j-jb,jb, dA( j+jb, j ), ldda, dW( j+jb, 0 ), ldda, queue[0] );

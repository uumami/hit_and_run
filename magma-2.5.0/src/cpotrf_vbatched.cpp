/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zpotrf_vbatched.cpp, normal z -> c, Sun Sep 15 17:15:31 2019
*/
#define PRECISION_c
    ...
    magma_int_t *info_array,  magma_int_t batchCount, magma_queue_t queue)
{
    ...
    magmaFloatComplex **hA_array   = NULL;
    ...
    magma_malloc_cpu((void**) &hA_array, batchCount*sizeof(magmaFloatComplex*));
    ...
    // compute the total size for invA = sum of magma_roundup( n[], CTRTRI_BATCHED_NB )*CTRTRI_BATCHED_NB; 
    magma_ivec_roundup( batchCount, n, CTRTRI_BATCHED_NB, dinvA_lvec, queue); 
    magma_ivec_mulc( batchCount, dinvA_lvec, CTRTRI_BATCHED_NB, dinvA_lvec, queue);
    ...
    // dinvA and dwork are workspace in ctrsm
    magmaFloatComplex* dinvA      = NULL;
    magmaFloatComplex* dwork      = NULL; 
    magma_cmalloc( &dinvA, total_invA_msize);
    magma_cmalloc( &dwork, total_dwork_msize );
    ...
    // dimensions to pass to claset
    magma_int_t dinvA_m, dinvA_n, dwork_m, dwork_n; 
    dinvA_m = total_invA_msize  / CTRTRI_BATCHED_NB; // full division, no remainder
    dwork_m = total_dwork_msize / nb;             // full division, no remainder
    dinvA_n = CTRTRI_BATCHED_NB; 
    dwork_n = nb; 
    magmablas_claset( MagmaFull, dinvA_m, dinvA_n, MAGMA_C_ZERO, MAGMA_C_ZERO, dinvA, dinvA_m, queue );
    magmablas_claset( MagmaFull, dwork_m, dwork_n, MAGMA_C_ZERO, MAGMA_C_ZERO, dwork, dwork_m, queue );
    ...
    magma_cset_pointer_var_cc( dwork_array, dwork, dw_aux, 0, 0, dwork_batch_offset, batchCount, queue);
    magma_cset_pointer_var_cc( dinvA_array, dinvA, dw_aux, 0, 0, dinvA_batch_offset, batchCount, queue);
    ...
    for(j = 0; j < max_n; j+=nb) {
        ...
        //panel
        magma_cdisplace_pointers_var_cc(dA_displ, dA_array, ldda, j, j, batchCount, queue);
        arginfo = magma_cpotrf_panel_vbatched(
                           uplo, njvec, max_n-j, 
                           ibvec, nb,  
                           dA_displ, ldda,
        ...
        if( njvec_max > 0 ){
            magma_int_t use_streamed_herk = magma_crecommend_cublas_gemm_stream(MagmaNoTrans, MagmaTrans, njvec_max, njvec_max, nb);
            if( use_streamed_herk ){ 
                if(cpu_arrays_copied == false) {
                    magma_getvector( batchCount, sizeof(magmaFloatComplex*), dA_array, 1, hA_array, 1, queue);
                ...
                for(k=0; k<batchCount; k++){
                    ...
                    if(ncpu[k]-j-my_ib > 0 && my_ib > 0){
                        magma_cherk( MagmaLower, MagmaNoTrans, ncpu[k]-j-my_ib, my_ib, 
                            d_alpha, 
                            (const magmaFloatComplex*) hA_array[k] + j+my_ib+j*lda[k], lda[k], 
                            d_beta,
                ...
                magma_cdisplace_pointers_var_cc(dA_displ, dA_array, ldda, j+ib, j, batchCount, queue);
                magma_cdisplace_pointers_var_cc(dW1_displ, dA_array, ldda, j+ib, j+ib, batchCount, queue);
                magmablas_cherk_vbatched_max_nocheck( uplo, MagmaNoTrans, njvec, ibvec, 
...
{   
    magma_int_t arginfo = 0;
    magma_int_t crossover = magma_get_cpotrf_vbatched_crossover();   
    if( max_n > crossover ) {   
        arginfo = magma_cpotrf_lg_vbatched(uplo, n, max_n, dA_array, ldda, info_array,  batchCount, queue);
    }
    else{
        arginfo = magma_cpotrf_lpout_vbatched(uplo, n, max_n, dA_array, ldda, 0, info_array, batchCount, queue);
    }
    magma_queue_sync(queue);
    return arginfo;
}
/***************************************************************************//**
    ...
    dA_array      Array of pointers, dimension (batchCount).
             Each is a COMPLEX array A on the GPU, dimension (LDDA,N)
...
{   
    ...
    arginfo = magma_cpotrf_vbatched_max_nocheck(uplo, n, dA_array, ldda, info_array,  batchCount, max_n, queue);

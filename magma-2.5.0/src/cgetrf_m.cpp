    ...
    *info = 0;
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( I=0; I < n; I += NB ) {
            ...
            timer_start( time );
            magmablas_csetmatrix_transpose_mgpu(ngpu, M, N, nb, A(0,I), lda,
            ...
            for( offset = 0; offset < min(m,I); offset += NB ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_setdevice(d);
                    magmablas_claswp( ldn_local, dAT(d,0,0), ldn_local, offset+1, offset+NBk, ipiv, 1, 
                                      queues[d][1] );
                }
            ...
            timer_start( time );
            magmablas_cgetmatrix_transpose_mgpu(ngpu, M, N, nb, dAT, ldn_local,

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgetri_gpu.cpp, normal z -> c, Sun Sep 15 17:13:20 2019
    ...
    *info = 0;
    ...
    for( j = jmax; j >= 0; j -= nb ) {
        ...
        magmablas_clacpy( MagmaFull, n-j, jb,
                          dA(j,j), ldda,
                          dL(j,0), lddl, queue );
        magmablas_claset( MagmaLower, n-j-1, jb, c_zero, c_zero, dA(j+1,j), ldda, queue );
    ...
    for( j = n-2; j >= 0; --j ) {
        jp = ipiv[j] - 1;
        if ( jp != j ) {
            magmablas_cswap( n, dA(0,j), 1, dA(0,jp), 1, queue );
        }
    }

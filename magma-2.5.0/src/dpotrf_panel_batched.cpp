/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zpotrf_panel_batched.cpp, normal z -> d, Sun Sep 15 17:15:22 2019
*/
...
#define dAarray(i,j)    dA_array, i, j
    ...
    if (n < nb) {
        printf("magma_dpotrf_panel error n < nb %lld < %lld\n", (long long) n, (long long) nb );
        return -101;
    }
...
#define dAarray(i,j)    dA_array, i, j
    ...
    if (panel_nb <= min_recpnb) {
        ...
        // update A22
        magma_dgemm_batched_core( MagmaNoTrans, MagmaConjTrans, n2, n2, n1,
                             alpha, dAarray(ai+n1, aj   ), ldda, 
                                    dAarray(ai+n1, aj   ), ldda, 
                             beta,  dAarray(ai+n1, aj+n1), ldda, 
                             batchCount, queue );

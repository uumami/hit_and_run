/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zhegst_m.cpp, normal z -> s, Sun Sep 15 17:14:48 2019
*/
#include "magma_internal.h"


/******************************************************************************/
static void magma_ssygst_m_1_U_row_update(
    ...
    magma_queue_t queue )
{
    ...
    magma_sgemm( MagmaConjTrans, MagmaNoTrans, kb, nk-kb, nb,
                 c_neg_one, dC1,              lddc1,
                            dC2 + kb*lddc2,   lddc2,
...
/******************************************************************************/
static void magma_ssygst_m_1_L_col_update(
    magma_int_t nk, magma_int_t nb,
    ...
    magma_queue_t queue )
{
    ...
    magma_ssyr2k( MagmaLower, MagmaNoTrans, kb, nb,
                  c_neg_one, dC1,    lddc1,
                             dC2,    lddc2,
                  d_one,     dA_col, ldda, queue );

    magma_sgemm( MagmaNoTrans, MagmaConjTrans, nk-kb, kb, nb,
...
/***************************************************************************//**
    Purpose
    -------
    SSYGST_M reduces a real symmetric-definite generalized
    ...
    magma_int_t *info)
{
    ...
    magmaFloat_ptr dw[MagmaMaxGPUs];
    ...
    magma_int_t nb = magma_get_ssygst_m_nb( n );
    ...
    *info = 0;
    ...
    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_SUCCESS != magma_smalloc( &dw[igpu], (dima*ldda + lddbc*lddbr + n*nb) )) {
        ...
        if (upper) {
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k+1 < nblock) {
                    ...
                    magma_ssymm( MagmaLeft, uplo, kb, n-(k+1)*nb,
                                 c_neg_half, dA(igpu_p, k/ngpu, k),    ldda,
                                             dB_r(igpu_p, ind_k, k+1), lddbr,
                                 c_one,      dA(igpu_p, k/ngpu, k+1),  ldda, queues[igpu_p][2] );

                    magma_sgetmatrix_async( kb, n-(k+1)*nb,
            ...
            if (n > nb) {
                ...
                for (magma_int_t j = 1; j < nblock; ++j) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_setdevice( igpu );
                        if (nloc[igpu] > 0) {
                            magma_ssetmatrix_async( jb, n-j*nb,
                                                    B(j, j),            ldb,
                            ...
                            if ( j < nblock-1 ) {
                                magma_sgemm( MagmaNoTrans, MagmaNoTrans, nloc[igpu], n-(j+1)*nb, nb,
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j=k+1; j < nblock; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            magma_setdevice( igpu );
                            // ssyr2k on j-th column
                            magma_ssygst_m_1_L_col_update(
                                n-j*nb, nb,
                ...
                if (k+1 < nblock) {
                    ...
                    magma_ssymm( MagmaRight, uplo, n-(k+1)*nb, kb,
                                 c_neg_half, dA(igpu_p, k, k/ngpu),    ldda,
                                             dB_c(igpu_p, k+1, ind_k), lddbc,
                                 c_one,      dA(igpu_p, k+1, k/ngpu),  ldda, queues[igpu_p][2] );

                    magma_sgetmatrix_async( n-(k+1)*nb, kb,
            ...
            if (n > nb) {
                ...
                for (magma_int_t j = 1; j < nblock; ++j) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_setdevice( igpu );
                        if (nloc[igpu] > 0) {
                            magma_ssetmatrix_async( (n-j*nb), jb,
                                                    B(j, j),            ldb,
                            ...
                            if ( j < nblock-1 ) {
                                magma_sgemm( MagmaNoTrans, MagmaNoTrans, n-(j+1)*nb, nloc[igpu], nb,
        ...
        if (upper) {
            ...
            if (n > nb) {
                ...
                for (magma_int_t k = 0; k < nblock; ++k) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_setdevice( igpu );
                        magma_ssetmatrix_async( k*nb + kb, kb,
                                                B(0, k),            ldb,
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j = 0; j < k; ++j) {
                        ...
                        magma_ssyr2k( uplo, MagmaNoTrans, nb, kb,
                                      c_one, dwork(igpu, j, 0),    n,
                                             dB_c(igpu, j, ind_k), lddbc,
                                      d_one, dA(igpu, j, j/ngpu),  ldda, queues[igpu][1] );

                        magma_sgemm( MagmaNoTrans, MagmaConjTrans, j*nb, nb, kb,
                ...
                for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                    ...
                    magma_queue_sync( queues[igpu][0] ); // sync B copy (conflicts with ssygst)
                }
            ...
            for (magma_int_t j = 0; j < nblock; ++j) {
                ...
                magma_setdevice( igpu );
                magma_int_t jb = min( nb, n-j*nb );
                magma_sgetmatrix_async( j*nb+jb, jb,
            ...
            if (n > nb) {
                ...
                for (magma_int_t k = 0; k < nblock; ++k) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_setdevice( igpu );
                        magma_ssetmatrix_async( kb, k*nb +kb,
                                                B(k, 0),            ldb,
                        ...
                        if (igpu == igpu_p) {
                            magma_sgetmatrix_async( kb, k*nb,
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j = 0; j < k; ++j) {
                        ...
                        magma_ssyr2k( uplo, MagmaConjTrans, nb, kb,
                                      c_one, dwork(igpu, 0, j),    nb,
                                             dB_r(igpu, ind_k, j), lddbr,
                                      d_one, dA(igpu, j/ngpu, j),  ldda, queues[igpu][1] );

                        magma_sgemm( MagmaConjTrans, MagmaNoTrans, nb, j*nb, kb,
                                     c_one, dwork(igpu, 0, j),    nb,
            ...
            for (magma_int_t j = 0; j < nblock; ++j) {
                ...
                magma_setdevice( igpu );
                magma_int_t jb = min( nb, n-j*nb );
                magma_sgetmatrix_async( jb, j*nb+jb,

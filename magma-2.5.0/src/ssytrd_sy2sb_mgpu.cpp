/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zhetrd_he2hb_mgpu.cpp, normal z -> s, Sun Sep 15 17:14:43 2019
...
/***************************************************************************//**
    ...
    @param[in,out]
    dAmgpu  REAL array of pointer, dimension (ngpu)
            Each point to a REAL array, dimension (LDDA, nlocal)
    ...
    @param[in,out]
    dTmgpu  REAL array of pointer, dimension (ngpu)
            Each point to a REAL array on the GPU, dimension n*nb,
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    float *dspace[MagmaMaxGPUs];
    float *dwork[MagmaMaxGPUs], *dworkbis[MagmaMaxGPUs];
    float *dvall[MagmaMaxGPUs], *dv[MagmaMaxGPUs], *dw[MagmaMaxGPUs];
    ...
    for( magma_int_t dev = 0; dev < ngpu; ++dev ) {
        magma_setdevice( dev );
        magma_smalloc( &dspace[dev], devworksiz );
    ...
    if (upper) {
        ...
        for (i = 1; i <= n-nb; i += nb) {
            ...
            // To be able to overlap the GET with the SSYR2K
            // it should be done on last stream.
            // TO Avoid a BUG that is overwriting the old_V
            // used atthis moment by ssyr2k with the new_V
            ...
            for( magma_int_t dev = 0; dev < ngpu; ++dev ) {
                // Here we have to wait until the broadcast of SSYMM has been done.
            ...
            // because next SSYR2K use streaming and may happen
            ...
            if (i + nb <= n-nb) {
                ...
                idev   = ((indi-1) / distblk) % ngpu;          // device with this block
                magma_setdevice( idev );
                //printf("LAST SSYR2K idev %d on A(%d,%d) of size %d\n",idev, indi-1,di,pk);

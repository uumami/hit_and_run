/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/ztrsm_m.cpp, normal z -> s, Sun Sep 15 17:14:51 2019
*/
...
#define dA(gpui, i, j) (dw[gpui] + dimb*lddb + (i)*nb + (j)*nb*ldda)
    ...
    for (dev = 0; dev < ngpu; ++dev) {
        magma_setdevice( dev );
        if (MAGMA_SUCCESS != magma_smalloc( &dw[dev], (dimb*lddb + dima*ldda) )) {
        ...
        if (notrans) {
            ...
            if (upper) {
                ...
                for (j = mbl-1; j >= 0; --j) {
                    ...
                    for (dev = 0; dev < ngpu; ++dev) {
                        magma_setdevice( dev );
                        magma_strsm( side, uplo, transa, diag, jb, nloc[dev],
                    ...
                    if (j > 0) {
                        for (dev = 0; dev < ngpu; ++dev) {
                            magma_setdevice( dev );
                            magma_sgemm( transa, MagmaNoTrans, j*nb, nloc[dev], jb,
                ...
                for (j = 0; j < mbl; ++j) {
                    ...
                    if ( j < mbl-1 ) {
                        for (dev = 0; dev < ngpu; ++dev) {
                            magma_setdevice( dev );
                            magma_sgemm( transa, MagmaNoTrans, m-(j+1)*nb, nloc[dev], nb,
        ...
        if (notrans) {
            ...
            if (upper) {
                ...
                for (k = 0; k < nbl; ++k) {
                    ...
                    for (dev = 0; dev < ngpu; ++dev) {
                        magma_setdevice( dev );
                        magma_strsm( side, uplo, transa, diag, mloc[dev], kb,
                    ...
                    if ( k < nbl-1 ) {
                        for (dev = 0; dev < ngpu; ++dev) {
                            magma_setdevice( dev );
                            magma_sgemm( MagmaNoTrans, transa, mloc[dev], n-(k+1)*nb, nb,
                ...
                for (k = nbl-1; k >= 0; --k) {
                    ...
                    if (k > 0) {
                        for (dev = 0; dev < ngpu; ++dev) {
                            magma_setdevice( dev );
                            magma_sgemm( MagmaNoTrans, transa, mloc[dev], k*nb, kb,
                                         c_neg_one, dB(dev, 0, k),   lddb,
                                                    dA(dev, k%2, 0), ldda,

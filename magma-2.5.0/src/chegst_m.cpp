/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zhegst_m.cpp, normal z -> c, Sun Sep 15 17:14:50 2019
*/
...
static void magma_chegst_m_1_U_row_update(
    ...
    magmaFloatComplex* dC1, magma_int_t lddc1,
    magmaFloatComplex* dC2, magma_int_t lddc2,
    magma_queue_t queue )
{
    ...
    magma_cher2k( MagmaUpper, MagmaConjTrans, kb, nb,
                  c_neg_one, dC1,    lddc1,
                             dC2,    lddc2,
                  d_one,     dA_row, ldda, queue );
    
    magma_cgemm( MagmaConjTrans, MagmaNoTrans, kb, nk-kb, nb,
                 c_neg_one, dC1,              lddc1,
                            dC2 + kb*lddc2,   lddc2,
    ...
    magma_queue_t queue )
{
    ...
    magma_cher2k( MagmaLower, MagmaNoTrans, kb, nb,
                  c_neg_one, dC1,    lddc1,
                             dC2,    lddc2,
                  d_one,     dA_col, ldda, queue );

    magma_cgemm( MagmaNoTrans, MagmaConjTrans, nk-kb, kb, nb,
    ...
    magma_int_t *info)
{
    ...
    magmaFloatComplex_ptr dw[MagmaMaxGPUs];
    ...
    *info = 0;
    ...
    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
        ...
        if (MAGMA_SUCCESS != magma_cmalloc( &dw[igpu], (dima*ldda + lddbc*lddbr + n*nb) )) {
        ...
        if (upper) {
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k+1 < nblock) {
                    ...
                    magma_chemm( MagmaLeft, uplo, kb, n-(k+1)*nb,
                                 c_neg_half, dA(igpu_p, k/ngpu, k),    ldda,
                                             dB_r(igpu_p, ind_k, k+1), lddbr,
                                 c_one,      dA(igpu_p, k/ngpu, k+1),  ldda, queues[igpu_p][2] );

                    magma_cgetmatrix_async( kb, n-(k+1)*nb,
            ...
            if (n > nb) {
                ...
                for (magma_int_t j = 1; j < nblock; ++j) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_setdevice( igpu );
                        if (nloc[igpu] > 0) {
                            magma_csetmatrix_async( jb, n-j*nb,
                                                    B(j, j),            ldb,
                            ...
                            if ( j < nblock-1 ) {
                                magma_cgemm( MagmaNoTrans, MagmaNoTrans, nloc[igpu], n-(j+1)*nb, nb,
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j=k+1; j < nblock; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            magma_setdevice( igpu );
                            // cher2k on j-th column
                            magma_chegst_m_1_L_col_update(
                                n-j*nb, nb,
                ...
                if (k+1 < nblock) {
                    ...
                    magma_chemm( MagmaRight, uplo, n-(k+1)*nb, kb,
                                 c_neg_half, dA(igpu_p, k, k/ngpu),    ldda,
                                             dB_c(igpu_p, k+1, ind_k), lddbc,
                                 c_one,      dA(igpu_p, k+1, k/ngpu),  ldda, queues[igpu_p][2] );

                    magma_cgetmatrix_async( n-(k+1)*nb, kb,
            ...
            if (n > nb) {
                ...
                for (magma_int_t j = 1; j < nblock; ++j) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_setdevice( igpu );
                        if (nloc[igpu] > 0) {
                            magma_csetmatrix_async( (n-j*nb), jb,
                                                    B(j, j),            ldb,
                            ...
                            if ( j < nblock-1 ) {
                                magma_cgemm( MagmaNoTrans, MagmaNoTrans, n-(j+1)*nb, nloc[igpu], nb,
        ...
        if (upper) {
            ...
            if (n > nb) {
                ...
                for (magma_int_t k = 0; k < nblock; ++k) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_setdevice( igpu );
                        magma_csetmatrix_async( k*nb + kb, kb,
                                                B(0, k),            ldb,
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j = 0; j < k; ++j) {
                        ...
                        magma_cher2k( uplo, MagmaNoTrans, nb, kb,
                                      c_one, dwork(igpu, j, 0),    n,
                                             dB_c(igpu, j, ind_k), lddbc,
                                      d_one, dA(igpu, j, j/ngpu),  ldda, queues[igpu][1] );

                        magma_cgemm( MagmaNoTrans, MagmaConjTrans, j*nb, nb, kb,
                ...
                for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                    ...
                    magma_queue_sync( queues[igpu][0] ); // sync B copy (conflicts with chegst)
                }
            ...
            if (n > nb) {
                ...
                for (magma_int_t k = 0; k < nblock; ++k) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        magma_setdevice( igpu );
                        magma_csetmatrix_async( kb, k*nb +kb,
                                                B(k, 0),            ldb,
                        ...
                        if (igpu == igpu_p) {
                            magma_chemm( MagmaLeft, uplo, kb, k*nb,
                                         c_half, dA(igpu, k/ngpu, k), ldda,
                                                 dB_r(igpu, k%2, 0),  lddbr,
                                         c_one,  dA(igpu, k/ngpu, 0), ldda, queues[igpu][k%2] );
                        }

                        magma_cgemm( MagmaNoTrans, MagmaNoTrans, nloc[igpu], k*nb, kb,
                                     c_one, dA(igpu, iloc[igpu], k), ldda,
                                            dB_r(igpu, k%2, 0),      lddbr,
                                     c_one, dA(igpu, iloc[igpu], 0), ldda, queues[igpu][k%2] );

                        magma_ctrmm( MagmaRight, uplo, MagmaNoTrans, MagmaNonUnit, nloc[igpu], kb, c_one, dB_r(igpu, k%2, k), lddbr,
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j = 0; j < k; ++j) {
                        ...
                        magma_cher2k( uplo, MagmaConjTrans, nb, kb,
                                      c_one, dwork(igpu, 0, j),    nb,
                                             dB_r(igpu, ind_k, j), lddbr,
                                      d_one, dA(igpu, j/ngpu, j),  ldda, queues[igpu][1] );

                        magma_cgemm( MagmaConjTrans, MagmaNoTrans, nb, j*nb, kb,
                                     c_one, dwork(igpu, 0, j),    nb,

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zpotrf_batched.cpp, normal z -> d, Sun Sep 15 17:15:21 2019
*/
...
#define dAarray(i,j)  dA_array, i, j   
    ...
    // queues for streamed herk
    create_stream = magma_drecommend_cublas_gemm_stream(MagmaNoTrans, MagmaConjTrans, n-nb, n-nb, nb);
    if(create_stream){
    ...
    magma_queue_t queue)
{
    ...
    if (n > crossover){   
        arginfo = magma_dpotrf_lg_batched(uplo, n, dA_array, ldda, info_array, batchCount, queue);
            ...
            arginfo = magma_dpotrf_lpin_batched(uplo, n, dA_array, ldda, 0, info_array, batchCount, queue);
        #else

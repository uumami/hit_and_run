/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgeqrf_expert_batched.cpp, normal z -> c, Sun Sep 15 17:15:28 2019
*/
    ...
    magma_int_t *info_array, magma_int_t batchCount, magma_queue_t queue)
{
    ...
    magmaFloatComplex **dR_displ  = NULL;
    magmaFloatComplex **dT_displ  = NULL;
    ...
    magma_cdisplace_pointers(dR_displ, dR_array, lddr, 0, 0, batchCount, queue); 
    magma_cdisplace_pointers(dT_displ, dT_array, lddt, 0, 0, batchCount, queue); 
    ...
    magmablas_claset_batched( MagmaFull, lddr, (provide_RT > 0 ? n:min(min_mn,nb)), MAGMA_C_ZERO, MAGMA_C_ZERO, dR_displ, lddr, batchCount, queue ); 
    magmablas_claset_batched( MagmaFull, lddt, (provide_RT > 0 ? n:min(min_mn,nb)), MAGMA_C_ZERO, MAGMA_C_ZERO, dT_displ, lddt, batchCount, queue );
    /*
    if ( provide_RT > 0 )
    {
        magmablas_claset( MagmaFull, lddr, n*batchCount, MAGMA_C_ZERO, MAGMA_C_ZERO, dR, lddr, queue );
        magmablas_claset( MagmaFull, lddt, n*batchCount, MAGMA_C_ZERO, MAGMA_C_ZERO, dT, lddt, queue );
    }
    else
    {
        magmablas_claset( MagmaFull, lddr, nb*batchCount, MAGMA_C_ZERO, MAGMA_C_ZERO, dR, lddr, queue );
        magmablas_claset( MagmaFull, lddt, nb*batchCount, MAGMA_C_ZERO, MAGMA_C_ZERO, dT, lddt, queue );
    }
    */
    ...
    magma_getvector( batchCount, sizeof(magmaFloatComplex*), dT_array, 1, cpuTarray, 1, queue);
    ...
    for (i=0; i < min_mn; i += nb)
    {
        ...
        magma_cdisplace_pointers(dW2_displ, dtau_array, 1, i, 0, batchCount, queue);
        if ( provide_RT > 0 )
        {
            offset_RT = i;
            magma_cdisplace_pointers(dR_displ, dR_array, lddr, (provide_RT == 1 ? offset_RT:0), offset_RT, batchCount, queue); 
            magma_cdisplace_pointers(dT_displ, dT_array, lddt, 0, offset_RT, batchCount, queue); 
        }
        ...
        if ( (n-ib-i) > 0)
        {
            ...
            //reset dW4_displ
            ldw = nb;
            magma_cset_pointer( dW4_displ, dwork, 1, 0, 0,  ldw*n, batchCount, queue );
            offset = ldw*n*batchCount;
            magma_cset_pointer( dW5_displ, dwork + offset, 1, 0, 0,  ldw*n, batchCount, queue );    
            ...
            else
            {
                ...
                magma_clarfb_gemm_batched( 
                            ...
                            (const magmaFloatComplex**)dT_displ, lddt,
                            dW1_displ,  ldda,
                            dW4_displ,  ldw,
                            dW5_displ, ldw,
                            batchCount, queue );
        ...
        if ( provide_RT == 0 )
        {
            magmablas_clacpy_batched( MagmaUpper, ib, ib, dR_displ, lddr, dW0_displ, ldda, batchCount, queue );
        }
    }

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zpotrf_vbatched.cpp, normal z -> s, Sun Sep 15 17:15:30 2019
*/
#define PRECISION_s
    ...
    magma_int_t *info_array,  magma_int_t batchCount, magma_queue_t queue)
{
    ...
    magma_malloc_cpu((void**) &hA_array, batchCount*sizeof(float*));
    ...
    // compute the total size for invA = sum of magma_roundup( n[], STRTRI_BATCHED_NB )*STRTRI_BATCHED_NB; 
    magma_ivec_roundup( batchCount, n, STRTRI_BATCHED_NB, dinvA_lvec, queue); 
    magma_ivec_mulc( batchCount, dinvA_lvec, STRTRI_BATCHED_NB, dinvA_lvec, queue);
    ...
    // dinvA and dwork are workspace in strsm
    float* dinvA      = NULL;
    float* dwork      = NULL; 
    magma_smalloc( &dinvA, total_invA_msize);
    magma_smalloc( &dwork, total_dwork_msize );
    ...
    // dimensions to pass to slaset
    magma_int_t dinvA_m, dinvA_n, dwork_m, dwork_n; 
    dinvA_m = total_invA_msize  / STRTRI_BATCHED_NB; // full division, no remainder
    dwork_m = total_dwork_msize / nb;             // full division, no remainder
    dinvA_n = STRTRI_BATCHED_NB; 
    dwork_n = nb; 
    magmablas_slaset( MagmaFull, dinvA_m, dinvA_n, MAGMA_S_ZERO, MAGMA_S_ZERO, dinvA, dinvA_m, queue );
    magmablas_slaset( MagmaFull, dwork_m, dwork_n, MAGMA_S_ZERO, MAGMA_S_ZERO, dwork, dwork_m, queue );
    ...
    magma_sset_pointer_var_cc( dwork_array, dwork, dw_aux, 0, 0, dwork_batch_offset, batchCount, queue);
    magma_sset_pointer_var_cc( dinvA_array, dinvA, dw_aux, 0, 0, dinvA_batch_offset, batchCount, queue);
    ...
    for(j = 0; j < max_n; j+=nb) {
        ...
        //panel
        magma_sdisplace_pointers_var_cc(dA_displ, dA_array, ldda, j, j, batchCount, queue);
        arginfo = magma_spotrf_panel_vbatched(
                           uplo, njvec, max_n-j, 
                           ibvec, nb,  
                           dA_displ, ldda,
        ...
        if( njvec_max > 0 ){
            magma_int_t use_streamed_herk = magma_srecommend_cublas_gemm_stream(MagmaNoTrans, MagmaTrans, njvec_max, njvec_max, nb);
            if( use_streamed_herk ){ 
                if(cpu_arrays_copied == false) {
                    magma_getvector( batchCount, sizeof(float*), dA_array, 1, hA_array, 1, queue);
                ...
                for(k=0; k<batchCount; k++){
                    ...
                    if(ncpu[k]-j-my_ib > 0 && my_ib > 0){
                        magma_ssyrk( MagmaLower, MagmaNoTrans, ncpu[k]-j-my_ib, my_ib, 
                            d_alpha, 
                            (const float*) hA_array[k] + j+my_ib+j*lda[k], lda[k], 
                            d_beta,
                ...
                magma_sdisplace_pointers_var_cc(dA_displ, dA_array, ldda, j+ib, j, batchCount, queue);
                magma_sdisplace_pointers_var_cc(dW1_displ, dA_array, ldda, j+ib, j+ib, batchCount, queue);
                magmablas_ssyrk_vbatched_max_nocheck( uplo, MagmaNoTrans, njvec, ibvec, 
...
{   
    magma_int_t arginfo = 0;
    magma_int_t crossover = magma_get_spotrf_vbatched_crossover();   
    if( max_n > crossover ) {   
        arginfo = magma_spotrf_lg_vbatched(uplo, n, max_n, dA_array, ldda, info_array,  batchCount, queue);
    }
    else{
        arginfo = magma_spotrf_lpout_vbatched(uplo, n, max_n, dA_array, ldda, 0, info_array, batchCount, queue);
    }
    magma_queue_sync(queue);
    return arginfo;
}
/***************************************************************************//**
    ...
    dA_array      Array of pointers, dimension (batchCount).
             Each is a REAL array A on the GPU, dimension (LDDA,N)
...
{   
    ...
    arginfo = magma_spotrf_vbatched_max_nocheck(uplo, n, dA_array, ldda, info_array,  batchCount, max_n, queue);

    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if(n <= nb){
        //panel
        magma_cgetf2_nopiv_internal_batched(m, n, dAarray(ai, aj), ldda, info_array, gbstep, batchCount, queue);
    }
    else{
        magma_int_t n1 = n / 2;
        magma_int_t n2 = n-n1;
        ...
        // recpanel
        magma_cgetf2_nopiv_batched(m, n1, dAarray(ai, aj), ldda, info_array, gbstep, batchCount, queue);
        // trsm
        ...
        if(m-min_mn1 > 0){
            ...
            magma_cgemm_batched_core(
                ...
                batchCount, queue );
            // recpanel
            magma_cgetf2_nopiv_batched(m-min_mn1, n2, dAarray(ai+n1, aj+n1), ldda, info_array, gbstep+n1, batchCount, queue);
        }
    }
    return 0;

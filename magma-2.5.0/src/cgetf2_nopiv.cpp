/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgetf2_nopiv.cpp, normal z -> c, Sun Sep 15 17:13:28 2019
    ...
    *info = 0;
    ...
    for (j = 1; j <= min_mn; ++j) {
        ...
        if ( ! MAGMA_C_EQUAL( *A(j,j), c_zero)) {
            ...
            if (j < m) {
                if (MAGMA_C_ABS( *A(j,j) ) >= sfmin) {
                    m_j = m - j;
                    inv_Ajj = MAGMA_C_DIV(c_one, *A(j,j));
                    blasf77_cscal( &m_j, &inv_Ajj, A(j+1,j), &ione );
                }
                else {
                    m_j = m - j;
                    for (i = 1; i <= m_j; ++i) {
        ...
        if (j < min_mn) {
            ...
            m_j = m - j;
            n_j = n - j;
            blasf77_cgeru( &m_j, &n_j, &c_neg_one,
                           A(j+1,j),   &ione,
                           A(j,j+1),   &lda,
                           A(j+1,j+1), &lda );
        }

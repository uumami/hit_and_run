/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zpotrf3_mgpu.cpp, normal z -> s, Sun Sep 15 17:13:07 2019
...
#define dlPT(id, i, j, k) (d_lP[(id)] + (k)*nb*lddp + (j)*nb   + (i))
    ...
    *info = 0;
    ...
    for( d=0; d < ngpu; d++ ) {
        magma_setdevice(d);
        if ( (MAGMA_SUCCESS != magma_smalloc( &d_dinvA[d], dinvA_length )) ||
             (MAGMA_SUCCESS != magma_smalloc( &dx[d],      2*nb*(upper ? n : m) )) ) {
                    ...
                    if ( nb2 > 0 ) {
                        ...
                        trace_gpu_start( d, stream2, "trsm", "trsm" );
                            ...
                            if (flag == 0) {
                                ...
                                magmablas_slaset( MagmaFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,flag), trsm_nb, queues[d][stream2] );
                                magma_queue_wait_event( queues[d][stream2], events[d][1] ); // panel received
                            }
                            magmablas_slaset( MagmaFull, nb2, jb, c_zero, c_zero, dx(d,1), nb2, queues[d][stream2] );

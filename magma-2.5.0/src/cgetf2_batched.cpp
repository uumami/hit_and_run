/*
   ...
   @generated from src/zgetf2_batched.cpp, normal z -> c, Sun Sep 15 17:15:10 2019
*/
    ...
    for( panelj=0; panelj < min_mn; panelj += nb)
    {
        ...
        for (step=0; step < ib; step++) {
            ...
            //if ( required_shmem_size >  (MAX_SHARED_ALLOWED*1024))
            {
                ...
                arginfo = magma_icamax_batched(m-gbj, dA_array, ai, aj, 1, gbj, ldda, ipiv_array, info_array, gbstep, batchCount, queue);
                ...
                if (gbj < m) {
                    arginfo = magma_cscal_cgeru_batched( m-gbj, ib-step, gbj, dA_array, ai, aj, ldda, info_array, gbstep, batchCount, queue );
                    if (arginfo != 0 ) return arginfo;
                }
            }
            else {
                ...
                //printf("calling cswap batched, ai = %d, step = %d\n", ai, gbj);
                arginfo = magma_cswap_batched(n, dA_array, ai, aj, ldda, gbj, ipiv_array, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
            }
        }
        ...
        if ( (n-panelj-ib) > 0) {
            ...
            //magma_cdisplace_pointers(dW0_displ, dA_array, ldda, ib+panelj, panelj, batchCount, queue);
            //magma_cdisplace_pointers(dW1_displ, dA_array, ldda, panelj, ib+panelj, batchCount, queue);
...
#define ipiv_array(i) ipiv_array, i
    if(n <= 32){
        magma_cgetf2_fused_batched(m, n, dA_array(ai,aj), ldda, ipiv_array, info_array, batchCount, queue);
    }
    else{
        magma_int_t n1 = n / 2;
        magma_int_t n2 = n - n1;
        ...
        magmaFloatComplex* aa;
        if(pprint){
        printf("Panel(%lld,%lld):\n", (long long) ai, (long long) aj);
        magma_getvector(1, sizeof(magmaFloatComplex*), dA_array, 1, &aa, 1, queue);
        magma_cprint_gpu(4, 4, aa, ldda, queue);
        }
    ...
    magma_queue_t queue)
{
    ...
    if(m > CGETF2_FUSED_BATCHED_MAX_ROWS){
        magma_cgetf2_batched_v1(m, n, dA_array, ai, aj, ldda, ipiv_array, info_array, gbstep, batchCount, queue);   
    }
    else{
        magma_cgetf2_batched_v2(m, n, dA_array, ai, aj, ldda, ipiv_array, dpivinfo_array, info_array, batchCount, queue);
    }

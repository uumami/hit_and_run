/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zbulge_applyQ_v2_m.cpp, normal z -> c, Sun Sep 15 17:14:41 2019
*/
    ...
    magma_int_t *info)
{
    ...
    */
    ...
    magmaFloatComplex *dE[MagmaMaxGPUs]={NULL};
    magmaFloatComplex *dwork[MagmaMaxGPUs]={NULL};
    magmaFloatComplex *dwork0[MagmaMaxGPUs], *dwork1[MagmaMaxGPUs];
    ...
    for (dev=0; dev < ngpu; ++dev) {
        ...
        magma_csetmatrix_async( N, ie_loc, E+lde*ne_loc*dev, lde, dE(dev, 0, 0), ldde, queues[dev][1] );
    }
                ...
                for (blki=mt; blki > 0; blki--) {
                    ...
                    if (ncpy == 0) {
                        ...
                        if (mysiz > 0) {
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                magma_setdevice( dev );
                                magma_csetmatrix_async(vld, Vblksiz, V(vpos), vld, dV1[dev], vld, queues[dev][1]);
                                magma_csetmatrix_async(tld, Vblksiz, T(tpos), tld, dT1[dev], tld, queues[dev][1]);
                            }
                    ...
                    if (blkid == copyst) {
                        ...
                        if (mysiz > 0) {
                            ...
                            if (flip == 0) { // now I am working on dV0 so copy the next and put it on dV1
                                ...
                                for( dev = 0; dev < ngpu; ++dev ) {
                                    magma_setdevice( dev );
                                    magma_csetmatrix_async(vld, Vblksiz, V(vpos), vld, dV0[dev], vld, queues[dev][0]);
                                    magma_csetmatrix_async(tld, Vblksiz, T(tpos), tld, dT0[dev], tld, queues[dev][0]);
                                }
                            }
                        }
                    }
                        ...
                        if (flip == 0) {
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for (magma_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ...
                                    //magma_clarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib, queues[dev][0] );
                                    magma_clarfb_gpu_gemm( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV0[dev]+lcvpos, lddv, dT0[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork0[dev], ib, dwvt0[dev], Vm, queues[dev][0] );
                                }
                            ...
                            for( dev = 0; dev < ngpu; ++dev ) {
                                ...
                                for (magma_int_t i=0; i < ie_loc; i += sz_bl) {
                                    ib = min(sz_bl, ie_loc-i);
                                    //magma_clarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib, queues[dev][1] );
                                    magma_clarfb_gpu_gemm( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV1[dev]+lcvpos, lddv, dT1[dev]+lctpos, lddt, dE(dev,myrow,i), ldde, dwork1[dev], ib, dwvt1[dev], Vm, queues[dev][1] );
                                }
            ...
            for (blki = mt; blki > 0; blki--) {
                ...
                for (blkj = nt-1; blkj >= 0; blkj--) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_csetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, queues[dev][0]);
                        magma_csetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, queues[dev][0]);
                        ...
                        for (magma_int_t i=0; i < NE; i += sz_bl) {
                            ib = min(sz_bl, NE-i);
                            magma_clarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV0, lddv, dT0, lddt, dE(myrow,i), ldde, dwork, NE, queues[dev][0] );
                        }
                        */
                    } // end for (Vm &Vn) > 0
                } // end for blkj
            ...
            for (blkj=0; blkj < nt; blkj++) {
                ...
                for (blki=1; blki <= mt; blki++) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_csetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, queues[dev][0]);
                        magma_csetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, queues[dev][0]);
                        magma_clarfb_gpu( MagmaRight, MagmaNoTrans, MagmaForward, MagmaColumnwise, NE, Vm, Vn, dV0, lddv, dT0, lddt, dE(0, myrow), ldde, dwork, NE, queues[dev][0] );
                        */
                    } // end for (Vm &Vn) > 0
                } // end for blki
            } // end fo blkj
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_int_t ie_loc   = min(ne_loc, NE - ne_loc*dev);
        magma_cgetmatrix_async( N, ie_loc, dE(dev, 0, 0), ldde, E+lde*ne_loc*dev, lde, queues[dev][0] );
        magma_event_record( myevent[dev][0], queues[dev][0] );
    }

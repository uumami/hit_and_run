/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zunmrq.cpp, normal z -> d, Sun Sep 15 17:14:01 2019
*/
...
/***************************************************************************//**
    ...
    as returned by DGERQF. Q is of order M if SIDE = MagmaLeft and of order N
    if SIDE = MagmaRight.
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nb >= k) {
        /* Use CPU code */
        lapackf77_dormrq( lapack_side_const(side), lapack_trans_const(trans),
        ...
        for (int i = i1; (step < 0 ? i >= i2 : i < i2); i += step) {
            ...
            nq_i = nq - k + i + ib;
            lapackf77_dlarft("Backward", "Rowwise", &nq_i, &ib, A(i,0), &lda, &tau[i], T, &ib);

            /* 1) set upper triangle of panel in A to identity,
               2) copy the panel from A to the GPU, and
               3) restore A                                      */
            magma_dpanel_to_q( MagmaUpper, ib, A(i,nq_i-ib), lda, T2 );
            magma_dsetmatrix( ib, nq_i, A(i,0), lda, dV(0,0), ib, queue );
            magma_dq_to_panel( MagmaUpper, ib, A(i,nq_i-ib), lda, T2 );
            ...
            magma_dlarfb_gpu(side, transt, MagmaBackward, MagmaRowwise,
                             mi, ni, ib,
                             dV(0,0), ib,
                             dT(0,0), ib,

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zpotrf3_mgpu.cpp, normal z -> c, Sun Sep 15 17:13:08 2019
...
/***************************************************************************//**
    ...
    @param[in,out]
    d_lP    COMPLEX array of pointers on the GPU, dimension (ngpu).
    ...
    @param[in,out]
    A       COMPLEX array on the CPU, dimension (LDA,H*NB)
...
#define dlPT(id, i, j, k) (d_lP[(id)] + (k)*nb*lddp + (j)*nb   + (i))
    ...
    *info = 0;
    ...
    for( d=0; d < ngpu; d++ ) {
        magma_setdevice(d);
        if ( (MAGMA_SUCCESS != magma_cmalloc( &d_dinvA[d], dinvA_length )) ||
             (MAGMA_SUCCESS != magma_cmalloc( &dx[d],      2*nb*(upper ? n : m) )) ) {
                    ...
                    if ( nb2 > 0 ) {
                        ...
                        trace_gpu_start( d, stream2, "trsm", "trsm" );
                            ...
                            if (flag == 0) {
                                ...
                                magmablas_claset( MagmaFull, trsm_nb, trsm_n, c_zero, c_zero, dinvA(d,flag), trsm_nb, queues[d][stream2] );
                                magma_queue_wait_event( queues[d][stream2], events[d][1] ); // panel received
                            }
                            magmablas_claset( MagmaFull, nb2, jb, c_zero, c_zero, dx(d,1), nb2, queues[d][stream2] );

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zlahr2_m.cpp, normal z -> d, Sun Sep 15 17:15:01 2019
       @author Mark Gates
*/
    ...
    struct dgehrd_data *data )
{
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        magma_setdevice( dev );
        magmablas_dlaset( MagmaFull, nb, nb, c_zero, c_zero, dV(dev,k,0), ldv, data->queues[dev] );
    }
    ...
    for (i = 0; i < nb; ++i) {
        ...
        *T(i,i) = tau[i];
        ...
        for( dev = 0; dev < ngpu; ++dev ) {
            ...
            if ( dn-dki1 > 0 ) {
                // yi = yi + yi{dev}
                blasf77_daxpy( &n_k, &c_one, Y(k,nb+dev), &ione, Y(k,i), &ione );
            }
        }
    }
    ...
    *A(k+nb,nb-1) = ei;
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        ...
        if ( dn-dki1 > 0 ) {
            // Y = Y + Am V
            for( i = 0; i < nb; ++i ) {
                blasf77_daxpy( &k, &c_one, Y(0,nb+nb*dev+i), &ione, Y(0,i), &ione );
            }
        }
    }
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        magma_setdevice( dev );
        magma_dsetmatrix_async( n, nb, Y, ldy, dY(dev, 0, 0), ldda, data->queues[dev] );
        magma_dsetmatrix_async( nb, nb, T, nb, dTi(dev),      nb,   data->queues[dev] );
    }

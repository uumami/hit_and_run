/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zbulge_applyQ_v2.cpp, normal z -> s, Sun Sep 15 17:14:31 2019
*/
                ...
                for (blki=mt; blki > 0; blki--) {
                    ...
                    if (ncpy == 0) {
                        ...
                        if (mysiz > 0) {
                            ...
                            vld  = mysiz * ldv;
                            tld  = mysiz * ldt;
                            magma_ssetmatrix_async(vld, Vblksiz, V(vpos), vld, dV1, vld, queues[1]);
                            magma_ssetmatrix_async(tld, Vblksiz, T(tpos), tld, dT1, tld, queues[1]);
                    ...
                    if (blkid == copyst) {
                        ...
                        if (mysiz > 0) {
                            ...
                            if (flip == 0) { // now I am working on dV0 so copy the next and put it on dV1
                                ...
                                magma_ssetmatrix_async(vld, Vblksiz, V(vpos), vld, dV0, vld, queues[0]);
                                magma_ssetmatrix_async(tld, Vblksiz, T(tpos), tld, dT0, tld, queues[0]);
                            }
                        }
                    }
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        if (flip == 0) {
                            ...
                            for (magma_int_t i=0; i < NE; i += sz_bl) {
                                ...
                                //magma_slarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV0+lcvpos, lddv, dT0+lctpos, lddt, dE(myrow,i), ldde, dwork0, lddw, queues[0] );
                                magma_slarfb_gpu_gemm( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV0+lcvpos, lddv, dT0+lctpos, lddt, dE(myrow,i), ldde, dwork0, lddw, dwvt0, lddv, queues[0] );
                            }
                            ...
                            for (magma_int_t i=0; i < NE; i += sz_bl) {
                                ib = min(sz_bl, NE-i);
                                lddw = min(lddwork,sz_bl);
                                //magma_slarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV1+lcvpos, lddv, dT1+lctpos, lddt, dE(myrow,i), ldde, dwork1, lddw, queues[1] );
                                magma_slarfb_gpu_gemm( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV1+lcvpos, lddv, dT1+lctpos, lddt, dE(myrow,i), ldde, dwork1, lddw, dwvt1, lddv, queues[1] );
                            }
            ...
            for (blki = mt; blki > 0; blki--) {
                ...
                for (blkj = nt-1; blkj >= 0; blkj--) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_ssetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, queues[0]);
                        magma_ssetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, queues[0]);
                        //printf("voici blki %d  rownbm %d mycol %d  coled %d  blkid %d vpos %d  tpos %d\n", blki, rownbm, mycol, coled, blkid, vpos, tpos);
                        for (magma_int_t i=0; i < NE; i += sz_bl) {
                            ib = min(sz_bl, NE-i);
                            magma_slarfb_gpu( MagmaLeft, MagmaNoTrans, MagmaForward, MagmaColumnwise, Vm, ib, Vn, dV0, lddv, dT0, lddt, dE(myrow,i), ldde, dwork, NE, queues[0] );
                        }
                    } // end for (Vm &Vn) > 0
                } // end for blkj
            } // end for blki
            ...
            for (blkj=0; blkj < nt; blkj++) {
                ...
                for (blki=1; blki <= mt; blki++) {
                    ...
                    if ((Vm > 0) && (Vn > 0)) {
                        ...
                        magma_ssetmatrix_async(Vm, Vn, V(vpos), ldv, dV0, lddv, queues[0]);
                        magma_ssetmatrix_async(Vn,  Vn, T(tpos), ldt, dT0, lddt, queues[0]);
                        magma_slarfb_gpu( MagmaRight, MagmaNoTrans, MagmaForward, MagmaColumnwise, NE, Vm, Vn, dV0, lddv, dT0, lddt, dE(0, myrow), ldde, dwork, NE, queues[0] );
                    } // end for (Vm &Vn) > 0
                } // end for blki
            } // end fo blkj
        } // end of version 91

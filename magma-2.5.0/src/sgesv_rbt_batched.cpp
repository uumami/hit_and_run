/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgesv_rbt_batched.cpp, normal z -> s, Sun Sep 15 17:15:18 2019
*/
                  ...
                  magma_int_t batchCount, magma_queue_t queue)
{
    ...
    info = magma_sgerbt_batched(MagmaTrue, n, nrhs, dA_array, n, dB_array, n, hu, hv, &info, batchCount, queue);
    if (info != MAGMA_SUCCESS)  {
        return info;
    }


    info = magma_sgetrf_nopiv_batched( n, n, dA_array, ldda, dinfo_array, batchCount, queue);
    if ( info != MAGMA_SUCCESS ) {
        return info;
    }
    ...
    info = magma_sgetrs_nopiv_batched( MagmaNoTrans, n, nrhs, dA_array, ldda, dB_array, lddb, dinfo_array, batchCount, queue );
    ...
    magma_ssetvector( 2*n, hv, 1, dv, 1, queue );

    for (i = 0; i < nrhs; i++)
        magmablas_sprbt_mv_batched(n, dv, dB_array+(i), batchCount, queue);

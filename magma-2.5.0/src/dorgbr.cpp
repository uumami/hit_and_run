/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zungbr.cpp, normal z -> d, Sun Sep 15 17:15:08 2019
...
/***************************************************************************//**
    Purpose
    -------
    DORGBR generates one of the real orthogonal matrices Q or P**H
    ...
    is of order N:
    if k < n, P**H = G(k) . . . G(2) G(1) and DORGBR returns the first m
    rows of P**H, where n >= m >= k;
    if k >= n, P**H = G(n-1) . . . G(2) G(1) and DORGBR returns P**H as
    an N-by-N matrix.
    ...
    @param[in,out]
    A       double array, dimension (LDA,N)
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        if (wantq) {
            if (m >= k) {
                ...
                lapackf77_dorgqr( &m, &n, &k, A, &lda, tau, work, &ineg_one, &iinfo );
            }
            else if (m > 1) {
                //magma_dorgqr2( m-1, m-1, m-1, A(1,1), lda, tau, work, -1, &iinfo );
                magma_int_t m1 = m-1;
                lapackf77_dorgqr( &m1, &m1, &m1, A(1,1), &lda, tau, work, &ineg_one, &iinfo );
            }
            lwkopt = MAGMA_D_REAL( work[0] );
            lwkmin = min_mn;
        }
        else {
            if (k < n) {
                magma_dorglq( m, n, k, A, lda, tau, work, -1, &iinfo );
            }
            else if (n > 1) {
                magma_dorglq( n-1, n-1, n-1, A(1,1), lda, tau, work, -1, &iinfo );
            }
            lwkopt = MAGMA_D_REAL( work[0] );
            lwkmin = lwkopt;
        }
    ...
    if (wantq) {
        ...
        if (m >= k) {
            // If m >= k, assume m >= n >= k
            magma_dorgqr2( m, n, k, A, lda, tau, /*work, lwork,*/ &iinfo );
        }
        else {
            // If m < k, assume m = n
            ...
            *A(0,0) = c_one;
            ...
            if (m > 1) {
                // Form Q(2:m,2:m)
                magma_dorgqr2( m-1, m-1, m-1, A(1,1), lda, tau, /*work, lwork,*/ &iinfo );
            }
        }
    }
    else {
        ...
        if (k < n) {
            // If k < n, assume k <= m <= n
            magma_dorglq( m, n, k, A, lda, tau, work, lwork, &iinfo );
        }
        else {
            // If k >= n, assume m = n
            ...
            *A(0,0) = c_one;
            ...
            if (n > 1) {
                // Form P**H(2:n,2:n)
                magma_dorglq( n-1, n-1, n-1, A(1,1), lda, tau, work, lwork, &iinfo );
            }
        }
    }

/*
   ...
   @generated from src/zgetrf_batched.cpp, normal z -> c, Sun Sep 15 17:15:13 2019
*/
...
#define ipiv_array(i_)    ipiv_array, i_
    ...
    if( m == n && m <= 32 ){
        ...
        if(arch >= 700){
            return magma_cgetrf_batched_smallsq_noshfl( m, dA_array, ldda, ipiv_array, info_array, batchCount, queue );
        }
        else{
            return magma_cgetrf_batched_smallsq_shfl( m, dA_array, ldda, ipiv_array, info_array, batchCount, queue );
        }
    }
    ...
    for (i = 0; i < min_mn; i += nb) 
    {
        ...
        pm = m-i;
        // panel
        arginfo = magma_cgetrf_recpanel_batched(
                    pm, ib, recnb, 
                    dAarray(i, i), ldda, 

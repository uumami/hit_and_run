/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zheevdx_2stage.cpp, normal z -> s, Sun Sep 15 17:14:39 2019
...
*/
    ...
    magma_ssytrd_sy2sb(uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dT1, info);
    ...
    if (! wantz) {
        ...
        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time sstedx = %6.2f\n", (long long) n, (long long) nb, time );
        magma_free( dwedc );
        magma_smove_eig(range, n, W, &il, &iu, vl, vu, m);
        ...
        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time sbulge_back = %6.2f\n", (long long) n, (long long) nb, time );
        ...
        magma_sormqr_2stage_gpu( MagmaLeft, MagmaNoTrans, n-nb, *m, n-nb, dA+nb, ldda,
                                 dZ+nb, n, dT1, nb, info );

        magma_sgetmatrix( n, *m, dZ, lddz, A, lda, queue );

        magma_queue_sync( queue );
        magma_queue_destroy( queue );

        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time sormqr + copy = %6.2f\n", (long long) n, (long long) nb, time );
        magma_free(dZ);
        magma_free(dA);
        timer_stop( time_total );

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zlaqps_gpu.cpp, normal z -> d, Sun Sep 15 17:13:48 2019
...
/***************************************************************************//**
    ...
    @param[in,out]
    dA      DOUBLE PRECISION array, dimension (LDDA,N), on the GPU.
    ...
    @param[in,out]
    dauxv   DOUBLE PRECISION array, dimension (NB), on the GPU
            Auxiliary vector.

    @param[in,out]
    dF      DOUBLE PRECISION array, dimension (LDDF,NB), on the GPU
            Matrix F' = L*Y'*A.
...
#define  dF(i, j) (dF  + (i) + (j)*(lddf))
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        if (pvt != k) {
            ...
            magmablas_dswap( i__1, dF(pvt, 0), lddf, dF(k, 0), lddf, queue );
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
        ...
        magma_dlarfg_gpu( m-rk, dA(rk, k), dA(rk + 1, k), &tau[k], &vn1[k], &dAks[k], queue );

        /* needed to avoid the race condition */
        if (k == 0) magma_dsetvector(  1,    &c_one,        1, dA(rk, k), 1, queue );
        else        magma_dcopymatrix( 1, 1, dA(offset, 0), 1, dA(rk, k), 1, queue );

        /* Compute Kth column of F:
           Compute  F(K+1:N,K) := tau(K)*A(RK:M,K+1:N)'*A(RK:M,K) on the GPU */
        if (k < n-1 || k > 0) magma_dgetvector( 1, &tau[k], 1, &tauk, 1, queue );
        if (k < n-1) {
            i__1 = m - rk;
            i__2 = n - k - 1;
        ...
        if (rk < min(m, n+offset)-1 ) {
            magmablas_dnrm2_row_check_adjust( n-k-1, tol3z, &vn1[k+1], &vn2[k+1], 
    ...
    magma_dcopymatrix( 1, k, dAks, 1, dA(offset, 0), ldda+1, queue );

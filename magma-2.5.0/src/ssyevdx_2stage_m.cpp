/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zheevdx_2stage_m.cpp, normal z -> s, Sun Sep 15 17:14:42 2019
...
/***************************************************************************//**
    Purpose
    -------
    SSYEVD_2STAGE_M computes all eigenvalues and, optionally, eigenvectors of a
    ...
    lwork   INTEGER
     ...
     -      If JOBZ = MagmaVec   and N > 1, LWORK >= LWSTG2 + 2*N + N**2.
            ...
            and is returned by magma_sbulge_getlwstg2.
    \n
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    #ifdef COMPLEX
    anrm = lapackf77_slansy("M", uplo_, &n, A, &lda, rwork);
    #else
...
*/
    ...
    float *TAU1  = work + sizE_onwork;
    float *TAU2  = TAU1 + sizTAU1;
    float *V2    = TAU2 + sizTAU2;
    float *T2    = V2   + sizV2;
    float *Wstg1 = T2   + sizT2;
    float *A2    = Wstg1; /* PAY ATTENTION THAT work[indA2] should be able to be of size lda2*n which it should be checked in any future modification of lwork.*/
    float *Z     = Wstg1;
    float *Wmqr1 = TAU2; // reuse TAU2 as work of unmqr for stage 1 since no need anymor efor TAU2 and V2 and T2 but pay attentionthat does not overlap with Z=Wstg1 so lwmqr1
...
#ifdef SINGLEGPU
    float *dT1sgpu;
    if (MAGMA_SUCCESS != magma_smalloc( &dT1sgpu, n*nb)) {
        *info = MAGMA_ERR_DEVICE_ALLOC;
        return *info;
    }
    magma_ssytrd_sy2sb(uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dT1sgpu, info);

    timer_stop( time );
    timer_printf( "  N= %10lld  nb= %5lld time ssytrd_sy2sb= %6.2f\n", (long long) n, (long long) nb, time );
    timer_start( time );
#else
    ...
    magma_ssetmatrix_1D_col_bcyclic( ngpu, n, n, distblk, A, lda, dA, ldda, distqueues );
    for( magma_int_t dev=0; dev < ngpu; dev++ ) {
        magma_setdevice( dev );
    ...
    magma_ssytrd_sy2sb_mgpu( uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dA, ldda,
                             dT1, nb, ngpu, distblk, queues, nstream, info );
    timer_stop( time_band );
    timer_printf("    time alloc %7.4f, ditribution %7.4f, ssytrd_sy2sb_m only = %7.4f\n", time_alloc, time_dist, time_band );
    ...
    timer_stop( time );
    timer_printf( "  N= %10lld  nb= %5lld time ssytrd_convert = %6.2f\n", (long long) n, (long long) nb, time );
    timer_start( time );

    magma_ssytrd_sb2st(uplo, n, nb, Vblksiz, A2, lda2, W, E, V2, ldv, TAU2, wantz, T2, ldt);

    timer_stop( time );
    timer_stop( time_total );
    timer_printf( "  N= %10lld  nb= %5lld time ssytrd_sb2st= %6.2f\n", (long long) n, (long long) nb, time );
    timer_printf( "  N= %10lld  nb= %5lld time ssytrd= %6.2f\n", (long long) n, (long long) nb, time_total );
    ...
    if (! wantz) {
        ...
        magma_sstedx(range, n, vl, vu, il, iu, W, E,
                     Z, ldz, Wedc, lwedc,
                     iwork, liwork, dwedc, info);
        magma_free( dwedc );
#else
        magma_sstedx_m(ngpu, range, n, vl, vu, il, iu, W, E,
                     Z, ldz, Wedc, lwedc,
                     iwork, liwork, info);
#endif

        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time sstedx_m = %6.2f\n", (long long) n, (long long) nb, time );
        ...
        magma_sbulge_back(uplo, n, nb, *m, Vblksiz, Z +ldz*(il-1), ldz, dZ, lddz,
                          V2, ldv, TAU2, T2, ldt, info);
#else
        magma_sbulge_back_m(ngpu, uplo, n, nb, *m, Vblksiz, Z +ldz*(il-1), ldz,
                          V2, ldv, TAU2, T2, ldt, info);
#endif // not SINGLEGPU


        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time sbulge_back_m = %6.2f\n", (long long) n, (long long) nb, time );
        timer_start( time );

#ifdef SINGLEGPU
        magma_sormqr( MagmaLeft, MagmaNoTrans, n-nb, *m, n-nb, A+nb, lda, TAU1,
                       Z +ldz*(il-1)+nb, ldz, Wmqr1, lwmqr1, info);
#else
        magma_sormqr_m(ngpu, MagmaLeft, MagmaNoTrans, n-nb, *m, n-nb, A+nb, lda, TAU1,
                       Z +ldz*(il-1)+nb, ldz, Wmqr1, lwmqr1, info);

        lapackf77_slacpy("A", &n, m, Z +ldz*(il-1), &ldz, A, &lda);
#endif // not SINGLEGPU

        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time sormqr_m + copy = %6.2f\n", (long long) n, (long long) nb, time );
#ifdef SINGLEGPU
        magma_free(dT1sgpu);
        magma_free(dZ);
#endif // not SINGLEGPU

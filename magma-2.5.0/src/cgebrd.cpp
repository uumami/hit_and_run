/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgebrd.cpp, normal z -> c, Sun Sep 15 17:15:06 2019
...
/***************************************************************************//**
    ...
    @param[out]
    tauq    COMPLEX array dimension (min(M,N))
            The scalar factors of the elementary reflectors which
            represent the orthogonal matrix Q. See Further Details.

    @param[out]
    taup    COMPLEX array, dimension (min(M,N))
            The scalar factors of the elementary reflectors which
...
#define dA(i, j) (dA + (j)*ldda + (i))
    ...
    *info = 0;
    ...
    if (MAGMA_SUCCESS != magma_cmalloc_cpu( &work2, lwork2 )) {
    ...
    for (i=0; i < (minmn - nx); i += nb) {
        ...
        if (m >= n) {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {
                *A(j, j  ) = MAGMA_C_MAKE( d[j], 0. );
                *A(j, j+1) = MAGMA_C_MAKE( e[j], 0. );
            }
        } else {
            jmax = i + nb;
            for (j = i; j < jmax; ++j) {

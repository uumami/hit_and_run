/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgeqlf.cpp, normal z -> c, Sun Sep 15 17:13:51 2019
...
/***************************************************************************//**
    ...
    lwork   INTEGER
            ...
            through magma_get_cgeqlf_nb( M, N ).
    \n
    ...
    if ( (nb > 1) && (nb < minmn) ) {
        ...
        for (i = minmn - kk + ki; i >= minmn - kk; i -= nb) {
            ...
            if (cols > 0) {
                ...
                magma_cpanel_to_q( MagmaLower, ib, A(rows-ib,cols), lda, work+ib*ib );
                magma_csetmatrix( rows, ib,
                                  A(0,cols),  lda,
                                  dA(0,cols), ldda, queues[1] );
                magma_cq_to_panel( MagmaLower, ib, A(rows-ib,cols), lda, work+ib*ib );

                // wait for main update (above) to finish with dwork
                magma_queue_sync( queues[0] );
                
                // Send the triangular part to the GPU
                magma_csetmatrix( ib, ib, work, ib, dwork(0), lddwork, queues[1] );
        ...
        magma_cgetmatrix( m, nu, dA(0,0), ldda, A(0,0), lda, queues[1] );
    } else {
        mu = m;
        nu = n;
    }

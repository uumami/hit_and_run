/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zhetrd.cpp, normal z -> c, Sun Sep 15 17:14:22 2019
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    #endif
    if (MAGMA_SUCCESS != magma_cmalloc( &dA, ldda*n + 2*lddw*nb + ldwork2 )) {
        magma_free_cpu( work2 );
        *info = MAGMA_ERR_DEVICE_ALLOC;
        return *info;
    }

    magmaFloatComplex *dwork  = dA + ldda*n;
    #ifdef FAST_HEMV
    magmaFloatComplex *dwork2 = dwork + 2*lddw*nb;
    #endif
    ...
    if (upper) {
        ...
        magma_csetmatrix( n, n, A(0, 0), lda, dA(0, 0), ldda, queue );
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            ...
            #ifdef FAST_HEMV
            magma_clatrd2( uplo, i+nb, nb, A(0, 0), lda, e, tau,
        ...
        for (i = 0; i < n-nx; i += nb) {
            ...
            #ifdef FAST_HEMV
            magma_clatrd2( uplo, n-i, nb, A(i, i), lda, &e[i], &tau[i],

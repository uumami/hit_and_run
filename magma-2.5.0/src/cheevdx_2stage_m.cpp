/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zheevdx_2stage_m.cpp, normal z -> c, Sun Sep 15 17:14:41 2019
...
*/
    ...
    magma_csetmatrix_1D_col_bcyclic( ngpu, n, n, distblk, A, lda, dA, ldda, distqueues );
    for( magma_int_t dev=0; dev < ngpu; dev++ ) {
        magma_setdevice( dev );
    ...
    if (! wantz) {
        ...
        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time cstedx_m = %6.2f\n", (long long) n, (long long) nb, time );
        ...
        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time cbulge_back_m = %6.2f\n", (long long) n, (long long) nb, time );
        timer_start( time );
        ...
        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time cunmqr_m + copy = %6.2f\n", (long long) n, (long long) nb, time );
#ifdef SINGLEGPU
        magma_free(dT1sgpu);
        magma_free(dZ);
#endif // not SINGLEGPU

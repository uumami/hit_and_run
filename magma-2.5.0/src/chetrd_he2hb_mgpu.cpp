/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zhetrd_he2hb_mgpu.cpp, normal z -> c, Sun Sep 15 17:14:44 2019
...
/***************************************************************************//**
    ...
    @param[in,out]
    dAmgpu  COMPLEX array of pointer, dimension (ngpu)
            Each point to a COMPLEX array, dimension (LDDA, nlocal)
    ...
    @param[in,out]
    dTmgpu  COMPLEX array of pointer, dimension (ngpu)
            Each point to a COMPLEX array on the GPU, dimension n*nb,
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (i = 1; i <= n-nb; i += nb) {
            ...
            if (i > 1 ) {
                // magma_cpanel_to_q copy the upper oof diagonal part of
                ...
                magma_cpanel_to_q(MagmaUpper, pn-1, A(i, i+1), lda, work);
                ...
                //magma_setdevice( 0 );
                magma_cq_to_panel(MagmaUpper, pn-1, A(i, i+1), lda, work);
            }
            ...
            magma_cpanel_to_q(MagmaUpper, pk, A(indi, indj), lda, work);
            ...
            magma_cq_to_panel(MagmaUpper, pk, A(indi, indj), lda, work);
            // ===============================================
            ...
            if (i + nb <= n-nb) {
                ...
                magma_cpanel_to_q(MagmaUpper, pk-1, A(n-pk+1, n-pk+2), lda, work);
                magma_cgetmatrix( pk, pk,
                                  dA(idev, indi, di+1), ldda,
                                  A(n-pk+1, n-pk+1),  lda, queues[ idev ][ 0 ] );
                magma_cq_to_panel(MagmaUpper, pk-1, A(n-pk+1, n-pk+2), lda, work);
            }

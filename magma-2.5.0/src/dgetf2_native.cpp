/*
   ...
   @generated from src/zgetf2_native.cpp, normal z -> d, Sun Sep 15 17:15:31 2019
*/
    ...
    magma_int_t gbstep, magma_queue_t queue)
{
    ...
    for( j=0; j < min_mn; j += nb){
        ...
        for (step=0; step < ib; step++) {
            ...
            arginfo = magma_idamax_native( m-gbj, dA, 1, gbj, ldda, dipiv, dinfo, gbstep, queue);
            ...
            if (gbj < m) {
                arginfo = magma_dscal_dger_native( m-gbj, ib-step, gbj, dA, ldda, dinfo, gbstep, queue );
                if (arginfo != 0 ) return arginfo;
            }
        }
    ...
    magma_queue_t queue, magma_queue_t update_queue)
{
    ...
    if( n <= nb){
        ...
        magma_dgetf2_native_fused( m, n, dA(0,0), ldda, dipiv, gbstep, update_flags, dinfo, queue );
        ...
        // lu on A1
        magma_dgetf2_native_recursive(m, n1, dA(0,0), ldda, dipiv, dipivinfo, dinfo, gbstep, queue, update_queue);

        // swap left
        #ifdef PARSWAP
        setup_pivinfo( dipivinfo, dipiv, m, n1, queue);  // setup pivinfo
        magma_dlaswp_rowparallel_native( n2, dA(0,n1), ldda, dA(0,n1), ldda, 0, n1, dipivinfo, queue);
        #else
        ...
        // lu on A2 
        magma_dgetf2_native_recursive(m-n1, n2, dA(n1,n1), ldda, dipiv+n1, dipivinfo, dinfo, gbstep, queue, update_queue );
        ...
        #ifdef PARSWAP
        magma_dlaswp_rowparallel_native(n1, dA(n1,0), ldda, dA(n1,0), ldda, n1, n, dipivinfo, queue);
        #else
        magma_dlaswp_rowserial_native(n1, dA(0,0), ldda, n1, n, dipiv, queue);
        #endif
    }
    ...
    magma_queue_t queue, magma_queue_t update_queue)
{
    ...
    if(m > DGETF2_FUSED_MAX_M || arch < 300){
      ...
      magma_dgetf2_native_recursive(m, n, dA, ldda, dipiv, dipivinfo, dinfo, gbstep, queue, update_queue);
    }    
    return 0;
}

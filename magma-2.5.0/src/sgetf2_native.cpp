/*
   ...
   @generated from src/zgetf2_native.cpp, normal z -> s, Sun Sep 15 17:15:31 2019
*/
    ...
    magma_int_t gbstep, magma_queue_t queue)
{
    ...
    for( j=0; j < min_mn; j += nb){
        ...
        for (step=0; step < ib; step++) {
            ...
            arginfo = magma_isamax_native( m-gbj, dA, 1, gbj, ldda, dipiv, dinfo, gbstep, queue);

            if (arginfo != 0 ) return arginfo;
            // Apply the interchange to columns 1:N. swap the whole row
            magma_sswap_native(n, dA, ldda, gbj, dipiv, queue);
            if (arginfo != 0 ) return arginfo;
            // Compute elements J+1:M of J-th column.
            if (gbj < m) {
                arginfo = magma_sscal_sger_native( m-gbj, ib-step, gbj, dA, ldda, dinfo, gbstep, queue );
                if (arginfo != 0 ) return arginfo;
            }
        }

        if ( (n-j-ib) > 0) {
            magma_sgetf2trsm_2d_native( ib, n-j-ib, 
                                        dA(j,j   ), ldda, 
    ...
    magma_queue_t queue, magma_queue_t update_queue)
{
    ...
    if( n <= nb){
        ...
        magma_sgetf2_native_fused( m, n, dA(0,0), ldda, dipiv, gbstep, update_flags, dinfo, queue );
        ...
        // lu on A1
        magma_sgetf2_native_recursive(m, n1, dA(0,0), ldda, dipiv, dipivinfo, dinfo, gbstep, queue, update_queue);

        // swap left
        #ifdef PARSWAP
        setup_pivinfo( dipivinfo, dipiv, m, n1, queue);  // setup pivinfo
        magma_slaswp_rowparallel_native( n2, dA(0,n1), ldda, dA(0,n1), ldda, 0, n1, dipivinfo, queue);
        #else
        magma_slaswp_rowserial_native(n2, dA(0,n1), ldda, 0, n1, dipiv, queue);
        #endif

        // update (trsm + gemm)
        magma_sgetf2trsm_2d_native( n1, n2, 
                                    dA(0,0), ldda, 
                                    dA(0,n1), ldda, queue);

        magma_sgemm( MagmaNoTrans, MagmaNoTrans, 
                     m-n1, n2, n1, 
                     MAGMA_S_NEG_ONE, dA(n1,  0), ldda, 
                                      dA(0 , n1), ldda, 
                     MAGMA_S_ONE,     dA(n1, n1), ldda, queue );

        // lu on A2 
        magma_sgetf2_native_recursive(m-n1, n2, dA(n1,n1), ldda, dipiv+n1, dipivinfo, dinfo, gbstep, queue, update_queue );
        ...
        #ifdef PARSWAP
        magma_slaswp_rowparallel_native(n1, dA(n1,0), ldda, dA(n1,0), ldda, n1, n, dipivinfo, queue);
        #else
        magma_slaswp_rowserial_native(n1, dA(0,0), ldda, n1, n, dipiv, queue);
        #endif
    }
...
/***************************************************************************//**
    ...
    @param[in,out]
    dA      A REAL array on the GPU, dimension (LDDA,N).
    ...
    magma_queue_t queue, magma_queue_t update_queue)
{
    ...
    if(m > SGETF2_FUSED_MAX_M || arch < 300){
      magma_sgetf2_native_blocked(m, n, dA, ldda, dipiv, dinfo, gbstep, queue);
    }
    else{
      magma_sgetf2_native_recursive(m, n, dA, ldda, dipiv, dipivinfo, dinfo, gbstep, queue, update_queue);
    }    
    return 0;
}

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zunmrq.cpp, normal z -> c, Sun Sep 15 17:14:01 2019
*/
...
/***************************************************************************//**
    ...
    as returned by CGERQF. Q is of order M if SIDE = MagmaLeft and of order N
    if SIDE = MagmaRight.
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nb >= k) {
        /* Use CPU code */
        lapackf77_cunmrq( lapack_side_const(side), lapack_trans_const(trans),
        ...
        magma_csetmatrix( m, n, C, ldc, dC(0,0), lddc, queue );
        ...
        for (int i = i1; (step < 0 ? i >= i2 : i < i2); i += step) {
            ...
            nq_i = nq - k + i + ib;
            lapackf77_clarft("Backward", "Rowwise", &nq_i, &ib, A(i,0), &lda, &tau[i], T, &ib);

            /* 1) set upper triangle of panel in A to identity,
               2) copy the panel from A to the GPU, and
               3) restore A                                      */
            magma_cpanel_to_q( MagmaUpper, ib, A(i,nq_i-ib), lda, T2 );
            magma_csetmatrix( ib, nq_i, A(i,0), lda, dV(0,0), ib, queue );
            magma_cq_to_panel( MagmaUpper, ib, A(i,nq_i-ib), lda, T2 );
            ...
            /* Apply H or H' */
            magma_csetmatrix( ib, ib, T, ib, dT(0,0), ib, queue );
            magma_clarfb_gpu(side, transt, MagmaBackward, MagmaRowwise,
                             mi, ni, ib,
                             dV(0,0), ib,
                             dT(0,0), ib,
                             dC, lddc, dwork, ldwork, queue);
        }
        magma_cgetmatrix( m, n, dC(0,0), lddc, C, ldc, queue );

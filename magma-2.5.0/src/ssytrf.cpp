/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zhetrf.cpp, normal z -> s, Sun Sep 15 17:14:07 2019
*/
    ...
    magma_int_t *ipiv, magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if ( upper ) {
        for (k = 0; k < n; k += nb ) {
            kb = min(nb, n-k);
            magma_ssetmatrix_async( k+kb, kb, A(0,k), lda, dA(0,k), ldda, queues[0] );
        }
    } else {
        for (k = 0; k < n; k += nb ) {
            kb = min(nb, n-k);
            magma_ssetmatrix_async( n-k, kb, A(k,k), lda, dA(k,k), ldda, queues[0] );
        }
    }
    trace_gpu_end( 0, 0 );

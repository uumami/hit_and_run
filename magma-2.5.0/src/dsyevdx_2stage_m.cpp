...
/***************************************************************************//**
    Purpose
    -------
    DSYEVD_2STAGE_M computes all eigenvalues and, optionally, eigenvectors of a
...
*/
    ...
    double *A2    = Wstg1; /* PAY ATTENTION THAT work[indA2] should be able to be of size lda2*n which it should be checked in any future modification of lwork.*/
    double *Z     = Wstg1;
    double *Wmqr1 = TAU2; // reuse TAU2 as work of unmqr for stage 1 since no need anymor efor TAU2 and V2 and T2 but pay attentionthat does not overlap with Z=Wstg1 so lwmqr1
...
#ifdef SINGLEGPU
    double *dT1sgpu;
    if (MAGMA_SUCCESS != magma_dmalloc( &dT1sgpu, n*nb)) {
        *info = MAGMA_ERR_DEVICE_ALLOC;
        return *info;
    }
    magma_dsytrd_sy2sb(uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dT1sgpu, info);
    ...
    magma_dsetmatrix_1D_col_bcyclic( ngpu, n, n, distblk, A, lda, dA, ldda, distqueues );
    for( magma_int_t dev=0; dev < ngpu; dev++ ) {
        magma_setdevice( dev );
    ...
    magma_dsytrd_sy2sb_mgpu( uplo, n, nb, A, lda, TAU1, Wstg1, lwstg1, dA, ldda,
                             dT1, nb, ngpu, distblk, queues, nstream, info );
    timer_stop( time_band );
    timer_printf("    time alloc %7.4f, ditribution %7.4f, dsytrd_sy2sb_m only = %7.4f\n", time_alloc, time_dist, time_band );
    ...
    if (! wantz) {
        ...
        magma_free( dwedc );
#else
        magma_dstedx_m(ngpu, range, n, vl, vu, il, iu, W, E,
                     Z, ldz, Wedc, lwedc,
                     iwork, liwork, info);
#endif

        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time dstedx_m = %6.2f\n", (long long) n, (long long) nb, time );
...
#else
        magma_dbulge_back_m(ngpu, uplo, n, nb, *m, Vblksiz, Z +ldz*(il-1), ldz,
                          V2, ldv, TAU2, T2, ldt, info);
#endif // not SINGLEGPU


        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time dbulge_back_m = %6.2f\n", (long long) n, (long long) nb, time );
        timer_start( time );

#ifdef SINGLEGPU
        magma_dormqr( MagmaLeft, MagmaNoTrans, n-nb, *m, n-nb, A+nb, lda, TAU1,
                       Z +ldz*(il-1)+nb, ldz, Wmqr1, lwmqr1, info);
#else
        magma_dormqr_m(ngpu, MagmaLeft, MagmaNoTrans, n-nb, *m, n-nb, A+nb, lda, TAU1,
                       Z +ldz*(il-1)+nb, ldz, Wmqr1, lwmqr1, info);

        lapackf77_dlacpy("A", &n, m, Z +ldz*(il-1), &ldz, A, &lda);
#endif // not SINGLEGPU

        timer_stop( time );
        timer_printf( "  N= %10lld  nb= %5lld time dormqr_m + copy = %6.2f\n", (long long) n, (long long) nb, time );
#ifdef SINGLEGPU
        magma_free(dT1sgpu);
        magma_free(dZ);
#endif // not SINGLEGPU

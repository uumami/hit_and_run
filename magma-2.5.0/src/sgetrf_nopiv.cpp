/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgetrf_nopiv.cpp, normal z -> s, Sun Sep 15 17:13:28 2019
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min_mn) {
        /* Use unblocked code. */
        magma_sgetf2_nopiv( m, n, A(1,1), lda, info );
    }
    else {
        /* Use blocked code. */
        for (j = 1; j <= min_mn; j += nb) {
            ...
            /* Factor diagonal and subdiagonal blocks and test for exact
               singularity. */
            m_j_jb = m - j - jb + 1;
            magma_sgetf2_nopiv( jb, jb, A(j,j), lda, &iinfo );
            blasf77_strsm( "R", "U", "N", "N", &m_j_jb, &jb, &c_one,
                           A(j,j),    &lda,
                           A(j+jb,j), &lda );
            ...
            if (j + jb <= n) {
                ...
                n_j_jb = n - j - jb + 1;
                blasf77_strsm( "Left", "Lower", "No transpose", "Unit",
                               &jb, &n_j_jb, &c_one,
                               A(j,j),    &lda,
                               A(j,j+jb), &lda );
                if (j + jb <= m) {
                    ...
                    m_j_jb = m - j - jb + 1;
                    n_j_jb = n - j - jb + 1;
                    blasf77_sgemm( "No transpose", "No transpose",

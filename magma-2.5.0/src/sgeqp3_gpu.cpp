/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgeqp3_gpu.cpp, normal z -> s, Sun Sep 15 17:13:48 2019
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    magmablas_slaset( MagmaFull, n+1, nb, c_zero, c_zero, df, n+1, queue );
    ...
    for (j = 0; j < n; ++j) {
        if (jpvt[j] != 0) {
            if (j != nfxd) {
                blasf77_sswap(&m, dA(0, j), &ione, dA(0, nfxd), &ione);  // TODO: ERROR, matrix not on CPU!
                jpvt[j]    = jpvt[nfxd];
                jpvt[nfxd] = j + 1;
            }
            else {
    ...
    if (nfxd > 0) {
        na = min(m,nfxd);
        lapackf77_sgeqrf(&m, &na, dA, &ldda, tau, dwork, &lwork, info);
        if (na < n) {
            n_j = n - na;
    ...
    if (nfxd < minmn) {
        ...
        magmablas_snrm2_cols( sm, sn, dA(nfxd,nfxd), ldda, &rwork[nfxd], queue );
        ...
        if (j < minmn) {
            ...
            lapackf77_slaqp2(&m, &n_j, &j, dA(0, j), &ldda, &jpvt[j],

...
/******************************************************************************/
static void magma_dsygst_m_1_U_row_update(
    ...
    magma_queue_t queue )
{
    ...
    magma_dgemm( MagmaConjTrans, MagmaNoTrans, kb, nk-kb, nb,
                 c_neg_one, dC1,              lddc1,
                            dC2 + kb*lddc2,   lddc2,
...
/******************************************************************************/
static void magma_dsygst_m_1_L_col_update(
    magma_int_t nk, magma_int_t nb,
    ...
    magma_queue_t queue )
{
    ...
    magma_dgemm( MagmaNoTrans, MagmaConjTrans, nk-kb, kb, nb,
    ...
    magma_int_t *info)
{
    ...
    magmaDouble_ptr dw[MagmaMaxGPUs];
    ...
    magma_int_t nb = magma_get_dsygst_m_nb( n );
        ...
        if (upper) {
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k+1 < nblock) {
                    ...
                    magma_dsymm( MagmaLeft, uplo, kb, n-(k+1)*nb,
                ...
                for (magma_int_t j = 1; j < nblock; ++j) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        if (nloc[igpu] > 0) {
                            ...
                            if ( j < nblock-1 ) {
                                magma_dgemm( MagmaNoTrans, MagmaNoTrans, nloc[igpu], n-(j+1)*nb, nb,
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j=k+1; j < nblock; ++j) {
                        ...
                        if (igpu != igpu_p) {
                            magma_setdevice( igpu );
                            // dsyr2k on j-th column
                            magma_dsygst_m_1_L_col_update(
                                n-j*nb, nb,
                ...
                if (k+1 < nblock) {
                    ...
                    magma_dsymm( MagmaRight, uplo, n-(k+1)*nb, kb,
                ...
                for (magma_int_t j = 1; j < nblock; ++j) {
                    ...
                    for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                        ...
                        if (nloc[igpu] > 0) {
                            ...
                            if ( j < nblock-1 ) {
                                magma_dgemm( MagmaNoTrans, MagmaNoTrans, n-(j+1)*nb, nloc[igpu], nb,
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j = 0; j < k; ++j) {
                        ...
                        magma_dsyr2k( uplo, MagmaNoTrans, nb, kb,
                                      c_one, dwork(igpu, j, 0),    n,
                                             dB_c(igpu, j, ind_k), lddbc,
                                      d_one, dA(igpu, j, j/ngpu),  ldda, queues[igpu][1] );

                        magma_dgemm( MagmaNoTrans, MagmaConjTrans, j*nb, nb, kb,
                ...
                for (magma_int_t igpu = 0; igpu < ngpu; ++igpu) {
                    ...
                    magma_queue_sync( queues[igpu][0] ); // sync B copy (conflicts with dsygst)
                }
            ...
            for (magma_int_t k = 0; k < nblock; ++k) {
                ...
                if (k > 0) {
                    ...
                    for (magma_int_t j = 0; j < k; ++j) {
                        ...
                        magma_dsyr2k( uplo, MagmaConjTrans, nb, kb,
                                      c_one, dwork(igpu, 0, j),    nb,
                                             dB_r(igpu, ind_k, j), lddbr,
                                      d_one, dA(igpu, j/ngpu, j),  ldda, queues[igpu][1] );

                        magma_dgemm( MagmaConjTrans, MagmaNoTrans, nb, j*nb, kb,
                                     c_one, dwork(igpu, 0, j),    nb,

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zlahef_gpu.cpp, normal z -> d, Sun Sep 15 17:14:16 2019
*/
...
/***************************************************************************//**
    ...
    DLASYF computes a partial factorization of a real symmetric
    ...
    @param[out]
    dW      (workspace) DOUBLE PRECISION array, dimension (LDDW,NB)
    ...
    *info = 0;
    ...
    if ( upper ) {
        ...
        for (k = n-1; k+1 > max(n-nb+1, nb); k -= kstep) {
            ...
            magma_dgetvector_async( 1, &dW( k, kw ), 1, &Z, 1, queues[0] );
            magma_queue_sync( queues[0] );
            abs_akk = fabs( MAGMA_D_REAL( Z ) );
            ...
            if ( max( abs_akk, colmax ) == 0.0 ) {
                ...
                if ( abs_akk >= alpha*colmax ) {
                    ...
                    magma_dgetvector_async( 1, &dW( imax, kw-1 ), 1, &Zimax, 1, queues[0] );
                    ...
                    if ( abs_akk >= alpha*colmax*( colmax / rowmax ) ) {
                        /* no interchange, use 1-by-1 pivot block */
                        kp = k;
                    } else if ( fabs( MAGMA_D_REAL( Zimax ) ) >= alpha*rowmax ) {
        ...
        for (k = 0; k < min(nb-1,n); k += kstep) {
            ...
            if ( max( abs_akk, colmax ) == 0.0 ) {
                ...
                if ( abs_akk >= alpha*colmax ) {
                    ...
                    magma_dgetvector_async( 1, &dW( imax, k+1 ), 1, &Zimax, 1, queues[0] );

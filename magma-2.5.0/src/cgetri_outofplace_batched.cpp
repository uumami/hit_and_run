/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgetri_outofplace_batched.cpp, normal z -> c, Sun Sep 15 17:15:20 2019
*/
...
#define dinvAarray(i,j)    dinvA_array, i, j
    ...
    magmablas_claset_batched( MagmaFull, n, n, MAGMA_C_ZERO, MAGMA_C_ONE, dinvA_array, lddia, batchCount, queue );

    for (j = 0; j < n; j += nb) {
        ib = min(nb, n-j);
        // dinvdiagA * Piv' = I * U^-1 * L^-1 = U^-1 * L^-1 * I
        // solve lower
        magmablas_ctrsm_recursive_batched(MagmaLeft, MagmaLower, MagmaNoTrans, MagmaUnit,
                n-j, ib, MAGMA_C_ONE, 
                dAarray(j, j), ldda, 
        ...
        // solve upper
        magmablas_ctrsm_recursive_batched( MagmaLeft, MagmaUpper, MagmaNoTrans, MagmaNonUnit,
                n, ib, MAGMA_C_ONE, 
                dAarray(0, 0), ldda, 

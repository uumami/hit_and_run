/*
   ...
   @generated from src/zgetf2_batched.cpp, normal z -> d, Sun Sep 15 17:15:09 2019
*/
    ...
    for( panelj=0; panelj < min_mn; panelj += nb)
    {
        ...
        for (step=0; step < ib; step++) {
            ...
            //if ( required_shmem_size >  (MAX_SHARED_ALLOWED*1024))
            {
                ...
                arginfo = magma_idamax_batched(m-gbj, dA_array, ai, aj, 1, gbj, ldda, ipiv_array, info_array, gbstep, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                arginfo = magma_dswap_batched(n, dA_array, ai, aj, ldda, gbj, ipiv_array, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                // Compute elements J+1:M of J-th column.
                if (gbj < m) {
                    arginfo = magma_dscal_dger_batched( m-gbj, ib-step, gbj, dA_array, ai, aj, ldda, info_array, gbstep, batchCount, queue );
                    if (arginfo != 0 ) return arginfo;
                }
            }
            else {
                ...
                arginfo = magma_dcomputecolumn_batched(m-panelj, panelj, step, dA_array, ai, aj, ldda, ipiv_array, info_array, gbstep, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
                // Apply the interchange to columns 1:N. swap the whole row
                
                //printf("calling dswap batched, ai = %d, step = %d\n", ai, gbj);
                arginfo = magma_dswap_batched(n, dA_array, ai, aj, ldda, gbj, ipiv_array, batchCount, queue);
                if (arginfo != 0 ) return arginfo;
            }
        }
        ...
        if ( (n-panelj-ib) > 0) {
            ...
            magma_dgetf2trsm_batched(ib, n-panelj-ib, dA_array, ai+panelj, ldda, batchCount, queue);
            ...
            magma_dgemm_batched_core( MagmaNoTrans, MagmaNoTrans, m-(panelj+ib), n-(panelj+ib), ib,
...
#define ipiv_array(i) ipiv_array, i
    if(n <= 32){
        magma_dgetf2_fused_batched(m, n, dA_array(ai,aj), ldda, ipiv_array, info_array, batchCount, queue);
    }
    else{
        magma_int_t n1 = n / 2;
        magma_int_t n2 = n - n1;
        ...
        magma_getvector(1, sizeof(double*), dA_array, 1, &aa, 1, queue);
        magma_dprint_gpu(4, 4, aa, ldda, queue);
        }
        ...
        magma_dgemm_batched_core( 
                ...
                m-n1, n2, n1, 
                MAGMA_D_NEG_ONE, dA_array(ai+n1,    aj), ldda, 
                                 dA_array(ai   , aj+n1), ldda, 
                MAGMA_D_ONE,     dA_array(ai+n1, aj+n1), ldda, 
                batchCount, queue );
    ...
    magma_queue_t queue)
{
    ...
    if(m > DGETF2_FUSED_BATCHED_MAX_ROWS){
        magma_dgetf2_batched_v1(m, n, dA_array, ai, aj, ldda, ipiv_array, info_array, gbstep, batchCount, queue);   
    }
    else{
        magma_dgetf2_batched_v2(m, n, dA_array, ai, aj, ldda, ipiv_array, dpivinfo_array, info_array, batchCount, queue);
    }

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zhegvdx_m.cpp, normal z -> c, Sun Sep 15 17:14:51 2019
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    magma_cheevdx_m(ngpu, jobz, range, uplo, n, A, lda, vl, vu, il, iu, m, w, work, lwork, rwork, lrwork, iwork, liwork, info);
    ...
    if (wantz && *info == 0) {
        ...
        if (itype == 1 || itype == 2) {
            ...
            if (MAGMA_SUCCESS != magma_cmalloc( &dA, ldda*(*m) ) ||
                MAGMA_SUCCESS != magma_cmalloc( &dB, lddb*n )) {
                magma_free( dA );
                magma_free( dB );
            ...
            magma_csetmatrix( n, (*m), A, lda, dA, ldda, queue );
            magma_ctrmm( MagmaLeft, uplo, trans, MagmaNonUnit,
                         n, *m, c_one, dB, lddb, dA, ldda, queue );
            magma_cgetmatrix( n, (*m), dA, ldda, A, lda, queue );

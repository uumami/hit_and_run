/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgetrf_m.cpp, normal z -> s, Sun Sep 15 17:13:29 2019
    ...
    magma_int_t *info)
{
    ...
    *info = 0;
    ...
    for( I=0; I < minmn-NB; I += NB ) {
        ...
        k2 = minmn;
        incx = 1;
        lapackf77_slaswp(&NB, A(0,I), &lda, &k1, &k2, ipiv, &incx);
    }
    ...
    if ( (nb <= 1) || (nb >= min(m,n)) ) {
        ...
        for( I=0; I < n; I += NB ) {
            ...
            for( offset = 0; offset < min(m,I); offset += NB ) {
                ...
                for( d=0; d < ngpu; d++ ) {
                    magma_setdevice(d);
                    magmablas_slaswp( ldn_local, dAT(d,0,0), ldn_local, offset+1, offset+NBk, ipiv, 1, 
                                      queues[d][1] );
                }

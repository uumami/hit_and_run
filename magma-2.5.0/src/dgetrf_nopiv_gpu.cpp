/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zgetrf_nopiv_gpu.cpp, normal z -> d, Sun Sep 15 17:13:18 2019
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(m,n)) {
        ...
        for( j=0; j < s; j++ ) {
            ...
            magma_dgetmatrix_async( m-j*nb, nb, dA(j,j), ldda, work, ldwork, queues[0] );
            ...
            magma_dgetrf_nopiv( rows, nb, work, ldwork, &iinfo );
            if ( *info == 0 && iinfo > 0 )
                *info = iinfo + j*nb;

            // send j-th panel to device
            magma_dsetmatrix_async( m-j*nb, nb, work, ldwork, dA(j, j), ldda, queues[0] );
            magma_queue_sync( queues[0] );
        ...
        if ( nb0 > 0 ) {
            ...
            magma_dgetmatrix( rows, nb0, dA(s,s), ldda, work, ldwork, queues[1] );
            
            // do the cpu part
            magma_dgetrf_nopiv( rows, nb0, work, ldwork, &iinfo );
            if ( *info == 0 && iinfo > 0 )
                *info = iinfo + s*nb;
    
            // send j-th panel to device
            magma_dsetmatrix( rows, nb0, work, ldwork, dA(s,s), ldda, queues[1] );

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from src/zlaqps.cpp, normal z -> s, Sun Sep 15 17:14:02 2019
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        *A(rk, k) = c_one;
        ...
        if (k < n-1) {
            ...
            magma_ssetmatrix( i__1, 1, A(rk, k), lda, dA(rk,k), ldda, queue );
    ...
    *kb = k + 1;
    ...
    while( lsticc > 0 ) {
        ...
        if (lsticc <= nb) {
            ...
            r2 = magma_snrm2( m-offset-nb, dA(offset + nb + 1, lsticc), ione, queue );
            
            //vn1[lsticc] = magma_snrm2( i__1, dA(rk + 1, lsticc), ione, queue );

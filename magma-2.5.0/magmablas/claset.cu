/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zlaset.cu, normal z -> c, Sun Sep 15 17:17:27 2019
...
/***************************************************************************//**
    Purpose
    -------
    CLASET initializes a 2-D array A to DIAG on the diagonal and
    ...
    if (uplo == MagmaLower) {
        for( unsigned int i=0; i < super_grid.x; ++i ) {
            ...
            for( unsigned int j=0; j < super_grid.y && j <= i; ++j ) {  // from left to diagonal
                ...
                if ( i == j ) {  // diagonal super block
                    claset_lower_kernel<<< grid, threads, 0, queue->cuda_stream() >>>
                        ( mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                }
                else {           // off diagonal super block
                    claset_full_kernel<<< grid, threads, 0, queue->cuda_stream() >>>
        ...
        for( unsigned int i=0; i < super_grid.x; ++i ) {
            ...
            for( unsigned int j=i; j < super_grid.y; ++j ) {  // from diagonal to right
                ...
                grid.y = magma_ceildiv( nn, BLK_Y );
                if ( i == j ) {  // diagonal super block
                    claset_upper_kernel<<< grid, threads, 0, queue->cuda_stream() >>>
                        ( mm, nn, offdiag, diag, dA(i*super_NB, j*super_NB), ldda );
                }
                else {           // off diagonal super block
                    claset_full_kernel<<< grid, threads, 0, queue->cuda_stream() >>>
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if (uplo == MagmaLower) {
        claset_lower_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>> (m, n, offdiag, diag, dAarray, ldda);
    }
    else if (uplo == MagmaUpper) {
        claset_upper_kernel_batched<<< grid, threads, 0, queue->cuda_stream() >>> (m, n, offdiag, diag, dAarray, ldda);
    }
    else {

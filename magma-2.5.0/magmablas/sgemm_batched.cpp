/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zgemm_batched.cpp, normal z -> s, Sun Sep 15 17:20:07 2019
    ...
    if(use_cublas){
        if(zero_offset){
            cublasSgemmBatched(
                    ...
                    int(m), int(n), int(k),
                    &alpha, (const float**)dA_array, int(ldda),
                            (const float**)dB_array, int(lddb),
                    &beta,                              dC_array, int(lddc), int(batchCount) );
        }
        else{
            float** dAarray = (float**)queue->get_dAarray();
            float** dBarray = (float**)queue->get_dBarray();
            float** dCarray = (float**)queue->get_dCarray();
            magma_int_t max_batchCount   = queue->get_maxBatch();
            for(magma_int_t i = 0; i < batchCount; i+=max_batchCount){
                magma_int_t batch = min(max_batchCount, batchCount-i);
                magma_sdisplace_pointers(dAarray, (float**)dA_array + i, ldda, Ai, Aj, batch, queue);
                magma_sdisplace_pointers(dBarray, (float**)dB_array + i, lddb, Bi, Bj, batch, queue);
                magma_sdisplace_pointers(dCarray, (float**)dC_array + i, lddc, Ci, Cj, batch, queue);
                cublasSgemmBatched(
                        queue->cublas_handle(), cublas_trans_const(transA), cublas_trans_const(transB),
                        int(m), int(n), int(k),
                        &alpha, (const float**)dAarray, int(ldda),
                                (const float**)dBarray, int(lddb),
...
*******************************************************************************/
extern "C" void
magmablas_sgemm_batched( magma_trans_t transA, magma_trans_t transB, 
                     magma_int_t m, magma_int_t n, magma_int_t k,
                     float alpha,
...
/******************************************************************************/
extern "C" void
magmablas_sgemm_batched_strided( magma_trans_t transA, magma_trans_t transB, 
                     magma_int_t m, magma_int_t n, magma_int_t k,
                     float alpha,
                     ...
                     magma_int_t batchCount, magma_queue_t queue )
{
    ...
    for(magma_int_t i = 0; i < batchCount; i+=max_batchCount){
        magma_int_t batch = min(max_batchCount, batchCount-i);
        magma_sset_pointer(dAarray, (float*)(dA + i * strideA), ldda, 0, 0, strideA, batch, queue);
        magma_sset_pointer(dBarray, (float*)(dB + i * strideB), lddb, 0, 0, strideB, batch, queue);
        magma_sset_pointer(dCarray, dC + i * strideC, lddc, 0, 0, strideC, batch, queue);
        magmablas_sgemm_batched_core( 
            transA, transB,
            m, n, k, 
...
/******************************************************************************/
extern "C" void
magma_sgemm_batched( magma_trans_t transA, magma_trans_t transB, 
                     magma_int_t m, magma_int_t n, magma_int_t k,
                     float alpha,
                     float const * const * dA_array, magma_int_t ldda,
                     float const * const * dB_array, magma_int_t lddb,

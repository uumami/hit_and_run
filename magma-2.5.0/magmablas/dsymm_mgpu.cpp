/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zhemm_mgpu.cpp, normal z -> d, Sun Sep 15 17:19:21 2019
       @author Mark Gates
       @author Azzam Haidar
*/
...
/***************************************************************************//**
    ...
    \param[in]
    dA      DOUBLE PRECISION array of DIMENSION ( LDDA, ka ), where ka is
    ...
    \param[in,out]
    dC      DOUBLE PRECISION array of DIMENSION ( LDDC, n ).
    ...
    magma_int_t gnode[MagmaMaxGPUs][MagmaMaxGPUs + 2], magma_int_t ncmplx )
{
    ...
    magmaDouble_ptr dwork2[MagmaMaxGPUs];
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        ...
        int q = (ngpu > 1 ? 1 : 0);
        magmablas_dlaset( MagmaFull, lddwork, n, c_zero, c_zero, dwork(dev,0,0), lddwork, queues[dev][q] );
        
        // set C = 0, on queue 0 for col gemm, except on first device,
        // corresponding to offset, since that device multiples by beta.
        if (dev != stdev) {
            magmablas_dlaset( MagmaFull, lddc, n, c_zero, c_zero, dC(dev,0,0), lddc, queues[dev][0] );
        }
    }
    ...
    if (blockoffset > 0) {
        ...
        magmablas_dsymmetrize_tiles( MagmaLower, fstblksiz,
    ...
    for( dev = 0; dev < ngpu; ++dev ) {
        ...
        magma_setdevice( dev );
        magmablas_dsymmetrize_tiles( MagmaLower, nb,
        ...
        if (remm%nb > 0) {
            ...
            if (devperm == mydevlstblk) {
                magmablas_dsymmetrize( MagmaLower, remm % nb,
    ...
    if (ngpu > 1) {
        for( magma_int_t i = fstblksiz; i < m; i += nb ) {
            ...
            for( dev = 0; dev < ngpu; ++dev ) {
                ...
                if (myrowsize > 0) {
                    magma_setdevice( dev );
                    magma_dgemm( MagmaConjTrans, MagmaNoTrans, myrowsize, n, ib,
    ...
    if (blockoffset > 0) {
        ...
        magma_setdevice( stdev );
        magma_dgemm( MagmaNoTrans, MagmaNoTrans, m, n, ib,
    ...
    for( magma_int_t i = fstblksiz; i < m; i += nb ) {
        ...
        magma_setdevice( dev );
        if (i == 0) {
            magma_dgemm( MagmaNoTrans, MagmaNoTrans, m-i, n, ib,
        ...
        if (ngpu == 1) {
            ...
            magma_dgemm( MagmaConjTrans, MagmaNoTrans, i, n, ib,

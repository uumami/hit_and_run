    ...
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zgetmatrix_transpose_mgpu.cpp, normal z -> d, Sun Sep 15 17:19:12 2019
...
#define dAT(d, j)   (dAT[(d)]   + (j)*nb)
    ...
    for (j=0; j < n; j += nb) {
        ...
        ib = min(n-j, nb);
        magmablas_dtranspose( ib, m, dAT(d,j_local), ldda, dwork(d,id), lddw, queues[d][id] );

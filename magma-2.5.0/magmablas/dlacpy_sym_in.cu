/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zlacpy_sym_in.cu, normal z -> d, Sun Sep 15 17:16:26 2019
...
*/
static __device__
void dlacpy_sym_in_full_device(
    int m, int n,
...
*/
static __device__
void dlacpy_sym_in_lower_device(
    ...
    if ( ind < m ) {
        if ( full ) {
            ...
            for( int jj=0; jj < BLK_Y; ++jj ) 
            {
                int j = rows[2*(iby+jj)];
                if (perm[ind] <= j)
                    dB[ind + (iby+jj)*lddb] = MAGMA_D_CONJ( dA[j + perm[ind]*ldda] );
                else
...
*/
static __device__
void dlacpy_sym_in_upper_device(
    int m, int n,
    const double *dA, int ldda,
    double       *dB, int lddb )
{
...
*/
__global__
void dlacpy_sym_in_full_kernel(
    int m, int n,
    const double *dA, int ldda,
    double       *dB, int lddb )
{
    dlacpy_sym_in_full_device(m, n, dA, ldda, dB, lddb);
}

__global__
void dlacpy_sym_in_lower_kernel(
    int m, int n, magma_int_t *rows, magma_int_t *perm,
    const double *dA, int ldda,
    double       *dB, int lddb )
{
    dlacpy_sym_in_lower_device(m, n, rows, perm, dA, ldda, dB, lddb);
}

__global__
void dlacpy_sym_in_upper_kernel(
    int m, int n,
    const double *dA, int ldda,
    double       *dB, int lddb )
{
    dlacpy_sym_in_upper_device(m, n, dA, ldda, dB, lddb);
}
    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower ) {
        dlacpy_sym_in_lower_kernel<<< grid, threads, 0, queue->cuda_stream() >>> ( m, n, rows, perm, dA, ldda, dB, lddb );
    }
    else if ( uplo == MagmaUpper ) {
        dlacpy_sym_in_upper_kernel<<< grid, threads, 0, queue->cuda_stream() >>> ( m, n, dA, ldda, dB, lddb );
    }
    else {
        dlacpy_sym_in_full_kernel <<< grid, threads, 0, queue->cuda_stream() >>> ( m, n, dA, ldda, dB, lddb );
    }
}

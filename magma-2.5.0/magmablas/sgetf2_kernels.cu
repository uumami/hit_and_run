/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zgetf2_kernels.cu, normal z -> s, Sun Sep 15 17:21:00 2019
*/
...
/******************************************************************************/
__global__ void
isamax_kernel_batched(int length, int chunk, float **x_array, int xi, int xj, int incx,
                   int step, int lda, magma_int_t** ipiv_array, magma_int_t *info_array, int gbstep)
{
    float *x_start = x_array[blockIdx.z] + xj * lda + xi;
...
/******************************************************************************/
__global__ void
isamax_kernel_native(int length, int chunk, magmaFloat_ptr x, int incx, 
...
{
    float *x = x_array[blockIdx.z] + xj * incx + xi;
    magma_int_t *ipiv = ipiv_array[blockIdx.z] + xi;
...
template<int N>
__device__
void sscal_sger_device( int m, int step, 
                         ...
                         magma_int_t *info, int gbstep)
{
    ...
    magmaFloat_ptr A = dA + step + step * lda;
    float rA[N], reg; 
...
/******************************************************************************/
__device__
void sscal_sger_generic_device( int m, int n, int step, 
                         magmaFloat_ptr dA, int lda, 
                         magma_int_t *info, int gbstep)
{
    const int tx  = threadIdx.x;
    ...
    reg = MAGMA_S_DIV(MAGMA_S_ONE, A[0]);
    rA  = A[ gtx ];
    rA *= reg;
...
template<int N>
__global__
void sscal_sger_1d_kernel_native( int m, int step, 
                                magmaFloat_ptr dA, int lda, 
                                magma_int_t *info, int gbstep)
{
    ...
    sscal_sger_device<N>(m, step, dA, lda, info, gbstep);
}


/******************************************************************************/
__global__
void sscal_sger_1d_generic_kernel_native( int m, int n, int step, 
                                magmaFloat_ptr dA, int lda, 
                                magma_int_t *info, int gbstep)
{
    // This dev function has a return statement inside, be sure
    // not to merge it with another dev function. Otherwise, the 
    // return statement should be converted into an if-statement 
    sscal_sger_generic_device(m, n, step, dA, lda, info, gbstep);
}
...
template<int N>
__global__
void sscal_sger_1d_kernel_batched(int m, int step, float **dA_array, int ai, int aj, int lda, magma_int_t *info_array, int gbstep)
{
...
/******************************************************************************/
__global__
void sscal_sger_1d_generic_kernel_batched(int m, int n, int step, float **dA_array, int ai, int aj, int lda, magma_int_t *info_array, int gbstep)
{
    const int batchid = blockIdx.z;
    float* dA = dA_array[batchid] + aj * lda + ai;
    magma_int_t *info = &info_array[batchid];
                                      ...
                                      magma_int_t batchCount, magma_queue_t queue)
{
    ...
    */
    ...
    switch(n){
        case  1:sscal_sger_1d_kernel_batched< 1><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  2:sscal_sger_1d_kernel_batched< 2><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  3:sscal_sger_1d_kernel_batched< 3><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  4:sscal_sger_1d_kernel_batched< 4><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  5:sscal_sger_1d_kernel_batched< 5><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  6:sscal_sger_1d_kernel_batched< 6><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  7:sscal_sger_1d_kernel_batched< 7><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  8:sscal_sger_1d_kernel_batched< 8><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        default:sscal_sger_1d_generic_kernel_batched<<<grid, threads, 0, queue->cuda_stream()>>>(m, n, step, dA_array, ai, aj, lda, info_array, gbstep); 
    }
    return 0;
}
    ...
    magma_queue_t queue)
{
    ...
    */
    ...
    dim3 threads(tbx, 1, 1);
    switch(n){
        case 1:sscal_sger_1d_kernel_native<1><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA, lda, info, gbstep);break;
        case 2:sscal_sger_1d_kernel_native<2><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA, lda, info, gbstep);break;
        case 3:sscal_sger_1d_kernel_native<3><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA, lda, info, gbstep);break;
        case 4:sscal_sger_1d_kernel_native<4><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA, lda, info, gbstep);break;
        case 5:sscal_sger_1d_kernel_native<5><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA, lda, info, gbstep);break;
        case 6:sscal_sger_1d_kernel_native<6><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA, lda, info, gbstep);break;
        case 7:sscal_sger_1d_kernel_native<7><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA, lda, info, gbstep);break;
        case 8:sscal_sger_1d_kernel_native<8><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA, lda, info, gbstep);break;
        default:sscal_sger_1d_generic_kernel_native<<<grid, threads, 0, queue->cuda_stream()>>>( m, n, step, dA, lda, info, gbstep);
    }
    return 0;
}
                                      ...
                                      int lda, magma_int_t **ipiv_array, magma_int_t *info_array, int gbstep)
{
    ...
    float *A_start = dA_array[blockIdx.z] + aj * lda + ai;
...
template<int WIDTH>
__device__ void
sgetf2_fused_device( int m, float* dA, int ldda, magma_int_t* dipiv,
                   float* swork, magma_int_t *info, int gbstep)
{
    const int tx = threadIdx.x;
    const int ty = threadIdx.y;

    float rA[WIDTH] = {MAGMA_S_ZERO};
    float reg = MAGMA_S_ZERO; 

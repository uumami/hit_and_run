/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zsetmatrix_transpose.cpp, normal z -> s, Sun Sep 15 17:17:43 2019
...
#define dwork(i_, j_) (dwork + (i_) + (j_)*lddw)
    ...
    for (i=nb; i < n; i += nb) {
        ...
        magmablas_stranspose( m, nb, dwork(0,(j%2)*nb), lddw, dAT(i-nb,0), ldda, queues[j%2] );
    }

    /* Transpose the last part of the matrix.                            */
    j++;
    magmablas_stranspose( m, ib, dwork(0,(j%2)*nb), lddw, dAT(i-nb,0), ldda, queues[j%2] );
}

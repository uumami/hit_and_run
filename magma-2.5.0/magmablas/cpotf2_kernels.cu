/*
   ...
   @generated from magmablas/zpotf2_kernels.cu, normal z -> c, Sun Sep 15 17:23:44 2019
 */
        ...
        magma_int_t *dinfo, magma_queue_t queue)
{
    ...
    for (magma_int_t j = 0; j < n; j += POTF2_NB) {
        ...
        if (ib == POTF2_NB)
        {
            cpotf2_smlpout_fixwidth_kernel
                <<< dimGrid, threads, shared_mem_size, queue->cuda_stream() >>>
                (rows, dA, lda, j, gbstep, dinfo );
        } else {
            cpotf2_smlpout_anywidth_kernel
                <<< dimGrid, threads, shared_mem_size, queue->cuda_stream() >>>
        ...
        magma_int_t *dinfo, magma_queue_t queue)
{
    ...
    if( n % POTF2_NB == 0){
        cpotf2_smlpin_fixwidth_kernel
            <<< grid, threads, shared_mem_size, queue->cuda_stream() >>>
            (n, dA, ldda, 0, gbstep, dinfo);
    }
    else{
        cpotf2_smlpin_anywidth_kernel
            <<< grid, threads, shared_mem_size, queue->cuda_stream() >>>
            (n, dA, ldda, 0, gbstep, dinfo);
    }
    return arginfo;

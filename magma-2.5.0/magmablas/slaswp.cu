/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zlaswp.cu, normal z -> s, Sun Sep 15 17:17:32 2019
...
/***************************************************************************//**
    ...
    ** Unlike LAPACK, here A is stored row-wise (hence dAT). **
    ...
    @param[in,out]
    dAT     REAL array on GPU, stored row-wise, dimension (LDDA,M)
    ...
    magma_queue_t queue )
{
    ...
    dim3 threads( NTHREADS );
    dim3 grid( magma_ceildiv( n, NTHREADS ) );
    slaswp2_kernel<<< grid, threads, 0, queue->cuda_stream() >>>

/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zlascl2.cu, normal z -> d, Sun Sep 15 17:17:13 2019
    ...
    magma_int_t *info )
{
    *info = 0;
    ...
    if (type == MagmaLower) {
        dlascl2_lower <<< grid, threads, 0, queue->cuda_stream() >>> (m, n, dD, dA, ldda);
    }
    else if (type == MagmaUpper) {
        dlascl2_upper <<< grid, threads, 0, queue->cuda_stream() >>> (m, n, dD, dA, ldda);
    }
    else if (type == MagmaFull) {
        dlascl2_full  <<< grid, threads, 0, queue->cuda_stream() >>> (m, n, dD, dA, ldda);
    }
}

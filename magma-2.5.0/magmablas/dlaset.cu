/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zlaset.cu, normal z -> d, Sun Sep 15 17:17:25 2019
...
*/
__global__
void dlaset_full_kernel_vbatched(
    magma_int_t* m, magma_int_t* n,
    double offdiag, double diag,
    double **dAarray, magma_int_t* ldda )
{
    ...
    dlaset_full_device(my_m, my_n, offdiag, diag, dAarray[batchid], (int)ldda[batchid]);
}

__global__
void dlaset_lower_kernel_vbatched(
    magma_int_t* m, magma_int_t* n,
    double offdiag, double diag,
    double **dAarray, magma_int_t* ldda )
{
    ...
    if( blockIdx.x >= (my_m+BLK_X-1)/BLK_X ) return;
    if( blockIdx.y >= (my_n+BLK_Y-1)/BLK_Y ) return;
    dlaset_lower_device(my_m, my_n, offdiag, diag, dAarray[batchid], (int)ldda[batchid]);
}

__global__
void dlaset_upper_kernel_vbatched(
    magma_int_t* m, magma_int_t* n,
    double offdiag, double diag,
    double **dAarray, magma_int_t* ldda )
{
    ...
    if( blockIdx.x >= (my_m+BLK_X-1)/BLK_X ) return;
    if( blockIdx.y >= (my_n+BLK_Y-1)/BLK_Y ) return;
    dlaset_upper_device(my_m, my_n, offdiag, diag, dAarray[batchid], (int)ldda[batchid]);
}

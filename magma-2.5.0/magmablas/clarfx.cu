/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zlarfx.cu, normal z -> c, Sun Sep 15 17:17:08 2019
    ...
    if ( !MAGMA_C_EQUAL(*tau, MAGMA_C_ZERO) ) {
        ...
        for (int j = tx; j < m; j += BLOCK_SIZE) {
            if (j == 0) {
                ...
                lsum += MAGMA_C_MUL( MAGMA_C_CONJ( v[j] ), dc[j] );
        }
        sum[tx] = lsum;
        ...
        else
        {
            if (blockIdx.x == it)
                ...
                *(T+blockIdx.x) = MAGMA_C_CONJ(z__1);
        }
    }

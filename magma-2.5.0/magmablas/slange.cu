/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/zlange.cu, normal z -> s, Sun Sep 15 17:16:33 2019
       @author Mark Gates
*/
...
/***************************************************************************//**
    ...
    SLANGE returns the value
       ...
       SLANGE = ( max(abs(A(i,j))), NORM = MagmaMaxNorm
                (
    ...
    magma_queue_t queue )
{
    ...
    if ( norm == MagmaInfNorm ) {
        ...
        slange_inf_kernel<<< grid, threads, 0, queue->cuda_stream() >>>( m, n, dA, ldda, dwork );
        magma_max_nan_kernel<<< 1, 512, 0, queue->cuda_stream() >>>( m, dwork );
    }
    else if ( norm == MagmaMaxNorm ) {
        dim3 grid( magma_ceildiv( m, NB_X ) );
        slange_max_kernel<<< grid, threads, 0, queue->cuda_stream() >>>( m, n, dA, ldda, dwork );
        magma_max_nan_kernel<<< 1, 512, 0, queue->cuda_stream() >>>( m, dwork );
    }
    else if ( norm == MagmaOneNorm ) {
        dim3 grid( n );
        slange_one_kernel<<< grid, threads, 0, queue->cuda_stream() >>>( m, n, dA, ldda, dwork );

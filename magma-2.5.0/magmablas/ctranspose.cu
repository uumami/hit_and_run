...
__global__
void ctranspose_kernel_batched_stride(
    ...
    magma_queue_t queue )
{
    ...
    ctranspose_kernel<<< grid, threads, 0, queue->cuda_stream() >>>
...
extern "C" void
magmablas_ctranspose_batched_stride(
    ...
    magma_queue_t queue )
{
    ...
    dim3 threads( NX, NY, 1 );
    dim3 grid( magma_ceildiv( m, NB ), magma_ceildiv( n, NB ), batchCount );
    //ctranspose_kernel_batched_stride<<< grid, threads, 0, queue->cuda_stream() >>>
    ctranspose_kernel_batched_stride<<< grid, threads>>>

...
/*
    ...
    They are used in dgetf2_batched.cpp
*/
...
{
    double *x_start = x_array[blockIdx.z] + xj * lda + xi;
...
// cublasIdamax always writes 32bit pivots, so make sure it is magma_int_t
...
/******************************************************************************/
extern "C" magma_int_t
magma_idamax_native( magma_int_t length, 
                     ...
                     magma_int_t gbstep, magma_queue_t queue)
{
    ...
    if( length <= 15360 ) {
        ...
        int chunk = magma_ceildiv( length, zamax );
        idamax_kernel_native<<< grid, threads, zamax * (sizeof(double) + sizeof(int)), queue->cuda_stream() >>>
        ...
        cublasIdamax(queue->cublas_handle(), length, x + step * lda + step, 1, (int*)(ipiv+step));
...
{
    double *x = x_array[blockIdx.z] + xj * incx + xi;
    magma_int_t *ipiv = ipiv_array[blockIdx.z] + xi;
...
template<int N>
__device__
void dscal_dger_device( int m, int step, 
                         ...
                         magma_int_t *info, int gbstep)
{
    ...
    magmaDouble_ptr A = dA + step + step * lda;
    double rA[N], reg; 
...
/******************************************************************************/
__device__
void dscal_dger_generic_device( int m, int n, int step, 
                         magmaDouble_ptr dA, int lda, 
                         magma_int_t *info, int gbstep)
{
    const int tx  = threadIdx.x;
    ...
    reg = MAGMA_D_DIV(MAGMA_D_ONE, A[0]);
    rA  = A[ gtx ];
    rA *= reg;
...
template<int N>
__global__
void dscal_dger_1d_kernel_native( int m, int step, 
                                magmaDouble_ptr dA, int lda, 
                                magma_int_t *info, int gbstep)
{
    ...
    dscal_dger_device<N>(m, step, dA, lda, info, gbstep);
}


/******************************************************************************/
__global__
void dscal_dger_1d_generic_kernel_native( int m, int n, int step, 
                                magmaDouble_ptr dA, int lda, 
                                magma_int_t *info, int gbstep)
{
    // This dev function has a return statement inside, be sure
    // not to merge it with another dev function. Otherwise, the 
    // return statement should be converted into an if-statement 
    dscal_dger_generic_device(m, n, step, dA, lda, info, gbstep);
}
...
template<int N>
__global__
void dscal_dger_1d_kernel_batched(int m, int step, double **dA_array, int ai, int aj, int lda, magma_int_t *info_array, int gbstep)
{
...
/******************************************************************************/
__global__
void dscal_dger_1d_generic_kernel_batched(int m, int n, int step, double **dA_array, int ai, int aj, int lda, magma_int_t *info_array, int gbstep)
{
    const int batchid = blockIdx.z;
    double* dA = dA_array[batchid] + aj * lda + ai;
    magma_int_t *info = &info_array[batchid];
...
/******************************************************************************/
extern "C"
magma_int_t magma_dscal_dger_batched(magma_int_t m, magma_int_t n, magma_int_t step,
                                      ...
                                      magma_int_t batchCount, magma_queue_t queue)
{
    ...
    */
    ...
    switch(n){
        case  1:dscal_dger_1d_kernel_batched< 1><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  2:dscal_dger_1d_kernel_batched< 2><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  3:dscal_dger_1d_kernel_batched< 3><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  4:dscal_dger_1d_kernel_batched< 4><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  5:dscal_dger_1d_kernel_batched< 5><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  6:dscal_dger_1d_kernel_batched< 6><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  7:dscal_dger_1d_kernel_batched< 7><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        case  8:dscal_dger_1d_kernel_batched< 8><<<grid, threads, 0, queue->cuda_stream()>>>( m, step, dA_array, ai, aj, lda, info_array, gbstep);break;
        default:dscal_dger_1d_generic_kernel_batched<<<grid, threads, 0, queue->cuda_stream()>>>(m, n, step, dA_array, ai, aj, lda, info_array, gbstep); 
    }
    return 0;
}
                                      ...
                                      int lda, magma_int_t **ipiv_array, magma_int_t *info_array, int gbstep)
{
    ...
    double *A_start = dA_array[blockIdx.z] + aj * lda + ai;
...
template<int WIDTH>
__device__ void
dgetf2_fused_device( int m, double* dA, int ldda, magma_int_t* dipiv,
                   double* swork, magma_int_t *info, int gbstep)
{
    const int tx = threadIdx.x;
    const int ty = threadIdx.y;
...
magma_dgetf2_fused_batched(
    ...
    magma_queue_t queue)
{
    if(m < 0 || m > DGETF2_FUSED_BATCHED_MAX_ROWS) {
        fprintf( stderr, "%s: m = %4lld not supported, must be between 0 and %4lld\n",
                 __func__, (long long) m, (long long) DGETF2_FUSED_BATCHED_MAX_ROWS);
        return -1;
    }
    else if(n < 0 || n > 32){
    ...
    switch(n)
    {
        case  1: dgetf2_fused_batched_kernel< 1><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case  2: dgetf2_fused_batched_kernel< 2><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case  3: dgetf2_fused_batched_kernel< 3><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case  4: dgetf2_fused_batched_kernel< 4><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case  5: dgetf2_fused_batched_kernel< 5><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case  6: dgetf2_fused_batched_kernel< 6><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case  7: dgetf2_fused_batched_kernel< 7><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case  8: dgetf2_fused_batched_kernel< 8><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case  9: dgetf2_fused_batched_kernel< 9><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 10: dgetf2_fused_batched_kernel<10><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 11: dgetf2_fused_batched_kernel<11><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 12: dgetf2_fused_batched_kernel<12><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 13: dgetf2_fused_batched_kernel<13><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 14: dgetf2_fused_batched_kernel<14><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 15: dgetf2_fused_batched_kernel<15><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 16: dgetf2_fused_batched_kernel<16><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 17: dgetf2_fused_batched_kernel<17><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 18: dgetf2_fused_batched_kernel<18><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 19: dgetf2_fused_batched_kernel<19><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 20: dgetf2_fused_batched_kernel<20><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 21: dgetf2_fused_batched_kernel<21><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 22: dgetf2_fused_batched_kernel<22><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 23: dgetf2_fused_batched_kernel<23><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 24: dgetf2_fused_batched_kernel<24><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 25: dgetf2_fused_batched_kernel<25><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 26: dgetf2_fused_batched_kernel<26><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 27: dgetf2_fused_batched_kernel<27><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 28: dgetf2_fused_batched_kernel<28><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 29: dgetf2_fused_batched_kernel<29><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 30: dgetf2_fused_batched_kernel<30><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 31: dgetf2_fused_batched_kernel<31><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;
        case 32: dgetf2_fused_batched_kernel<32><<<grid, threads, shared_size, queue->cuda_stream()>>>(m, dA_array, ai, aj, ldda, dipiv_array, info_array, batchCount); break;

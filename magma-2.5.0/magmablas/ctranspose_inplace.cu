/*
    -- MAGMA (version 2.5.0) --
       ...
       @generated from magmablas/ztranspose_inplace.cu, normal z -> c, Sun Sep 15 17:18:10 2019
...
__global__ void ctranspose_inplace_odd(
    int n,
    magmaFloatComplex *matrix, int lda )
{
...
__global__ void ctranspose_inplace_even(
    int n,
    magmaFloatComplex *matrix, int lda )
{
    __shared__ magmaFloatComplex sA[ NB ][ NB+1 ];
    ...
    magma_queue_t queue )
{
    ...
    if ( nblock % 2 == 1 ) {
        ...
        ctranspose_inplace_odd<<< grid, threads, 0, queue->cuda_stream() >>>( n, dA, ldda );
    }
    else {
